...
void TakeNap(const double delaySeconds) {
  ...
  DBG_MESSAGE("wait for {} milliseconds",
...
ScopedTimer::ScopedTimer(const std::string &context, RE::TESObjectREFR *refr)
    : m_startTime(microsecondsNow()), m_context(context) {
  std::ostringstream fullContext;
  fullContext << context << ' '
              << (refr ? std::string(refr->GetBaseObject()->GetName())
                       : std::string())
              << "/0x"
              << StringUtils::FromFormID(
                     refr ? refr->GetBaseObject()->GetFormID() : InvalidForm);
...
// Encoded data consists of its length (type size_t) followed by the compressed
// version of the input
bool DecodeBrotli(const std::string &compressed, nlohmann::json &output) {
  if (compressed.length() < sizeof(size_t)) {
    REL_ERROR("Inflating {} bytes failed, record too short",
              compressed.length());
    return false;
  }
  size_t inputSize(compressed.length());
  size_t sizeHint(
      *reinterpret_cast<size_t *>(const_cast<char *>(compressed.c_str())));
  size_t outputSize(sizeHint);
  std::string inflated(outputSize, 0);
  BrotliDecoderResult result(BrotliDecoderDecompress(
      inputSize,
      reinterpret_cast<const uint8_t *>(compressed.c_str() + sizeof(size_t)),
      &outputSize,
      reinterpret_cast<uint8_t *>(const_cast<char *>(inflated.c_str()))));
  if (result == BROTLI_DECODER_RESULT_SUCCESS) {
    REL_MESSAGE("Inflated {} bytes to {} vs size-hint of {}", inputSize,
                outputSize, sizeHint);
    ...
    try {
      ...
      REL_ERROR("Inflated record not JSON-parsable '{}' etc. not loadable, "
                "error:\n{}",
                inflated.substr(0, 25), e.what());
      return false;
    }
  } else {
    REL_ERROR("Inflating {} bytes failed, error {}", inputSize,
              shse::BrotliDecoderResultString(result));
    return false;
  }
}
...
bool EncodeBrotli(const nlohmann::json &j, std::string &encoded) {
  ...
  size_t *length(
      reinterpret_cast<size_t *>(const_cast<char *>(encoded.c_str())));
  *length = jsonStr.length();
  static constexpr int BrotliQuality(1); // favour fast speed over small size
  if (BrotliEncoderCompress(
          BrotliQuality, BROTLI_DEFAULT_WINDOW, BROTLI_MODE_TEXT,
          jsonStr.length(), reinterpret_cast<const uint8_t *>(jsonStr.c_str()),
          &outputSize,
          reinterpret_cast<uint8_t *>(const_cast<char *>(
              encoded.c_str() + sizeof(size_t)))) != BROTLI_FALSE) {
    REL_MESSAGE("Compressed {} bytes to {}, ratio {:0.3f}", jsonStr.length(),
                outputSize, double(jsonStr.length()) / double(outputSize));
...
ParseFormsType(const nlohmann::json &formsType) {
  ...
  std::transform(formsType.begin(), formsType.end(), std::back_inserter(forms),
                 [&](const nlohmann::json &next) {
                   ...
                   return std::make_pair(next["plugin"].get<std::string>(),
                                         formIDs);
                 });
  return forms;
}
