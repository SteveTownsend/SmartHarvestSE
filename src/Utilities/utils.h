...
std::unordered_set<const FORMTYPE *> ToForms(const nlohmann::json &formsType) {
  ...
  for (const auto &entry : formNamesByPlugin) {
    for (const auto nextID : entry.second) {
      ...
      const FORMTYPE *form(shse::LoadOrder::Instance().LookupForm<FORMTYPE>(
          shse::LoadOrder::Instance().AsRaw(formID), entry.first));
      if (!form) {
        REL_WARNING("FormsCondition requires valid Forms, got {}/0x{:08x}",
