...
void CosaveData::Clear() {
  // Clear state that we will reseed. LoadOrder preserves current state and
  // compares to cosave version.
...
void CosaveData::SeedState() {
  ...
  for (const auto &record : m_records) {
    REL_MESSAGE("Seed state from cosave data {}",
                SerializationRecordName(record.first));
    try {
      switch (record.first) {
...
bool CosaveData::Serialize(SKSE::SerializationInterface *intf) {
  ...
  if (!intf->WriteRecord('LORD', 1, record.c_str(),
  ...
  if (!CompressionUtils::EncodeBrotli(shse::CollectionManager::Collectibles(),
                                      record)) {
    return false;
  }
  if (!intf->WriteRecord('COLL', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
  ...
  if (!CompressionUtils::EncodeBrotli(shse::VisitedPlaces::Instance(),
                                      record)) {
    return false;
  }
  if (!intf->WriteRecord('PLAC', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
  ...
  if (!intf->WriteRecord('PRTY', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
  ...
  if (!intf->WriteRecord('VCTM', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
  ...
  if (!CompressionUtils::EncodeBrotli(shse::AdventureTargets::Instance(),
                                      record)) {
    return false;
  }
  if (!intf->WriteRecord('ADVN', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
  ...
  if (!CompressionUtils::EncodeBrotli(
          shse::CollectionManager::ExcessInventory(), record)) {
    return false;
  }
  if (!intf->WriteRecord('EXCS', 1, record.c_str(),
                         static_cast<uint32_t>(record.length()))) {
...
bool CosaveData::Deserialize(SKSE::SerializationInterface *intf) {
  ...
  while (intf->GetNextRecordInfo(readType, version, length)) {
    ...
    shse::SerializationRecordType recordType(
        shse::SerializationRecordType::MAX);
    switch (readType) {
    case 'LORD':
      // Load Order
