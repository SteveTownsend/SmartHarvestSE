...
LoadOrder::LoadOrder()
    : m_shsePriority(-1), m_cosaveShsePriority(0),
      m_coSaveLoadOrderDiffers(false) {}
...
bool LoadOrder::Analyze(void) {
  ...
  // Process each ESP and ESPFE/ESL in the master list of RE::TESFile / loaded
  // mod instances
  int priority(0);
  for (const auto modFile : dhnd->files) {
    ...
    if (oldName.compare(0, oldName.length(), &modFile->fileName[0]) == 0) {
      REL_ERROR("Prior mod plugin version ({}) is incompatible with current "
                "plugin ({})",
                oldName.c_str(), &MODNAME[0]);
      return false;
    }
    ...
    m_loadInfoByName.insert(
        std::make_pair(modFile->fileName, LoadInfo({formIDMask, priority})));
    REL_MESSAGE("{} has FormID mask 0x{:08x}, priority {}", modFile->fileName,
                formIDMask, priority);
    ++priority;
  }
  return true;
...
RE::TESForm *LoadOrder::LookupForm(RE::FormID a_localFormID,
  ...
  if (g_mergeMapperInterface) {
    if (g_mergeMapperInterface->wasMerged(modName.c_str())) {
      const char *newName;
      RE::FormID newID;
      std::tie(newName, newID) =
          g_mergeMapperInterface->GetNewFormID(modName.c_str(), a_localFormID);
      DBG_VMESSAGE("Merged form {}/0x{:08x} has new FormID {}/0x{:08x}",
                   modName, formID, newName, newID);
      formID = newID;
      a_modName = newName;
    }
  }
...
RE::FormID LoadOrder::GetFormIDMask(const std::string &modName) const {
  ...
  if (g_mergeMapperInterface) {
    if (g_mergeMapperInterface->wasMerged(modName.c_str())) {
      ...
      RE::FormID newID;
      std::tie(newName, newID) =
          g_mergeMapperInterface->GetNewFormID(modName.c_str(), 0);
      matched = m_loadInfoByName.find(newName);
      if (matched != m_loadInfoByName.cend()) {
        DBG_VMESSAGE("Plugin {} has merged plugin FormIDMask {}/0x{:08x}",
                     modName, newName, matched->second.m_mask);
        return matched->second.m_mask;
      } else {
...
// returns true iff mod listed, which means it is active by virtue of exclusion
// of 0xff above
bool LoadOrder::IncludesMod(const std::string &modName) const {
  ...
  return g_mergeMapperInterface &&
         g_mergeMapperInterface->wasMerged(modName.c_str());
}
...
bool LoadOrder::ModPrecedesSHSE(const std::string &modName) const {
  ...
  auto matched(m_loadInfoByName.find(modName));
  if (matched != m_loadInfoByName.cend()) {
    DBG_VMESSAGE("Plugin {} has priority {}", modName,
                 matched->second.m_priority);
  ...
  if (g_mergeMapperInterface &&
    ...
    matched = m_loadInfoByName.find(newName);
    if (matched != m_loadInfoByName.cend()) {
      DBG_VMESSAGE("Plugin {} has merged plugin {} with priority {}", modName,
                   newName, matched->second.m_priority);
      return matched->second.m_priority < m_shsePriority;
    } else {
      REL_WARNING("Plugin {} cannot be mapped to any merged plugin", modName);
    }
...
bool LoadOrder::ModOwnsForm(const std::string &modName,
                            const RE::FormID formID) const {
  RecursiveLockGuard guard(m_loadLock);
  RE::FormID modMask(GetFormIDMask(modName));
  if (modMask == InvalidPlugin)
    return false;
  return (modMask & LightFormIDSentinel) == LightFormIDSentinel
             ? modMask == (formID & LightFormIDMask)
             : modMask == (formID & RegularFormIDMask);
}
...
void LoadOrder::AsJSON(nlohmann::json &j) const {
  ...
  std::for_each(m_loadInfoByName.cbegin(), m_loadInfoByName.cend(),
                [&](const auto &loadInfo) {
                  ordered.insert(
                      std::make_pair(loadInfo.second, loadInfo.first));
                });
...
// used to relink old form IDs to Load Order from saved game - no-op if the two
// Load Orders are the same
void LoadOrder::UpdateFrom(const nlohmann::json &j) {
  ...
  for (const auto &entry : j["order"]) {
    ...
    RE::FormID formID(
        StringUtils::ToFormID(entry["formIDMask"].get<std::string>()));
  ...
  // Need to reconcile cosave Load Order with current Load Order. Warn that this
  // may fail.
  m_coSaveLoadOrderDiffers = m_cosaveLoadInfoByName != m_loadInfoByName;
  if (m_coSaveLoadOrderDiffers) {
    REL_WARNING("Cosave Load Order inconsistent with current Load Order, "
                "FormIDs from cosave may be unusable");
  }
}
...
RE::TESForm *LoadOrder::RehydrateCosaveForm(const RE::FormID cosaveID) const {
  ...
  if (m_coSaveLoadOrderDiffers) {
    ...
    if (cosaveMod != m_cosaveModNameByMask.cend()) {
      target = RE::TESDataHandler::GetSingleton()->LookupForm(
          AsRaw(cosaveID), cosaveMod->second);
      if (target) {
        if (cosaveID != target->GetFormID()) {
          REL_WARNING("FormID {}/0x{:08x} in non-aligned cosave mapped into "
                      "current Load Order as 0x{:08x}",
                      cosaveMod->second, cosaveID, target->GetFormID());
        }
      } else {
        REL_WARNING("FormID {}/0x{:08x} in non-aligned cosave cannot be loaded",
                    cosaveMod->second, cosaveID);
      }
    } else {
      REL_WARNING("FormID 0x{:08x} in non-aligned cosave is not valid for "
                  "cosave Load Order",
                  cosaveID);
    }
  } else {
    // Load Order is the same, or mapping failed - check if FormID is still
    // valid
    target = RE::TESForm::LookupByID(cosaveID);
    if (target) {
      DBG_VMESSAGE("FormID 0x{:08x} from cosave mapped OK to 0x{:08x}",
                   cosaveID, target->GetFormID());
    } else {
      REL_WARNING("FormID 0x{:08x} from notionally identical cosave Load Order "
                  "cannot be loaded",
                  cosaveID);
    }
  }
...
// only used for CELLs, best guess mapping as they are not guaranteed to be
// in-RAM
RE::FormID LoadOrder::MapCosaveFormID(const RE::FormID cosaveID,
