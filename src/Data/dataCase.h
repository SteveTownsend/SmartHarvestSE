...
public:
  ...
  bool SetObjectTypeForFormID(const RE::FormID formID,
                              const ObjectType objectType);
  bool SetObjectTypeForForm(const RE::TESForm *form,
                            const ObjectType objectType);
  void ForceObjectTypeForForm(const RE::TESForm *form,
                              const ObjectType objectType);
  ObjectType GetObjectTypeForFormType(const RE::FormType formType) const;
  bool SetObjectTypeForFormType(const RE::FormType formType,
                                const ObjectType objectType);
  ...
  ConvertIfLeveledItem(const RE::TESBoundObject *form) const;
  ...
  inline bool IsBookGlowableKeyword(RE::BGSKeyword *keyword) const {
    return keyword && m_glowableBookKeywords.find(keyword->GetFormID()) !=
                          m_glowableBookKeywords.cend();
  }
  ...
  inline bool
  IsOffLimitsContainer(const RE::TESObjectREFR *containerRef) const {
  ...
  template <typename T>
  T *FindExactMatch(const std::string &defaultESP,
                    const RE::FormID maskedFormID) {
    ...
    if (typedForm) {
      DBG_MESSAGE("Found exact match 0x{:08x} for {}:0x{:06x}",
                  typedForm->GetFormID(), defaultESP.c_str(), maskedFormID);
    } else {
      DBG_MESSAGE("No exact match for {}:0x{:06x}", defaultESP.c_str(),
                  maskedFormID);
    }
    return typedForm;
  }
  ...
  void SetSyntheticFloraHarvested(const RE::TESObjectREFR *candidate,
...
private:
  ...
  std::unordered_map<const RE::TESProduceForm *, const RE::TESBoundObject *>
      m_produceFormContents;
  ...
  std::unordered_map<const RE::BGSPerk *, float>
  ...
  template <typename T> ObjectType DefaultIngredientObjectType(const T *form) {
    return ObjectType::unknown;
  }

  template <> ObjectType DefaultIngredientObjectType(const RE::TESFlora *form);
  template <>
  ObjectType DefaultIngredientObjectType(const RE::TESObjectTREE *form);

  class ProduceFormCategorizer : public LeveledItemCategorizer {
  public:
    ProduceFormCategorizer(RE::TESProduceForm *produceForm,
  ...
  template <typename T> void CategorizeByIngredient() {
    ...
    for (T *target : dhnd->GetFormArray<T>()) {
      ...
      if (!ingredient) {
        REL_WARNING("No ingredient for {}/0x{:08x}", targetName,
                    target->GetFormID());
        continue;
      }
      ...
      if (leveledItem) {
        ...
        REL_VMESSAGE("{}/0x{:08x} has ingredient Leveled Item, type {}",
                     targetName, target->GetFormID(),
        ...
        if (storedType != ObjectType::unknown) {
          REL_VMESSAGE(
              "{}/0x{:08x} has ingredient {}/0x{:08x} stored as type {}",
              targetName, target->GetFormID(), ingredient->GetName(),
              ingredient->GetFormID(), GetObjectTypeName(storedType).c_str());
          ProducerLootables::Instance().SetLootableForProducer(target,
                                                               ingredient);
        } else {
          storedType = DefaultIngredientObjectType(target);
          REL_VMESSAGE(
              "{}/0x{:08x} has ingredient {}/0x{:08x} with default type {}",
              targetName, target->GetFormID(), ingredient->GetName(),
              ingredient->GetFormID(), GetObjectTypeName(storedType).c_str());
        }
        if (storedType != ObjectType::unknown) {
          // Store mapping of Produce holder to ingredient - this is the most
          // correct type for this item producer
          SetObjectTypeForForm(target, storedType);
        } else {
          REL_WARNING("Target {}/0x{:08x} not stored", targetName,
                      target->GetFormID());
        }
      }
    }
  ...
  ObjectType DecorateIfEnchanted(const RE::TESForm *form,
                                 const ObjectType rawType);
  void SetObjectTypeByKeywords();

  template <typename T> ObjectType ConsumableObjectType(T *consumable) {
    return ObjectType::unknown;
  }

  template <>
  ObjectType ConsumableObjectType<RE::AlchemyItem>(RE::AlchemyItem *consumable);
  template <>
  ObjectType
  ConsumableObjectType<RE::IngredientItem>(RE::IngredientItem *consumable);

  template <typename T> void CategorizeConsumables() {
    RE::TESDataHandler *dhnd = RE::TESDataHandler::GetSingleton();
    if (!dhnd)
      return;
    for (T *consumable : dhnd->GetFormArray<T>()) {
      ...
      if (GetFormObjectType(consumable->GetFormID()) != ObjectType::unknown) {
        DBG_VMESSAGE("Skipping previously categorized form {}/0x{:08x}",
                     formName.c_str(), consumable->GetFormID());
        continue;
      }
  ...
  template <typename T> ObjectType DefaultObjectType() {
  ...
  template <typename T>
  ObjectType OverrideIfBadChoice(const RE::TESForm *,
                                 const ObjectType objectType) {
    return objectType;
  }
  template <>
  ObjectType
  OverrideIfBadChoice<RE::TESObjectARMO>(const RE::TESForm *form,
                                         const ObjectType objectType);
  template <>
  ObjectType
  OverrideIfBadChoice<RE::TESObjectWEAP>(const RE::TESForm *form,
                                         const ObjectType objectType);
  ...
  std::unordered_map<const RE::TESObjectACTI *, ResourceType>
      m_resourceTypeByOreVein;
  ...
  // Mirror the kHarvested check we get for free on FLOR/TREE records - store
  // REFR state and game-time-of-record
  mutable std::unordered_map<const RE::TESObjectREFR *, std::pair<bool, float>>
      m_syntheticFloraHarvested;

  ObjectType GetObjectTypeForActivationText(const std::string &verb) const;

  template <typename T> void CategorizeByKeyword() {
    RE::TESDataHandler *dhnd = RE::TESDataHandler::GetSingleton();
    if (!dhnd)
      return;
    ...
    for (T *typedForm : dhnd->GetFormArray<T>()) {
      ...
      if ((typedForm->formFlags & T::RecordFlags::kNonPlayable) ==
          T::RecordFlags::kNonPlayable) {
        DBG_VMESSAGE("{}/0x{:08x} is NonPlayable", formName,
                     typedForm->GetFormID());
        continue;
      }
      RE::BGSKeywordForm *keywordForm(typedForm->As<RE::BGSKeywordForm>());
      if (!keywordForm) {
        DBG_WARNING("{}/0x{:08x} Not a Keyword", formName,
                    typedForm->GetFormID());
        continue;
      }
      ...
      for (uint32_t index = 0; index < keywordForm->GetNumKeywords(); ++index) {
        std::optional<RE::BGSKeyword *> keyword(
            keywordForm->GetKeywordAt(index));
        if (!keyword)
          continue;
        // lock not needed during initialization
        const auto matched(
            m_objectTypeByForm.find(keyword.value()->GetFormID()));
        if (matched != m_objectTypeByForm.cend()) {
          // if default type, postpone storage in case there is a more specific
          // match
          if (matched->second == DefaultObjectType<T>()) {
            continue;
          }
          // Immersive Armors gives some armors the VendorItemSpellTome keyword.
          // No, they are not spellBook
          if (keyword == m_spellTomeKeyword) {
            // spellBook is not a legal mapping for MISC, ARMO or WEAP. Revisit
            // if we add more instantiations.
            REL_WARNING("Spell tome Keyword not valid for {}/0x{:08x}",
                        formName, typedForm->GetFormID());
          } else if (correctType != ObjectType::unknown) {
            REL_WARNING("{}/0x{:08x} mapped to {} already stored with keyword "
                        "{}, check data",
                        formName, typedForm->GetFormID(),
                        GetObjectTypeName(matched->second).c_str(),
                        GetObjectTypeName(correctType).c_str());
          } else {
      ...
      // Also, check model path for - you guessed it - clutter. Some base game
      // MISC objects lack keywords.
      if (typedForm->value > 0 || CheckObjectModelPath(typedForm, "clutter")) {
        SetObjectTypeForForm(typedForm, ObjectType::clutter);
        continue;
      }
      DBG_VMESSAGE("{}/0x{:08x} not mappable", formName,
                   typedForm->GetFormID());
    }
  }

  void GetTranslationData(void);
  void ActivationVerbsByType(const char *activationVerbKey,
                             const ObjectType objectType);
