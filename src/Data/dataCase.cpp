...
void DataCase::GetTranslationData() {
  ...
  path += std::string("_");
  path +=
      std::string((setting && setting->GetType() == RE::Setting::Type::kString)
                      ? setting->data.s
                      : "ENGLISH");
  path += std::string(".txt");
  ...
  while (true) {
    ...
    // use the rest of the line as a value, even if it's empty - omit trailing
    // whitespace
    size_t whitespace(0);
    ...
    std::wstring translation(buf.c_str() + offset,
                             buf.c_str() + buf.length() - whitespace);
    ...
    DBG_VMESSAGE("Translation entry: {} -> {}", keyS.c_str(),
                 translationS.c_str());
  }
...
// process comma-separated list of allowed ACTI verbs, to make localization
// INI-based
void DataCase::ActivationVerbsByType(const char *activationVerbKey,
  ...
  while (std::getline(verbStream, nextVerb, ',')) {
    auto inserted(m_objectTypeByActivationVerb.insert(
        std::make_pair(nextVerb, objectType)));
    if (inserted.second) {
      REL_MESSAGE("Activation Verb {}/{} registered as ObjectType {}",
                  activationVerbKey, nextVerb.c_str(),
                  GetObjectTypeName(objectType).c_str());
    } else {
      // dup verb in Translation file
      REL_WARNING(
          "Ignoring Activation verb {}/{} already registered as ObjectType {}",
          activationVerbKey, nextVerb.c_str(),
          GetObjectTypeName(inserted.first->second).c_str());
    }
  }
}

// Some activation verbs are used to handle referenced forms as a catch-all,
// though we prefer other rules
void DataCase::StoreActivationVerbs() {
  // https://github.com/SteveTownsend/SmartHarvestSE/issues/56
  // Clutter categorization here is not correct - typically these are quest
  // items that we need the player to activate maybe reinstate with a glow
  // function later ActivationVerbsByType("$SHSE_ACTIVATE_VERBS_CLUTTER",
  // ObjectType::clutter);
  ...
  // ActivationVerbsByType("$SHSE_ACTIVATE_VERBS_MANUAL",
  // ObjectType::manualLoot);
}

ObjectType
DataCase::GetObjectTypeForActivationText(const std::string &verb) const {
  // #436 support Oblivion Interaction Icons alt text by extending logic to
  // match as Posix Basic RE for use with std::basic_regex
  auto verbMatched(std::find_if(
      m_objectTypeByActivationVerb.cbegin(),
      m_objectTypeByActivationVerb.cend(),
      [&](const decltype(m_objectTypeByActivationVerb)::value_type &vt)
          -> bool {
        return std::regex_match(verb, std::basic_regex(vt.first));
      }));
...
void DataCase::CategorizeByActivationVerb() {
  ...
  for (RE::TESObjectACTI *activator : dhnd->GetFormArray<RE::TESObjectACTI>()) {
    ...
    if (activator->GetActivateText(RE::PlayerCharacter::GetSingleton(),
                                   activationText)) {
      // read Activation Text up to newline - the text we see is the data in the
      // ESP file, plus a newline, plus the target
      std::istringstream input;
      input.str(std::string(activationText));
      std::string verb;
      std::getline(input, verb);
      DBG_VMESSAGE("Categorizing {}/0x{:08x} by activation verb {}", formName,
                   activator->GetFormID(), verb);
      ...
      if (activatorType != ObjectType::unknown) {
        if (SetObjectTypeForForm(activator, activatorType)) {
          ...
          if (activatorType == ObjectType::oreVein) {
            ...
            if (std::string(formName).find(
                    std::string("Heart Stone Deposit")) !=
                    std::string::npos || // Dragonborn
                std::string(formName).find(std::string("Sulfur Deposit")) !=
                    std::string::npos) // CACO
            {
              resourceType = ResourceType::volcanic;
            } else if (std::string(formName).find(std::string("Geode")) !=
                       std::string::npos) {
              resourceType = ResourceType::geode;
            } else {
              resourceType = ResourceType::ore;
            }
            m_resourceTypeByOreVein.insert(
                std::make_pair(activator, resourceType));
            REL_VMESSAGE("{}/0x{:08x} has ResourceType {}", formName,
                         activator->GetFormID(),
                         PrintResourceType(resourceType));
          }
          // Creation Club and others use Harvest for ACTI. We bucket as flora
          // but need custom harvesting logic, like critter.
          else if (activatorType == ObjectType::flora) {
            m_syntheticFlora.insert(activator);
            REL_VMESSAGE("ACTI {}/0x{:08x} must be treated as flora", formName,
                         activator->GetFormID());
          }
        }
        continue;
    ...
    DBG_MESSAGE("{}/0x{:08x} not mappable, uses verb '{}'", formName,
                activator->GetFormID(), std::string(activationText));
  }
}
...
void DataCase::FindCraftingItems(void) {
  ...
  for (RE::BGSConstructibleObject *cobj :
       dhnd->GetFormArray<RE::BGSConstructibleObject>()) {
    unsigned int total(0);
    unsigned int added(0);
    cobj->requiredItems.ForEachContainerObject(
        [&](RE::ContainerObject &item) -> RE::BSContainer::ForEachResult {
    ...
    DBG_MESSAGE("Added {} of {} items for COBJ 0x{:08x}", added, total,
                cobj->GetFormID());
  }
}
...
void DataCase::AnalyzePerks(void) {
  ...
  for (const RE::BGSPerk *perk : dhnd->GetFormArray<RE::BGSPerk>()) {
    DBG_MESSAGE("Perk {}/0x{:08x} being checked", perk->GetName(),
                perk->GetFormID());
    for (const RE::BGSPerkEntry *perkEntry : perk->perkEntries) {
      if (perkEntry->GetType() != RE::PERK_ENTRY_TYPE::kEntryPoint)
        continue;

      const RE::BGSEntryPointPerkEntry *entryPoint(
          static_cast<const RE::BGSEntryPointPerkEntry *>(perkEntry));
        ...
        if (entryPoint->entryData.function ==
            ...
            entryPoint->functionData->GetType() ==
                RE::BGSEntryPointFunctionData::ENTRY_POINT_FUNCTION_DATA::
                    kOneValue) {
          const RE::BGSEntryPointFunctionDataOneValue *oneValued(
              static_cast<const RE::BGSEntryPointFunctionDataOneValue *>(
                  entryPoint->functionData));
          REL_MESSAGE("Modify Harvested Ingredients factor {:0.2f} from perk "
                      "{}/0x{:08x}",
                      oneValued->data, perk->GetName(), perk->GetFormID());
          m_modifyHarvestedPerkMultipliers.insert(
              std::make_pair(perk, oneValued->data));
        } else {
          REL_WARNING(
              "Modify Harvested Ingredients unsupported for perk {}/0x{:08x}, "
              "function {}, type {}",
              perk->GetName(), perk->GetFormID(),
              static_cast<std::uint8_t>(entryPoint->entryData.function.get()),
              entryPoint->functionData
                  ? int(entryPoint->functionData->GetType())
                  : -1);
        }
      }
    }
  }
  // List MGEF with Slow Time archetype, save the ones with Value Modifier on
  // Bow Speed Bonus
...
void DataCase::ExcludeFactionContainers() {
  ...
  for (RE::TESFaction *faction : dhnd->GetFormArray<RE::TESFaction>()) {
    ...
    if ((faction->data.flags & RE::FACTION_DATA::Flag::kVendor) ==
        RE::FACTION_DATA::Flag::kVendor) {
      containerRef = faction->vendorData.merchantContainer;
      if (containerRef) {
        REL_VMESSAGE("Blocked faction/vendor container : {}({:08x})",
                     containerRef->GetName(), containerRef->GetFormID());
    ...
    if (containerRef) {
      REL_VMESSAGE("Blocked stolenGoodsContainer : {}({:08x})",
                   containerRef->GetName(), containerRef->GetFormID());
      m_offLimitsContainers.insert(containerRef->GetFormID());
    }
    ...
    if (containerRef) {
      REL_VMESSAGE("Blocked playerInventoryContainer : {}({:08x})",
                   containerRef->GetName(), containerRef->GetFormID());
      m_offLimitsContainers.insert(containerRef->GetFormID());
    }
  }
...
bool DataCase::ReferencesBlacklistedContainer(
...
void DataCase::ExcludeVendorContainers() {
  ...
  // Vendor chests contain LVLI with substring VendorGold - there's no way to
  // check that on the fly because for LVLI records, EDID is not loaded Check
  // for exact match in Load Order using {plugin file, plugin-relative Form ID}
  // tuple
  ...
  std::vector<std::tuple<std::string, RE::FormID>> vendorItemLVLI = {
      ...
      {"Skyrim.esm", 0x9af0f}  // LItemMiscVendorLockpicks75 - catches a lot of
                               // mod-added vendor/merchant CONT
  };
  ...
  for (const auto &lvliDef : vendorItemLVLI) {
    ...
    if (lvliForm) {
      REL_MESSAGE("LVLI {}:0x{:08x} found for Vendor Container contents",
                  espName, lvliForm->GetFormID());
      vendorSentinels.insert(lvliForm);
    } else {
      REL_ERROR(
          "LVLI {}/0x{:08x} not found, should be Vendor Container contents",
          espName, formID);
    }
  }
  if (vendorSentinels.size() != vendorItemLVLI.size()) {
    REL_ERROR("LVLI count {} (base game) for Vendor Sentinels inconsistent "
              "with expected {}",
  ...
  std::vector<std::tuple<std::string, RE::FormID>> modVendorSentinelLVLI = {
      ...
      {"ccbgssse001-fish.esm",
       0x1223de}, // ccBGSSSE001_LItemVendorFishingClothes50
  ...
  for (const auto &container : oneOffContainers) {
    ...
    RE::TESObjectCONT *chestForm(
        FindExactMatch<RE::TESObjectCONT>(espName, formID));
    if (chestForm) {
      REL_MESSAGE("CONT {}:0x{:08x} added to Blacklist", espName,
                  chestForm->GetFormID());
  ...
  for (RE::TESObjectCONT *container : dhnd->GetFormArray<RE::TESObjectCONT>()) {
    if (m_containerBlackList.contains(container)) {
      DBG_MESSAGE("Skip already-blacklisted Container {}/0x{:08x}",
                  container->GetName(), container->GetFormID());
      continue;
    }
    ...
    container->ForEachContainerObject(
        [&](RE::ContainerObject &entry) -> RE::BSContainer::ForEachResult {
          auto entryContents(entry.obj);
          if (vendorSentinels.find(entryContents->As<RE::TESLevItem>()) !=
              vendorSentinels.cend()) {
            REL_MESSAGE("Block Vendor Container {}/0x{:08x}",
                        container->GetName(), container->GetFormID());
            matched = true;
            // only continue if insert fails, not that this will likely do much
            // good
            ...
            DBG_MESSAGE(
                "{}/0x{:08x} in Container {}/0x{:08x} not sentinel Vendor Item",
                entryContents->GetName(), entryContents->GetFormID(),
                container->GetName(), container->GetFormID());
          }
          // continue the scan
          return RE::BSContainer::ForEachResult::kContinue;
    ...
    if (!matched) {
      DBG_MESSAGE("Ignoring non-Vendor Container {}/0x{:08x}",
                  container->GetName(), container->GetFormID());
    }
  }
}
...
void DataCase::IncludeLootableContainers() {
  ...
  std::vector<std::tuple<std::string, RE::FormID>> oneOffContainers = {
      ...
      {"FEC.esp", 0x817}, {"FEC.esp", 0x825}, {"FEC.esp", 0x843},
      {"FEC.esp", 0x844}, {"FEC.esp", 0x86a}, {"FEC.esp", 0x876},
      {"FEC.esp", 0x908}};
  for (const auto &container : oneOffContainers) {
    std::string espName(std::get<0>(container));
    RE::FormID formID(std::get<1>(container));
    RE::TESObjectCONT *chestForm(
        FindExactMatch<RE::TESObjectCONT>(espName, formID));
    if (chestForm) {
      REL_MESSAGE("CONT {}:0x{:08x} added to Whitelist", espName,
                  chestForm->GetFormID());
...
void DataCase::ExcludeImmersiveArmorsGodChest() {
  ...
  RE::TESObjectCONT *godChestForm(FindExactMatch<RE::TESObjectCONT>(
      "Hothtrooper44_ArmorCompilation.esp", 0x4b352));
  if (godChestForm) {
    REL_MESSAGE("Block Immersive Armors 'all the loot' chest {}/0x{:08x}",
                godChestForm->GetName(), godChestForm->GetFormID());
    m_containerBlackList.insert(godChestForm);
  }
}

void DataCase::ExcludeGrayCowlStonesChest() {
  // check for best matching candidate in Load Order - use exact match as the
  // name is the very vague "Chest"
  RE::TESObjectCONT *stonesChestForm(
      FindExactMatch<RE::TESObjectCONT>("Gray Fox Cowl.esm", 0x1a184));
  if (stonesChestForm) {
    REL_MESSAGE("Block Gray Cowl Stones chest {}/0x{:08x}",
                stonesChestForm->GetName(), stonesChestForm->GetFormID());
...
void DataCase::CheckAutoMiningOK() {
  // check for incompatible Mining mods and disable auto-mining if any are
  // active
...
bool DataCase::AutoMiningDisabled() const { return m_miningDisabled; }

void DataCase::ExcludeBuildYourNobleHouseIncomeChest() {
  // check for matching form in Load Order
  RE::TESObjectCONT *incomeChestForm(
      FindExactMatch<RE::TESObjectCONT>("LC_BuildYourNobleHouse.esp", 0xaacdd));
  if (incomeChestForm) {
...
void DataCase::IncludeFossilMiningExcavation() {
  ...
  if (excavationSiteForm) {
    REL_MESSAGE("Record Fossil Mining Excavation Site {}/0x{:08x} as "
                "oreVein:volcanicDigSite",
                excavationSiteForm->GetName(), excavationSiteForm->GetFormID());
    SetObjectTypeForForm(excavationSiteForm, ObjectType::oreVein);
    m_resourceTypeByOreVein.insert(
        std::make_pair(excavationSiteForm->As<RE::TESObjectACTI>(),
                       ResourceType::volcanicDigSite));
  }
}
...
void DataCase::IncludeSeptimSpecialCases() {
  ...
  for (const auto goldPileFormID : pilesOfGold) {
    RE::TESForm *goldPileForm(RE::TESDataHandler::GetSingleton()->LookupForm(
        goldPileFormID, espName));
    if (goldPileForm) {
...
void DataCase::IncludeCoinReplacerRedux() {
  ...
  for (const auto coinPileFormID : pilesOfCoin) {
    RE::TESForm *coinPileForm(
        LoadOrder::Instance().LookupForm(coinPileFormID, crrName));
    if (coinPileForm) {
...
void DataCase::IncludeCorpseCoinage() {
  ...
  RE::TESForm *corpseCoinageForm(
      LoadOrder::Instance().LookupForm(corpseCoinageFormID, espName));
...
void DataCase::IncludeBSBruma() {
  ...
  RE::TESForm *ayleidGoldForm(RE::TESDataHandler::GetSingleton()->LookupForm(
      ayleidGoldFormID, espName));
  if (ayleidGoldForm) {
...
void DataCase::IncludeCOIN() {
  ...
  static std::vector<RE::FormID> interestingCoins(
      {0xde5012, 0xde5013, 0xde5014, 0xde5015, 0xde5016, 0xde5017, 0xde5018,
       0xde5019, 0xde5020, 0xde5021, 0xde5022, 0xde5023, 0xde5024});
  for (const auto coinFormID : interestingCoins) {
    RE::TESForm *coinForm(RE::TESDataHandler::GetSingleton()->LookupForm(
        coinFormID, injectedName));
    if (coinForm) {
...
void DataCase::HandleHearthfireExtendedApiary() {
  ...
  const RE::TESForm *apiaryForm(
      LoadOrder::Instance().LookupForm(apiaryFormID, espName));
  if (apiaryForm && apiaryForm->Is(RE::FormType::Activator)) {
    // the ACTI can be inspected repeatedly and (after first pass) fruitlessly
...
void DataCase::RecordUnderwear() {
  ...
  m_underwear = CollectionManager::SpecialCases().CollectionByLabel(
...
void DataCase::RecordOffLimitsLocations() {
  ...
  for (const auto &pluginForm : illegalCells) {
    ...
    if (cell) {
      REL_MESSAGE("No looting in cell {}/0x{:08x}", cell->GetName(),
                  cell->GetFormID());
...
void DataCase::RecordPlayerHouses(void) {
  ...
  for (const auto &pluginForm : houseCells) {
    ...
    const RE::TESObjectCELL *cell(
        FindExactMatch<RE::TESObjectCELL>(espName, formID));
    if (cell) {
      REL_MESSAGE("CELL {}/0x{:08x} treated as Player House", cell->GetName(),
                  cell->GetFormID());
  ...
  std::vector<std::tuple<std::string, RE::FormID>> houseLocations = {
      {"ccvsvsse004-beafarmer.esl", 0xd38}
      // ccVSVSSE004_BunkhouseInteriorLocation
  };
  for (const auto &pluginForm : houseLocations) {
    std::string espName(std::get<0>(pluginForm));
    RE::FormID formID(std::get<1>(pluginForm));
    const RE::BGSLocation *location(
        FindExactMatch<RE::BGSLocation>(espName, formID));
    if (location) {
      REL_MESSAGE("LCTN {}/0x{:08x} treated as Player House",
...
void DataCase::BlockOffLimitsContainers() {
  // block all the known off-limits containers - list is invariant during gaming
  // session
  RecursiveLockGuard guard(m_blockListLock);
...
void DataCase::BlockFirehoseSource(const RE::TESObjectREFR *refr) {
  ...
  if (!refr)
    return;
  // looted REFR was 'blocked while I am in this cell' before the triggering
  // event was fired
...
void DataCase::AddFirehose(const RE::TESForm *form) {
  RecursiveLockGuard guard(m_blockListLock);
  REL_MESSAGE("Record 0x{:08x}/{} as FireHose", form->GetFormID(),
              form->GetName());
  m_firehoseForms.insert(form);
}

void DataCase::BlockReference(const RE::TESObjectREFR *refr,
                              const Lootability reason) {
  if (!refr
...
void DataCase::BlockReferenceByID(const RE::FormID refrID,
                                  const Lootability reason) {
  RecursiveLockGuard guard(m_blockListLock);
...
Lootability DataCase::IsReferenceBlocked(const RE::TESObjectREFR *refr) const {
  ...
  return blocked == m_blockRefr.cend() ? Lootability::Lootable
                                       : blocked->second;
}
...
void DataCase::ResetBlockedReferences(const bool gameReload) {
  ...
  // Volcanic dig sites from Fossil Mining and firehose item sources are only
  // cleared on game reload, to simulate the delay in becoming lootable again.
  // Only allow one auto-loot visit per gaming session, unless player dies.
  // Firehose item sources are BYOH mined materials and HearthfiresExtended
  // Apiary
  ...
  for (const auto refrID : m_blockRefr) {
    ...
    if (GetEffectiveObjectType(refr->GetBaseObject()) == ObjectType::oreVein &&
        OreVeinResourceType(refr->GetBaseObject()->As<RE::TESObjectACTI>()) ==
            ResourceType::volcanicDigSite) {
      firehouseSources.insert(refrID.first);
    }
  }
  DBG_MESSAGE("Reset blocked REFRs apart from {} volcanic and {} firehose",
              firehouseSources.size() - m_firehoseSources.size(),
              m_firehoseSources.size());
...
void DataCase::UnblockReferences(const Lootability reason) {
  ...
  for (auto refr = m_blockRefr.begin(); refr != m_blockRefr.end();) {
    if (refr->second == reason) {
      DBG_VMESSAGE("Reset blocked REFR 0x{:08x} with reason {}", refr->first,
                   LootabilityName(reason));
...
void DataCase::RefreshKnownIngredients() {
  ...
  for (auto &ingredientFlag : m_ingredientEffectsKnown) {
    RE::IngredientItem *ingredient(
        RE::TESForm::LookupByID<RE::IngredientItem>(ingredientFlag.first));
    if (!ingredient) {
...
bool DataCase::AllEffectsKnown(const RE::IngredientItem *ingredient) const {
  ...
  for (const auto effect : ingredient->effects) {
    if (!effect->baseEffect->GetKnown()) {
      DBG_VMESSAGE("Found unknown effect {}/0x{:08x} on ingredient {}/0x{:08x}",
                   effect->baseEffect->GetFullName(),
                   effect->baseEffect->GetFormID(), ingredient->GetFullName(),
  ...
  if (known) {
    DBG_VMESSAGE("All effects known for ingredient {}/0x{:08x}",
                 ingredient->GetFullName(), ingredient->GetFormID());
  }
  return known;
}

bool DataCase::IsEphemeralForm(const RE::TESForm *form) const {
  return form->GetFormType() != RE::FormType::AlchemyItem &&
         form->IsDynamicForm();
}
...
bool DataCase::BlockForm(const RE::TESForm *form, const Lootability reason) {
  if (!form)
    return false;
  // ephemeral dynamic forms must never be recorded as their FormID may be
  // reused
...
// used for BlackList Collections. Also blocks the form for this loaded game,
// and on reload.
bool DataCase::BlockFormPermanently(const RE::TESForm *form,
                                    const Lootability reason) {
  if (!form)
    return false;
#if NDEBUG && !defined(_FULL_LOGGING)
...
bool DataCase::SetObjectTypeForFormID(const RE::FormID formID,
                                      const ObjectType objectType) {
  const RE::TESForm *target(RE::TESForm::LookupByID(formID));
  if (target) {
    return SetObjectTypeForForm(target, objectType);
  } else {
    REL_WARNING("FormID 0x{:08x} cannot be loaded, ignoring attempt to "
                "categorize as {}",
                formID, GetObjectTypeName(objectType));
    return false;
  }
}
...
void DataCase::RegisterPlayerCreatedALCH(RE::AlchemyItem *consumable) {
  ...
  REL_MESSAGE("Register player-created ALCH {}/0x{:08x} as {}",
              consumable->GetFullName(), consumable->GetFormID(),
              GetObjectTypeName(objectType));
  SetObjectTypeForForm(consumable, objectType);
}

[[nodiscard]] bool
DataCase::IsSyntheticFlora(const RE::TESBoundObject *boundObject) const {
  if (!boundObject)
    return false;
  RecursiveLockGuard guard(m_blockListLock);
...
[[nodiscard]] bool
DataCase::IsSyntheticFloraHarvested(const RE::TESObjectREFR *candidate) const {
  RecursiveLockGuard guard(m_blockListLock);
  if (candidate) {
    auto harvestedState(m_syntheticFloraHarvested.find(candidate));
    if (harvestedState != m_syntheticFloraHarvested.end()) {
      // reset after a sensible interval so we can check if the synthetic flora
      // script has respawned the yielded loot
      constexpr float SyntheticFloraResetInterval(5.0);
      float elapsed(RE::Calendar::GetSingleton()->GetCurrentGameTime() -
                    harvestedState->second.second);
      if (elapsed > SyntheticFloraResetInterval) {
        REL_MESSAGE("Reset Harvested flag for REFR 0x{:08x} synthetic Flora "
                    "0x{:08x} after {:.2f} game-days",
                    candidate->GetFormID(),
                    candidate->GetBaseObject()->GetFormID(), elapsed);
...
void DataCase::SetSyntheticFloraHarvested(const RE::TESObjectREFR *candidate,
                                          const bool isHarvested) {
  RecursiveLockGuard guard(m_blockListLock);
  if (candidate) {
    m_syntheticFloraHarvested[candidate] = {
        isHarvested, RE::Calendar::GetSingleton()->GetCurrentGameTime()};
  }
}

bool DataCase::SetObjectTypeForForm(const RE::TESForm *form,
                                    const ObjectType objectType) {
  const auto inserted(
      m_objectTypeByForm.insert(std::make_pair(form->GetFormID(), objectType)));
  std::string name(form->GetName());
  if (name.empty()) {
    name = FormUtils::SafeGetFormEditorID(form);
  }
  if (inserted.second) {
    REL_VMESSAGE("{}/0x{:08x} uses ObjectType {}", name, form->GetFormID(),
                 GetObjectTypeName(objectType));
    ...
    REL_WARNING("Cannot use ObjectType {} for {}/0x{:08x}, already using {}",
                GetObjectTypeName(objectType), name, form->GetFormID(),
                GetObjectTypeName(inserted.first->second));
  }
  return inserted.second;
}

void DataCase::ForceObjectTypeForForm(const RE::TESForm *form,
                                      const ObjectType objectType) {
  auto inserted(
      m_objectTypeByForm.insert(std::make_pair(form->GetFormID(), objectType)));
  if (inserted.second) {
    REL_VMESSAGE("{}/0x{:08x} force-categorized as {}", form->GetName(),
                 form->GetFormID(), GetObjectTypeName(objectType).c_str());
    if (objectType == ObjectType::ingredient) {
      m_ingredientEffectsKnown.insert({form->GetFormID(), false});
    }
  } else {
    REL_WARNING("{}/0x{:08x} force-categorized as {}, overwriting {}",
...
ObjectType
DataCase::GetObjectTypeForFormType(const RE::FormType formType) const {
...
bool DataCase::SetObjectTypeForFormType(const RE::FormType formType,
                                        const ObjectType objectType) {
  const auto inserted(m_objectTypeByFormType.insert({formType, objectType}));
  if (inserted.second) {
    REL_VMESSAGE("Formtype {} categorized as {}", GetFormTypeName(formType),
                 GetObjectTypeName(objectType));
  } else {
    REL_WARNING("Cannot categorize FormType {} as {}, already stored as {}",
                GetFormTypeName(formType), GetObjectTypeName(objectType),
                GetObjectTypeName(inserted.first->second));
  }
  return inserted.second;
}
...
ResourceType
DataCase::OreVeinResourceType(const RE::TESObjectACTI *mineable) const {
...
DataCase::ConvertIfLeveledItem(const RE::TESBoundObject *form) const {
...
bool DataCase::SkipAmmoLooting(RE::TESObjectREFR *refr) {
  ...
  if (pos == RE::NiPoint3()) {
    DBG_VMESSAGE("Arrow position unknown {:0.2f},{:0.2f},{:0.2f}", pos.x, pos.y,
                 pos.z);
  ...
  if (!m_arrowCheck.contains(refr)) {
    DBG_VMESSAGE("Newly detected, save arrow position {:0.2f},{:0.2f},{:0.2f}",
                 pos.x, pos.y, pos.z);
    ...
    if (prev != pos) {
      ...
      // check for in-flight arrow movement speed is scaled if player is current
      // experiencing Slow Time
      const double ArrowInFlightUnitsPerScan(
          PlayerState::Instance().ArrowMovingThreshold());
      if (fabs(dx) > ArrowInFlightUnitsPerScan ||
          fabs(dy) > ArrowInFlightUnitsPerScan ||
          fabs(dz) > ArrowInFlightUnitsPerScan) {
        DBG_VMESSAGE("In flight: change in arrow position "
                     "dx={:0.2f},dy={:0.2f},dz={:0.2f} vs {:0.2f}",
                     dx, dy, dz, ArrowInFlightUnitsPerScan);
        m_arrowCheck[refr] = pos;
        skip = true;
      } else {
        DBG_VMESSAGE("OK, not in flight, change in arrow position "
                     "dx={:0.2f},dy={:0.2f},dz={:0.2f} vs {:0.2f}",
                     dx, dy, dz, ArrowInFlightUnitsPerScan);
      }
    } else {
...
void DataCase::CategorizeLootables() {
  ...
  // consumable item categorization is useful for Activator, Flora, Tree and
  // direct access
  ...
  // Classes inheriting from TESProduceForm may have an ingredient, categorized
  // as the appropriate consumable This 'ingredient' can be MISC (e.g. Coin
  // Replacer Redux Coin Purses) so those must be done first, as above by
  // keyword or forcing as septims
  ...
  // Handle any special cases based on Load Order, including base game 'known
  // exceptions'
...
void DataCase::HandleExceptions() {
  // on first pass, detect off limits containers and other special cases to
  // avoid rescan on game reload
...
void DataCase::SetObjectTypeByKeywords() {
  ...
  std::unordered_set<std::string> glowableBooks = {// Legacy of the Dragonborn
                                                   "VendorItemJournal",
                                                   "VendorItemNote"};
  ...
  for (RE::BGSKeyword *keywordDef : dhnd->GetFormArray<RE::BGSKeyword>()) {
    ...
    if (keywordName.empty()) {
      REL_WARNING("KYWD record 0x{:08x} has missing/blank EDID, skip",
                  keywordDef->GetFormID());
      continue;
    }
    // Store player house keyword for SearchTask usage
    if (keywordName == "LocTypePlayerHouse" ||
        keywordName == "BYOHHouseLocationKeyword") {
      REL_VMESSAGE("Found PlayerHouse KYWD {}/0x{:08x}", keywordName,
                   keywordDef->GetFormID());
      PlayerHouses::Instance().AddLocationKeyword(keywordDef);
      continue;
    }
    // Immersive Armors gives some armors the VendorItemSpellTome keyword. No,
    // they are not spellBook
    if (keywordName == "VendorItemSpellTome") {
      REL_VMESSAGE("Found Spell Tome KYWD {}/0x{:08x}", keywordName,
                   keywordDef->GetFormID());
      m_spellTomeKeyword = keywordDef;
    }
    // SPERG mining resource types
    if (keywordName == "VendorItemOreIngot" || keywordName == "VendorItemGem") {
      REL_VMESSAGE("Found SPERG Prospector Perk resource type KYWD {}/0x{:08x}",
                   keywordName, keywordDef->GetFormID());
      ScanGovernor::Instance().SetSPERGKeyword(keywordDef);
    }
    if (glowableBooks.find(keywordName) != glowableBooks.cend()) {
      REL_VMESSAGE("Found Glowable Book KYWD {}/0x{:08x}", keywordName,
                   keywordDef->GetFormID());
    ...
    if (matched != typeByKeyword.cend()) {
    ...
    } else if (const auto substringMatch =
                   std::find_if(
                       typeByVendorItemSubstring.cbegin(),
                       typeByVendorItemSubstring.cend(),
                       [&](const std::pair<std::string, ObjectType> &comparand)
                           -> bool {
                         if (keywordName.find(comparand.first) !=
                             std::string::npos) {
      ...
      DBG_VMESSAGE("KYWD 0x{:08x} ({}) matched substring",
                   keywordDef->GetFormID(), keywordName);
    } else {
      DBG_VMESSAGE("KYWD 0x{:08x} ({}) skipped", keywordDef->GetFormID(),
                   keywordName.c_str());
      continue;
    }
    SetObjectTypeForForm(keywordDef,
                         DecorateIfEnchanted(keywordDef, objectType));
  }
}
...
template <>
ObjectType
DataCase::OverrideIfBadChoice<RE::TESObjectARMO>(const RE::TESForm *form,
                                                 const ObjectType objectType) {
...
template <>
ObjectType
DataCase::OverrideIfBadChoice<RE::TESObjectWEAP>(const RE::TESForm *form,
                                                 const ObjectType objectType) {
...
template <>
ObjectType
DataCase::ConsumableObjectType<RE::AlchemyItem>(RE::AlchemyItem *consumable) {
  const static RE::FormID drinkSound = 0x0B6435;
  if (consumable->IsFood())
    return (consumable->data.consumptionSound &&
            consumable->data.consumptionSound->formID == drinkSound)
               ? ObjectType::drink
               : ObjectType::food;
  else
...
template <>
ObjectType
DataCase::ConsumableObjectType<RE::IngredientItem>(RE::IngredientItem *) {
  return ObjectType::ingredient;
}

bool DataCase::PerksAddLeveledItemsOnDeath(const RE::Actor *actor) const {
  const auto deathPerk = std::find_if(
      m_leveledItemOnDeathPerks.cbegin(), m_leveledItemOnDeathPerks.cend(),
      [=](const RE::BGSPerk *perk) -> bool {
        return actor->HasPerk(const_cast<RE::BGSPerk *>(perk));
      });
  if (deathPerk != m_leveledItemOnDeathPerks.cend()) {
    DBG_VMESSAGE("Leveled item added at death for perk {}/0x{:08x}",
                 (*deathPerk)->GetName(), (*deathPerk)->GetFormID());
    return true;
  }
  return false;
}
...
float DataCase::PerkIngredientMultiplier(const RE::Actor *actor) const {
  ...
  std::for_each(
      m_modifyHarvestedPerkMultipliers.cbegin(),
      m_modifyHarvestedPerkMultipliers.cend(), [&](const auto &perkEntry) {
        if (actor->HasPerk(const_cast<RE::BGSPerk *>(perkEntry.first))) {
          if (matched) {
            DBG_VMESSAGE("Perk conflict ingredient harvesting via {}/0x{:08x}, "
                         "discarding",
                         perkEntry.first->GetName(),
                         perkEntry.first->GetFormID());
          } else {
            DBG_VMESSAGE(
                "Perk {}/0x{:08x} used for harvesting, multiplier {:0.2f}",
                perkEntry.first->GetName(), perkEntry.first->GetFormID(),
                perkEntry.second);
...
bool DataCase::CheckObjectModelPath(const RE::TESForm *thisForm,
...
void DataCase::CategorizeStatics() {
  ...
  auto wispCoreForm(
      RE::TESDataHandler::GetSingleton()->LookupForm(WispCore, "Skyrim.esm"));
  ...
  for (const auto nonPotion : nonPotions) {
    // using modname does not always work because formids with 0x1cc prefix are
    // hard-coded to not follow the pattern
    RE::TESForm *nonPotionForm(RE::TESForm::LookupByID(nonPotion.first));
    if (!nonPotionForm) {
      nonPotionForm = RE::TESDataHandler::GetSingleton()->LookupForm(
          nonPotion.first, cacoName);
    }
    if (nonPotionForm) {
  ...
  static std::vector<RE::FormID> clayOrStone(
      {0x9f2, 0x9f3, 0xA14, 0x306b, 0x310b, 0xa511});
  for (const auto clayOrStoneFormID : clayOrStone) {
    RE::TESForm *clayOrStoneForm(RE::TESDataHandler::GetSingleton()->LookupForm(
        clayOrStoneFormID, hearthFiresName));
    if (clayOrStoneForm) {
...
DataCase::ProduceFormCategorizer::ProduceFormCategorizer(
    RE::TESProduceForm *produceForm, const RE::TESLevItem *rootItem,
    const std::string &targetName)
    : LeveledItemCategorizer(rootItem), m_targetName(targetName),
      m_produceForm(produceForm), m_contents(nullptr),
      m_contentsType(ObjectType::unknown) {}

void DataCase::ProduceFormCategorizer::ProcessContentLeaf(
    RE::TESBoundObject *itemForm, ObjectType itemType) {
  if (!m_contents) {
    REL_VMESSAGE("Target {}/0x{:08x} has contents type {} in form {}/0x{:08x}",
                 m_targetName, m_rootItem->GetFormID(),
                 GetObjectTypeName(itemType), itemForm->GetName(),
                 itemForm->GetFormID());
    if (!DataCase::GetInstance()
             ->m_produceFormContents.insert({m_produceForm, itemForm})
             .second) {
      REL_WARNING("Leveled Item {}/0x{:08x} contents already present",
                  m_targetName, m_rootItem->GetFormID());
    } else {
    ...
    DBG_VMESSAGE("Target {}/0x{:08x} contents type {} already recorded",
                 m_targetName, m_rootItem->GetFormID(),
                 GetObjectTypeName(itemType));
  } else {
    // 'ingredient' supersedes 'food' as object type - currently this is the
    // only clash, and 'ingredient' is subjectively assumed more likely to be
    // wanted by the average user than 'food'
    ObjectType existingType(
        DataCase::GetInstance()->GetObjectTypeForForm(m_contents));
    if (itemType == ObjectType::ingredient &&
        existingType == ObjectType::food) {
      REL_WARNING("Target {}/0x{:08x} contents type {} overwriting type {} for "
                  "form {}/0x{:08x}",
                  m_targetName, m_rootItem->GetFormID(),
                  GetObjectTypeName(itemType), GetObjectTypeName(existingType),
                  m_contents->GetName(), m_contents->GetFormID());
      DataCase::GetInstance()->ForceObjectTypeForForm(m_contents, itemType);
      m_contentsType = itemType;
    } else {
      REL_WARNING("Target {}/0x{:08x} contents type {} already stored under "
                  "different form {}/0x{:08x} as type {}",
                  m_targetName, m_rootItem->GetFormID(),
                  GetObjectTypeName(itemType), m_contents->GetName(),
                  m_contents->GetFormID(), GetObjectTypeName(existingType));
    }
  }
}
