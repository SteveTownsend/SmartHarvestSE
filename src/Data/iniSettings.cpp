  ...
  if (result) {
    ...
    for (itSection = beginSection(); itSection != endSection(); ++itSection) {
      for (itKey = beginKey(*itSection); itKey != endKey(*itSection); ++itKey)
        DBG_VMESSAGE("[{}] {} = {:0.2f} ", (*itSection).c_str(),
                     (*itKey).c_str(),
                     GetValue<double>(*itSection, *itKey, 0.0));
    }
#endif
  }
  return result;
...
bool INIFile::CreateSectionString(PrimaryType m_section_first,
  ...
  if (!IsType(m_section_first) || !IsType(m_section_second))
    ...
    return false;
  if ((section[1] = PrimaryTypeString(m_section_first)).empty() ||
      (section[2] = SecondaryTypeString(m_section_second)).empty())
    return false;
...
double INIFile::GetSetting(PrimaryType m_section_first,
                           SecondaryType m_section_second, std::string m_key) {
  std::string section;
  ...
  DBG_DMESSAGE("Get config setting {}/{}/{} = {}",
               INIFile::PrimaryTypeString(m_section_first),
               INIFile::SecondaryTypeString(m_section_second), key.c_str(),
               setting);
  return setting;
}

void INIFile::PutSetting(PrimaryType m_section_first,
                         SecondaryType m_section_second, std::string m_key,
                         double m_value) {
  std::string section;
  std::string key = m_key;
