...
void SettingsCache::Refresh(void) {
  ...
  // Value for feet per unit from
  // https://www.creationkit.com/index.php?title=Unit
  double dSetting(ini->GetSetting(INIFile::PrimaryType::harvest,
                                  INIFile::SecondaryType::config,
                                  "RadiusFeet"));
  m_outdoorsRadius = dSetting / DistanceUnitInFeet;
  REL_VMESSAGE("Search radius outdoors {:0.2f} feet -> {:0.2f} units", dSetting,
               m_outdoorsRadius);

  dSetting =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "IndoorsRadiusFeet");
  m_indoorsRadius = dSetting / DistanceUnitInFeet;
  REL_VMESSAGE("Search radius indoors {:0.2f} feet -> {:0.2f} units", dSetting,
               m_indoorsRadius);

  m_verticalFactor =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "VerticalRadiusFactor");
  REL_VMESSAGE("Vertical search radius factor {:0.2f}", m_verticalFactor);

  m_respectDoors = ini->GetSetting(INIFile::PrimaryType::harvest,
                                   INIFile::SecondaryType::config,
                                   "DoorsPreventLooting") != 0.;
  REL_VMESSAGE("Respect Doors as loot barriers {}", m_respectDoors);

  m_unencumberedInPlayerHome =
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config,
                      "UnencumberedInPlayerHome") != 0.0;
  REL_VMESSAGE("Unencumbered in Player House {}", m_unencumberedInPlayerHome);
  m_unencumberedIfWeaponDrawn =
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config,
                      "UnencumberedIfWeaponDrawn") != 0.0;
  REL_VMESSAGE("Unencumbered if weapon drawn {}", m_unencumberedIfWeaponDrawn);
  m_unencumberedInCombat = ini->GetSetting(INIFile::PrimaryType::common,
                                           INIFile::SecondaryType::config,
                                           "UnencumberedInCombat") != 0.0;
  REL_VMESSAGE("Unencumbered when in combat {}", m_unencumberedInCombat);

  m_disableDuringCombat = ini->GetSetting(INIFile::PrimaryType::harvest,
                                          INIFile::SecondaryType::config,
                                          "DisableDuringCombat") != 0.0;
  REL_VMESSAGE("Disable during combat {}", m_disableDuringCombat);
  m_disableWhileWeaponIsDrawn =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config,
                      "DisableWhileWeaponIsDrawn") != 0.0;
  REL_VMESSAGE("Disable while weapon is drawn {}", m_disableWhileWeaponIsDrawn);
  m_disableWhileConcealed = ini->GetSetting(INIFile::PrimaryType::harvest,
                                            INIFile::SecondaryType::config,
                                            "DisableWhileConcealed") != 0.0;
  REL_VMESSAGE("Disable while concealed {}", m_disableWhileConcealed);
  m_fortuneHuntingEnabled = ini->GetSetting(INIFile::PrimaryType::common,
                                            INIFile::SecondaryType::config,
                                            "FortuneHuntingEnabled") != 0.0;
  REL_VMESSAGE("Fortune Hunting Enabled {}", m_fortuneHuntingEnabled);
  m_fortuneHuntItem =
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config, "FortuneHuntItem") != 0.0;
  REL_VMESSAGE("Fortune Hunt Item {}", m_fortuneHuntItem);
  m_fortuneHuntNPC =
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config, "FortuneHuntNPC") != 0.0;
  REL_VMESSAGE("Fortune Hunt NPC {}", m_fortuneHuntNPC);
  m_fortuneHuntContainer = ini->GetSetting(INIFile::PrimaryType::common,
                                           INIFile::SecondaryType::config,
                                           "FortuneHuntContainer") != 0.0;
  REL_VMESSAGE("Fortune Hunt Container {}", m_fortuneHuntContainer);
  m_collectionsEnabled = ini->GetSetting(INIFile::PrimaryType::common,
                                         INIFile::SecondaryType::config,
                                         "CollectionsEnabled") != 0.0;
  REL_VMESSAGE("Collections Enabled {}", m_collectionsEnabled);
  m_notifyLocationChange = ini->GetSetting(INIFile::PrimaryType::common,
                                           INIFile::SecondaryType::config,
                                           "NotifyLocationChange") != 0.0;
  REL_VMESSAGE("Notify Player of Location Change {}", m_notifyLocationChange);

  m_valuableItemThreshold =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "ValuableItemThreshold");
  REL_VMESSAGE("Valuable Item Threshold {:0.2f}", m_valuableItemThreshold);
  m_checkWeightlessValue = ini->GetSetting(INIFile::PrimaryType::harvest,
                                           INIFile::SecondaryType::valueWeight,
                                           "CheckWeightlessValue") != 0.0;
  REL_VMESSAGE("Check Weightless Value {}", m_checkWeightlessValue);
  m_weightlessMinimumValue = static_cast<int>(ini->GetSetting(
      INIFile::PrimaryType::harvest, INIFile::SecondaryType::valueWeight,
      "WeightlessMinimumValue"));
  REL_VMESSAGE("Weightless Minimum Value {}", m_weightlessMinimumValue);
  m_valueWeightDefault =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "ValueWeightDefault");
  ...
  for (size_t index = 1; index <= TypeCount; ++index, ++vw, ++lootingType,
              ++excessHandling, ++excessCount, ++excessWeight) {
    std::string typeName(GetObjectTypeName(ObjectType(index)));
    *vw = ini->GetSetting(INIFile::PrimaryType::harvest,
                          INIFile::SecondaryType::valueWeight, typeName);
    REL_VMESSAGE("Value Weight {:0.2f} for {} items", *vw, typeName);
    *lootingType = LootingTypeFromIniSetting(
        ini->GetSetting(INIFile::PrimaryType::harvest,
                        INIFile::SecondaryType::itemObjects, typeName));
    REL_VMESSAGE("Looting Type {} for {} items",
                 LootingTypeString(*lootingType), typeName);
    *excessHandling = ExcessInventoryHandlingFromIniSetting(
        ini->GetSetting(INIFile::PrimaryType::harvest,
                        INIFile::SecondaryType::excessHandling, typeName));
    REL_VMESSAGE("Excess Handling {} for {} items",
                 ExcessInventoryHandlingString(*excessHandling), typeName);
    *excessCount = static_cast<int>(
        ini->GetSetting(INIFile::PrimaryType::harvest,
                        INIFile::SecondaryType::maxItems, typeName));
    REL_VMESSAGE("Excess Count {} for {} items", *excessCount, typeName);
    *excessWeight =
        ini->GetSetting(INIFile::PrimaryType::harvest,
                        INIFile::SecondaryType::maxWeight, typeName);
    REL_VMESSAGE("Excess Weight {:0.2f} for {} items", *excessWeight, typeName);
  }
  // convert from percentage to multiplier
  m_saleValuePercentMultiplier =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "SaleValuePercent") /
      100.0;
  REL_VMESSAGE("Sale Value Percent Multiplier {}",
               m_saleValuePercentMultiplier);
  m_handleExcessCraftingItems =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config,
                      "HandleExcessCraftingItems") != 0;
  REL_VMESSAGE("Handle Excess Crafting Items {}", m_handleExcessCraftingItems);
  m_craftingItemsExcessHandling =
      ExcessInventoryHandlingFromIniSetting(ini->GetSetting(
          INIFile::PrimaryType::harvest, INIFile::SecondaryType::config,
          "CraftingItemsExcessHandling"));
  REL_VMESSAGE("Crafting Item Excess Handling {}",
               ExcessInventoryHandlingString(m_craftingItemsExcessHandling));
  m_craftingItemsExcessCount = static_cast<int>(ini->GetSetting(
      INIFile::PrimaryType::harvest, INIFile::SecondaryType::config,
      "CraftingItemsExcessCount"));
  REL_VMESSAGE("Crafting Items Excess Count {}", m_craftingItemsExcessCount);
  m_craftingItemsExcessWeight = ini->GetSetting(INIFile::PrimaryType::harvest,
                                                INIFile::SecondaryType::config,
                                                "CraftingItemsExcessWeight");
  REL_VMESSAGE("Crafting Items Excess Weight {:0.2f}",
               m_craftingItemsExcessWeight);

  m_deadBodyLooting = DeadBodyLootingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config, "EnableLootDeadbody"));
  REL_VMESSAGE("Dead Body Looting Type {}",
               DeadBodyLootingString(m_deadBodyLooting));
  m_enchantedObjectHandling = EnchantedObjectHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "EnchantedItemLoot"));
  REL_VMESSAGE("Enchanted Object Handling {}",
               EnchantedObjectHandlingString(m_enchantedObjectHandling));

  m_delaySeconds =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "IntervalSeconds");
  REL_VMESSAGE("Scan interval {:0.2f} seconds", m_delaySeconds);
  m_delaySecondsIndoors =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "IndoorsIntervalSeconds");
  REL_VMESSAGE("Scan interval indoors {:0.2f} seconds", m_delaySecondsIndoors);

  m_crimeCheckSneaking = OwnershipRuleFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "CrimeCheckSneaking"));
  REL_VMESSAGE("Crime Check Sneaking {}",
               OwnershipRuleName(m_crimeCheckSneaking));
  m_crimeCheckNotSneaking = OwnershipRuleFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "CrimeCheckNotSneaking"));
  REL_VMESSAGE("Crime Check Not Sneaking {}",
               OwnershipRuleName(m_crimeCheckNotSneaking));
  m_playerBelongingsLoot = SpecialObjectHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "PlayerBelongingsLoot"));
  REL_VMESSAGE("Player Belongings Loot {}",
               SpecialObjectHandlingName(m_playerBelongingsLoot));
  m_lockedChestLoot = LockedContainerHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "LockedChestLoot"));
  REL_VMESSAGE("Locked Chest Loot {}",
               LockedContainerHandlingName(m_lockedChestLoot));
  m_bossChestLoot = SpecialObjectHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "BossChestLoot"));
  REL_VMESSAGE("Boss Chest Loot {}",
               SpecialObjectHandlingName(m_bossChestLoot));
  m_valuableItemLoot = SpecialObjectHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "ValuableItemLoot"));
  REL_VMESSAGE("Valuable Item Loot {}",
               SpecialObjectHandlingName(m_valuableItemLoot));
  m_playContainerAnimation =
      ContainerAnimationHandlingFromIniSetting(ini->GetSetting(
          INIFile::PrimaryType::harvest, INIFile::SecondaryType::config,
          "PlayContainerAnimation"));
  REL_VMESSAGE("Play Container Animation {}",
               ContainerAnimationHandlingName(m_playContainerAnimation));
  m_questObjectLoot = QuestObjectHandlingFromIniSetting(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "QuestObjectLoot"));
  REL_VMESSAGE("Quest Object Loot {}",
               QuestObjectHandlingName(m_questObjectLoot));

  m_enableLootContainer = ini->GetSetting(INIFile::PrimaryType::common,
                                          INIFile::SecondaryType::config,
                                          "EnableLootContainer") != 0.0;
  REL_VMESSAGE("Enable Loot Container {}", m_enableLootContainer);
  m_enableHarvest =
      ini->GetSetting(INIFile::PrimaryType::common,
                      INIFile::SecondaryType::config, "EnableHarvest") != 0.0;
  REL_VMESSAGE("Enable Harvest {}", m_enableHarvest);

  m_lootAllowedItemsInSettlement =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config,
                      "LootAllowedItemsInSettlement") != 0;
  REL_VMESSAGE("Loot Allowed Items In Settlement {}",
               m_lootAllowedItemsInSettlement);
  m_lootAllowedItemsInPlayerHouse =
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config,
                      "LootAllowedItemsInPlayerHouse") != 0;
  REL_VMESSAGE("Loot Allowed Items In Player House {}",
               m_lootAllowedItemsInPlayerHouse);
  m_unknownIngredientLoot = ini->GetSetting(INIFile::PrimaryType::harvest,
                                            INIFile::SecondaryType::config,
                                            "UnknownIngredientLoot") != 0;
  REL_VMESSAGE("Unknown Ingredient Loot {}", m_unknownIngredientLoot);
  m_whiteListTargetNotify = ini->GetSetting(INIFile::PrimaryType::harvest,
                                            INIFile::SecondaryType::config,
                                            "WhiteListTargetNotify") != 0;
  REL_VMESSAGE("White List Target Notify {}", m_whiteListTargetNotify);
  m_manualLootTargetNotify = ini->GetSetting(INIFile::PrimaryType::harvest,
                                             INIFile::SecondaryType::config,
                                             "ManualLootTargetNotify") != 0;
  REL_VMESSAGE("Manual Loot Target Notify {}", m_manualLootTargetNotify);

  m_preventPopulationCenterLooting =
      PopulationCenterSizeFromIniSetting(INIFile::GetInstance()->GetSetting(
          INIFile::PrimaryType::common, INIFile::SecondaryType::config,
          "PreventPopulationCenterLooting"));
  REL_VMESSAGE("Prevent Population Center Looting {}",
               PopulationCenterSizeName(m_preventPopulationCenterLooting));
  m_maxMiningItems = static_cast<int16_t>(
      ini->GetSetting(INIFile::PrimaryType::harvest,
                      INIFile::SecondaryType::config, "MaxMiningItems"));
  REL_VMESSAGE("Max Mining Items {}", m_maxMiningItems);
  m_miningToolsRequired = ini->GetSetting(INIFile::PrimaryType::harvest,
                                          INIFile::SecondaryType::config,
                                          "MiningToolsRequired") != 0;
  REL_VMESSAGE("Mining Tools Required {}", m_miningToolsRequired);
  m_disallowMiningIfSneaking = ini->GetSetting(INIFile::PrimaryType::harvest,
                                               INIFile::SecondaryType::config,
                                               "DisallowMiningIfSneaking") != 0;
  REL_VMESSAGE("Disallow mining if sneaking {}", m_disallowMiningIfSneaking);
}

double SettingsCache::OutdoorsRadius() const { return m_outdoorsRadius; }
double SettingsCache::IndoorsRadius() const { return m_indoorsRadius; }
double SettingsCache::VerticalFactor() const { return m_verticalFactor; }
bool SettingsCache::RespectDoors() const { return m_respectDoors; }
...
bool SettingsCache::CollectionsEnabled() const { return m_collectionsEnabled; }
...
ExcessInventoryHandling
SettingsCache::ExcessInventoryHandlingType(ObjectType objectType) const {
  size_t index(std::min(TypeCount, size_t(objectType)) - 1);
...
double SettingsCache::DelaySeconds() const {
  double delay(LocationTracker::Instance().IsPlayerIndoors()
                   ? m_delaySecondsIndoors
                   : m_delaySeconds);
...
bool SettingsCache::EnableHarvest() const { return m_enableHarvest; }
...
int16_t SettingsCache::MaxMiningItems() const { return m_maxMiningItems; }
