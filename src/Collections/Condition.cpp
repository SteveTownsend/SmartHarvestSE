...
bool PluginCondition::operator()(const ConditionMatcher &matcher) const {
  for (const auto plugin : m_formIDMaskByPlugin) {
    if (LoadOrder::Instance().ModOwnsForm(plugin.first,
                                          matcher.Form()->GetFormID()))
      return true;
  }
  return false;
}
...
FormListCondition::FormListCondition(
    const std::vector<std::pair<std::string, std::string>> &pluginFormList) {
  for (const auto &entry : pluginFormList) {
    // schema enforces 8-char HEX format
    RE::FormID formID(StringUtils::ToFormID(entry.second));
    RE::BGSListForm *formList(LoadOrder::Instance().LookupForm<RE::BGSListForm>(
        LoadOrder::Instance().AsRaw(formID), entry.first));
    if (!formList) {
      REL_ERROR("FormListCondition cannot resolve FormList {}/0x{:08x}",
                entry.first.c_str(), formID);
      return;
    }
...
void FormListCondition::FlattenMembers(const RE::BGSListForm *formList) {
  ...
  for (const auto candidate : formList->forms) {
    if (candidate->GetFormType() == RE::FormType::FormList) {
      ...
      DBG_VMESSAGE("FormList Member found {}/0x{:08x}", candidate->GetName(),
                   candidate->GetFormID());
...
FormsCondition::FormsCondition(
  ...
  for (const auto &entry : pluginForms) {
    ...
    for (const auto nextID : entry.second) {
      ...
      RE::TESForm *form(LoadOrder::Instance().LookupForm(
          LoadOrder::Instance().AsRaw(formID), entry.first));
      if (!form) {
        REL_ERROR("FormsCondition requires valid Forms, got {}/0x{:08x}",
                  entry.first.c_str(), formID);
        return;
      }
...
// This is O(n) in KYWD record count but only happens during startup, and there
...
KeywordCondition::KeywordCondition(const std::vector<std::string> &keywords) {
  ...
  // store keywords to match for this collection. Schema enforces uniqueness in
  // input list.
  std::vector<std::string> keywordsLeft(keywords);
  for (const RE::BGSKeyword *keywordRecord :
       dhnd->GetFormArray<RE::BGSKeyword>()) {
    auto matched(std::find_if(keywordsLeft.begin(), keywordsLeft.end(),
                              [&](const std::string &keyword) -> bool {
                                return FormUtils::SafeGetFormEditorID(
                                           keywordRecord) == keyword;
                              }));
    if (matched != keywordsLeft.end()) {
      m_keywords.insert(keywordRecord);
      DBG_VMESSAGE("BGSKeyword recorded for {}",
                   FormUtils::SafeGetFormEditorID(keywordRecord).c_str());
...
bool KeywordCondition::operator()(const ConditionMatcher &matcher) const {
  const RE::BGSKeywordForm *keywordHolder(
      matcher.Form()->As<RE::BGSKeywordForm>());
  if (!keywordHolder)
    return false;
  return std::find_if(m_keywords.cbegin(), m_keywords.cend(),
                      [=](const RE::BGSKeyword *keyword) -> bool {
...
// This condition relies on objectType at scan time - it cannot be reliably
// determined at startup, so Collection Members are not resolved then.
CategoryCondition::CategoryCondition(
  ...
  for (const auto &category : categories) {
    ...
    if (objectType != ObjectType::unknown) {
      ...
      DBG_VMESSAGE("Category {} mapped to ObjectType {}", category.c_str(),
...
bool CategoryCondition::operator()(const ConditionMatcher &matcher) const {
  // short linear scan
  return std::find(m_objectTypes.cbegin(), m_objectTypes.cend(),
                   matcher.GetObjectType()) != m_objectTypes.cend();
}
...
const std::unordered_map<std::string, RE::FormType>
    SignatureCondition::m_validSignatures = {
        {"ALCH", RE::FormType::AlchemyItem}, {"ARMO", RE::FormType::Armor},
        {"BOOK", RE::FormType::Book},        {"INGR", RE::FormType::Ingredient},
        {"KEYM", RE::FormType::KeyMaster},   {"MISC", RE::FormType::Misc},
        {"SLGM", RE::FormType::SoulGem},     {"WEAP", RE::FormType::Weapon}};

const std::vector<RE::FormType> SignatureCondition::m_validFormTypes = {
    RE::FormType::AlchemyItem, RE::FormType::Armor,     RE::FormType::Book,
    RE::FormType::Ingredient,  RE::FormType::KeyMaster, RE::FormType::Misc,
    RE::FormType::SoulGem,     RE::FormType::Weapon};

// This is O(n) in TESV Record Type count but only happens during startup, and
...
SignatureCondition::SignatureCondition(
  ...
  for (const auto &signature : signatures) {
    ...
    if (matched != m_validSignatures.cend()) {
      ...
      DBG_VMESSAGE("Record Signature {} mapped to FormType {}",
                   signature.c_str(), static_cast<int>(matched->second));
    }
  }
}
...
bool SignatureCondition::operator()(const ConditionMatcher &matcher) const {
  // short linear scan
  return std::find(m_formTypes.cbegin(), m_formTypes.cend(),
                   matcher.Form()->GetFormType()) != m_formTypes.cend();
}

std::string
SignatureCondition::FormTypeAsSignature(const RE::FormType formType) {
...
const decltype(SignatureCondition::m_validSignatures)
SignatureCondition::ValidSignatures() {
  return m_validSignatures;
}

bool SignatureCondition::IsValidFormType(const RE::FormType formType) {
  return std::find(m_validFormTypes.cbegin(), m_validFormTypes.cend(),
                   formType) != m_validFormTypes.cend();
}
...
const std::unordered_map<std::string, INIFile::SecondaryType>
    ScopeCondition::m_validScopes = {
...
ScopeCondition::ScopeCondition(const std::vector<std::string> &scopes) {
  for (const auto &scope : scopes) {
    ...
    if (matched != m_validScopes.cend()) {
      ...
      std::string target(
          INIFile::GetInstance()->SecondaryTypeString(matched->second));
      DBG_VMESSAGE("Scope {} mapped to FormType {}", scope.c_str(),
                   target.c_str());
    }
  }
}
...
bool ScopeCondition::operator()(const ConditionMatcher &matcher) const {
  // Scope is aggregated during form filtering game-data load, for use in live
  // checking
  ...
  return std::find(m_scopes.cbegin(), m_scopes.cend(), matcher.Scope()) !=
         m_scopes.cend();
}

std::string
ScopeCondition::SecondaryTypeAsScope(const INIFile::SecondaryType scope) {
  // very short linear scan, for file dump
...
NameMatchCondition::NameMatchCondition(const bool isNPC,
                                       const std::string &matchIf,
                                       const std::vector<std::string> &names)
    : m_isNPC(isNPC) {
  m_matchIf = NameMatchTypeByName(matchIf);
  if (m_matchIf != NameMatchType::Invalid) {
    DBG_VMESSAGE("Match {} mapped to NameMatchType {}", matchIf,
                 static_cast<int>(m_matchIf));
  }
  // skip empty values
  std::copy_if(std::cbegin(names), std::cend(names),
...
FilterTree::FilterTree(const Operator op, const unsigned int depth)
    : m_operator(op), m_depth(depth) {}
...
void CategoryRule::AsJSON(nlohmann::json &j) const { m_condition->AsJSON(j); }
...
ConditionMatcher::ConditionMatcher(const RE::TESForm *form)
    : m_form(form), m_scope(INIFile::SecondaryType::NONE2),
...
// accumulates scopes seen in FilterTree during game-data load, to optimize
// collectible check during periodic REFR scan
void ConditionMatcher::AddScope(const INIFile::SecondaryType scope) const {
  if (std::find(m_scopesSeen.cbegin(), m_scopesSeen.cend(), scope) ==
      m_scopesSeen.cend()) {
