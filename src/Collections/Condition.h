...
public:
  FormsCondition(
      const std::vector<std::pair<std::string, std::vector<std::string>>>
          &pluginForms);
  virtual std::unordered_set<const RE::TESForm *>
  StaticMembers() const override;
  virtual bool operator()(const ConditionMatcher &matcher) const;
  virtual void AsJSON(nlohmann::json &j) const override;
...
public:
  // Store Form Types to match for this collection. Schema enforces uniqueness
  // and validity in input list. The list below must match the JSON schema and
  // CommonLibSSE RE::FormType.
...
private:
  ...
  static const std::unordered_map<std::string, INIFile::SecondaryType>
      m_validScopes;
};
...
public:
  NameMatchCondition(const bool isNPC, const std::string &matches,
...
private:
  inline bool ResultIfAllNamesFail() const {
    return m_matchIf == NameMatchType::NotEquals ||
           m_matchIf == NameMatchType::Omits;
  }
...
public:
  ...
  virtual std::unordered_set<const RE::TESForm *>
  StaticMembers() const override;
  virtual std::vector<ObjectType> GetObjectTypes(void) const override {
...
public:
  ...
  inline const std::vector<INIFile::SecondaryType> &ScopesSeen() const {
    return m_scopesSeen;
  }
...
private:
  ...
  // INIFile::SecondaryType::NONE2 is a sentinel to indicate no filtering on
  // scope, during game-data load
