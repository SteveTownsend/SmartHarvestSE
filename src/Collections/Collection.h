...
public:
  CollectionPolicy(const CollectibleHandling action, const bool notify,
                   const bool repeat)
  ...
  inline bool operator==(const CollectionPolicy &rhs) const {
    return m_action == rhs.m_action && m_notify == rhs.m_notify &&
...
class ItemCollected {
public:
  ItemCollected(const RE::TESForm *item, const Collection *collection,
...
protected:
  ...
  virtual void SetMemberFrom(const nlohmann::json &member,
                             const RE::TESForm *form) = 0;
...
public:
  Collection(const CollectionGroup *owningGroup, const std::string &name,
             const std::string &description, const CollectionPolicy &policy,
             const bool overridesGroup, std::unique_ptr<ItemRule> filter);
  virtual ~Collection();
  bool IsActive() const;
  void Disable();
  ...
  InScopeAndCollectibleFor(const ConditionMatcher &matcher) const;
  ...
  inline void SetPolicy(const CollectionPolicy &policy) {
    m_effectivePolicy = policy;
  }
  inline bool OverridesGroup() const { return m_overridesGroup; }
  inline void SetOverridesGroup(const bool overridesGroup) {
    m_overridesGroup = overridesGroup;
  }
  virtual size_t Count() const = 0;
  inline size_t Observed() const { return m_observed.size(); }
  virtual std::string GetStatusMessage() const = 0;
  bool HaveObserved(const RE::TESForm *form) const;
  bool RecordItem(const RE::TESForm *form, const float gameTime,
                  const bool suppressSpam);
  void Reset();

  nlohmann::json MakeJSON() const;
  void AsJSON(nlohmann::json &j) const;
  void UpdateFrom(const nlohmann::json &collectionState,
  ...
  inline void SetScopes(const std::vector<INIFile::SecondaryType> &scopes) {
    m_scopes = scopes;
  }
...
public:
  ...
  virtual inline std::vector<ObjectType> ObjectTypes(void) const override {
    return std::vector<ObjectType>();
  }
  virtual inline size_t Count() const override { return m_members.size(); }
  virtual inline std::string GetStatusMessage() const override {
    return "$SHSE_CONDITION_COLLECTION_PROGRESS";
  }
  virtual inline std::unordered_set<const RE::TESForm *>
...
protected:
  ...
  virtual void SetMemberFrom(const nlohmann::json &member,
                             const RE::TESForm *form) override;
...
public:
  ...
  virtual inline std::vector<ObjectType> ObjectTypes(void) const override {
    return m_itemRule->GetObjectTypes();
  }
  inline size_t Count() const { return Observed(); }
  virtual inline std::string GetStatusMessage() const override {
    return "$SHSE_CATEGORY_COLLECTION_PROGRESS";
  }
  virtual inline std::unordered_set<const RE::TESForm *>
  Members() const override {
...
class CollectionGroup {
public:
  CollectionGroup(CollectionManager &manager, const std::string &name,
                  const CollectionPolicy &policy, const bool useMCM,
                  const nlohmann::json &collections);
  inline const std::vector<std::shared_ptr<Collection>> &Collections() const {
    return m_collections;
  }
  std::shared_ptr<Collection>
  CollectionByName(const std::string &collectionName) const;
