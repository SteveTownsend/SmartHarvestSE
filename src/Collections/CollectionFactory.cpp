...
CollectionFactory::ParsePlugin(const nlohmann::json &pluginRule) const {
  std::vector<std::string> plugins;
  plugins.reserve(pluginRule.size());
  std::transform(
      pluginRule.cbegin(), pluginRule.cend(), std::back_inserter(plugins),
...
CollectionFactory::ParseFormList(const nlohmann::json &formListRule) const {
  std::vector<std::pair<std::string, std::string>> pluginFormLists;
  pluginFormLists.reserve(formListRule.size());
  std::transform(formListRule.cbegin(), formListRule.cend(),
                 std::back_inserter(pluginFormLists),
                 [&](const nlohmann::json &next) {
                   return std::make_pair(next["listPlugin"], next["formID"]);
                 });
  return std::make_unique<FormListCondition>(pluginFormLists);
}

std::unique_ptr<FormsCondition>
CollectionFactory::ParseForms(const nlohmann::json &formsRule) const {
  return std::make_unique<FormsCondition>(JSONUtils::ParseFormsType(formsRule));
}

std::unique_ptr<KeywordCondition>
CollectionFactory::ParseKeyword(const nlohmann::json &keywordRule) const {
  std::vector<std::string> keywords;
  keywords.reserve(keywordRule.size());
  std::transform(
      keywordRule.cbegin(), keywordRule.cend(), std::back_inserter(keywords),
      [&](const nlohmann::json &next) { return next.get<std::string>(); });
  return std::make_unique<KeywordCondition>(keywords);
}

std::unique_ptr<SignatureCondition>
CollectionFactory::ParseSignature(const nlohmann::json &signatureRule) const {
  std::vector<std::string> signatures;
  signatures.reserve(signatureRule.size());
  std::transform(
      signatureRule.cbegin(), signatureRule.cend(),
...
CollectionFactory::ParseScope(const nlohmann::json &scopeRule) const {
  std::vector<std::string> scopes;
  scopes.reserve(scopeRule.size());
  std::transform(
      scopeRule.cbegin(), scopeRule.cend(), std::back_inserter(scopes),
      [&](const nlohmann::json &next) { return next.get<std::string>(); });
  return std::make_unique<ScopeCondition>(scopes);
}

std::unique_ptr<NameMatchCondition>
CollectionFactory::ParseNameMatch(const nlohmann::json &nameMatchRule) const {
  std::vector<std::string> names;
  names.reserve(nameMatchRule["names"].size());
  std::transform(
      std::cbegin(nameMatchRule["names"]), std::cend(nameMatchRule["names"]),
      std::back_inserter(names),
      [&](const nlohmann::json &next) { return next.get<std::string>(); });
  return std::make_unique<NameMatchCondition>(
      nameMatchRule["isNPC"].get<bool>(),
      nameMatchRule["matchIf"].get<std::string>(), names);
}

CollectionPolicy
CollectionFactory::ParsePolicy(const nlohmann::json &policy) const {
  return CollectionPolicy(
      ParseCollectibleHandling(policy["action"].get<std::string>()),
      policy["notify"].get<bool>(), policy["repeat"].get<bool>());
}

std::unique_ptr<FilterTree>
CollectionFactory::ParseFilter(const nlohmann::json &filter,
...
CollectionFactory::ParseCategory(const nlohmann::json &categoryRule) const {
  std::unique_ptr<CategoryRule> rule(std::make_unique<CategoryRule>());
  std::vector<std::string> categories;
  categories.reserve(categoryRule.size());
  std::transform(
      categoryRule.cbegin(), categoryRule.cend(),
...
std::shared_ptr<Collection> CollectionFactory::ParseCollection(
    const CollectionGroup *owningGroup, const nlohmann::json &collection,
  ...
  DBG_VMESSAGE("Collection {}, overrides Policy = {}", name.c_str(),
               overridesPolicy ? "true" : "false");
  if (collection.find("rootFilter") != collection.cend()) {
    return std::make_shared<ConditionCollection>(
        owningGroup, name, collection["description"].get<std::string>(),
        policy != collection.cend() ? ParsePolicy(collection["policy"])
                                    : defaultPolicy,
        overridesPolicy, ParseFilter(collection["rootFilter"], 0));
  } else if (collection.find("category") != collection.cend()) {
    return std::make_shared<CategoryCollection>(
        owningGroup, name, collection["description"].get<std::string>(),
        policy != collection.cend() ? ParsePolicy(collection["policy"])
                                    : defaultPolicy,
        overridesPolicy, ParseCategory(collection["category"]));
  }
  return std::shared_ptr<Collection>();
}

std::shared_ptr<CollectionGroup>
CollectionFactory::ParseGroup(CollectionManager &manager,
                              const nlohmann::json &group,
                              const std::string &groupName) const {
  return std::make_shared<CollectionGroup>(
      manager, groupName, ParsePolicy(group["groupPolicy"]),
      group["useMCM"].get<bool>(), group["collections"]);
}
