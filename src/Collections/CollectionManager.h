...
typedef std::tuple<RE::TESBoundObject *, const INIFile::SecondaryType,
                   const ObjectType>
    OwnedItem;

/*
case-insensitive indexing for std::map and std::multimap is required because
strings passed from Papyrus VM are non-deterministic in case
See
...
struct ci_equals {
  bool operator()(const std::string &lhs, const std::string &rhs) const {
    return std::ranges::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin(),
                              rhs.cend(), ichar_equals);
  }
};
// hasher
struct ci_hash {
...
public:
  ...
  TreatAsCollectible(const ConditionMatcher &matcher);
  std::pair<bool, CollectibleHandling>
  TreatAsCollectible(const ConditionMatcher &matcher, const bool recordDups);
  void Refresh() const;
  void CollectFromContainer(const RE::TESObjectREFR *refr);
  bool ItemIsCollectionCandidate(RE::TESBoundObject *item) const;
  void CheckEnqueueAddedItem(RE::TESBoundObject *form,
                             const INIFile::SecondaryType scope,
                             const ObjectType objectType);
  void ProcessAddedItems();
  inline bool IsMCMEnabled() const {
    return SettingsCache::Instance().CollectionsEnabled();
  }
  ...
  void
  RecordCollectibleObjectTypes(const std::shared_ptr<Collection> &collection);
  ...
  std::string NameByIndexInGroup(const std::string &groupName,
                                 const int collectionIndex) const;
  std::string DescriptionByIndexInGroup(const std::string &groupName,
                                        const int collectionIndex) const;
  static std::string MakeLabel(const std::string &groupName,
                               const std::string &collectionName);
  const Collection *CollectionByLabel(const std::string &groupName,
                                      const std::string &collectionName) const;
  Collection *MutableCollectionByLabel(const std::string &groupName,
                                       const std::string &collectionName);

  bool PolicyRepeat(const std::string &groupName,
                    const std::string &collectionName) const;
  bool PolicyNotify(const std::string &groupName,
                    const std::string &collectionName) const;
  CollectibleHandling PolicyAction(const std::string &groupName,
                                   const std::string &collectionName) const;
  void PolicySetRepeat(const std::string &groupName,
                       const std::string &collectionName,
                       const bool allowRepeats);
  void PolicySetNotify(const std::string &groupName,
                       const std::string &collectionName, const bool notify);
  void PolicySetAction(const std::string &groupName,
                       const std::string &collectionName,
  ...
  void GroupPolicySetRepeat(const std::string &groupName,
                            const bool allowRepeats);
  void GroupPolicySetNotify(const std::string &groupName, const bool notify);
  void GroupPolicySetAction(const std::string &groupName,
                            const CollectibleHandling action);

  size_t TotalItems(const std::string &groupName,
                    const std::string &collectionName) const;
  size_t ItemsObtained(const std::string &groupName,
                       const std::string &collectionName) const;
  std::string StatusMessage(const std::string &groupName,
                            const std::string &collectionName) const;
...
private:
  bool LoadData(void);
  void LoadCollectionFiles(const std::wstring &pattern,
                           nlohmann::json_schema::json_validator &validator);
  bool LoadCollectionGroup(const std::filesystem::path &defFile,
                           const std::string &groupName,
                           nlohmann::json_schema::json_validator &validator);
  void
  BuildDecisionTrees(const std::shared_ptr<CollectionGroup> &collectionGroup);
  void
  RecordCollectibleForm(const std::shared_ptr<Collection> &collection,
                        const RE::TESForm *form,
                        std::unordered_set<const RE::TESForm *> &uniqueMembers);
  void ResolveMembership(void);
  void AddToRelevantCollections(const ConditionMatcher &matcher,
                                const float gameTime);
  void ReconcileInventory(std::vector<OwnedItem> &additions);
  void EnqueueAddedItem(RE::TESBoundObject *,
                        const INIFile::SecondaryType scope,
                        const ObjectType objectType);
  ...
  // case-insensitive indexing to account for Papyrus sourced strings being
  // mon-deterministic in case
  std::unordered_map<std::string, std::shared_ptr<Collection>, ci_hash,
                     ci_equals>
      m_allCollectionsByLabel;
  mutable std::multimap<std::string, std::string, ci_less>
      m_activeCollectionsByGroupName;
  std::unordered_map<std::string, std::string, ci_hash, ci_equals>
      m_mcmVisibleFileByGroupName;
  std::unordered_map<std::string, std::shared_ptr<CollectionGroup>, ci_hash,
                     ci_equals>
      m_allGroupsByName;
  // Link each Form to the Collections in which it belongs
  std::unordered_multimap<RE::FormID, std::shared_ptr<Collection>>
      m_collectionsByFormID;
  std::unordered_multimap<ObjectType, std::shared_ptr<Collection>>
      m_collectionsByObjectType;

  std::vector<OwnedItem> m_addedItemQueue;
  std::unordered_set<const RE::TESForm *> m_lastInventoryCollectibles;
  std::chrono::time_point<std::chrono::high_resolution_clock>
      m_lastInventoryCheck;
