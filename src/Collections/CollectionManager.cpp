...
CollectionManager &CollectionManager::Collectibles() {
  if (!m_collectibles) {
    m_collectibles = std::make_unique<CollectionManager>(
        L"SHSE\\.Collections\\.(.*)\\.json$");
  }
...
CollectionManager &CollectionManager::ExcessInventory() {
  if (!m_excessInventory) {
    m_excessInventory = std::make_unique<CollectionManager>(
        L"SHSE\\.ExcessInventory\\.(.*)\\.json$");
  }
...
CollectionManager &CollectionManager::SpecialCases() {
  if (!m_specialCases) {
    m_specialCases =
        std::make_unique<CollectionManager>(L"SHSE\\.Builtin\\.(.*)\\.json$");
  }
...
bool CollectionManager::ItemIsCollectionCandidate(
    RE::TESBoundObject *item) const {
  RecursiveLockGuard guard(m_collectionLock);
  return m_collectionsByFormID.contains(item->GetFormID()) ||
         m_collectionsByObjectType.contains(GetEffectiveObjectType(item));
}

void CollectionManager::CollectFromContainer(const RE::TESObjectREFR *refr) {
  // Container trait was checked in the script that invokes this via
  // SHSE_PluginProxy but we must make sure here
  ...
  lister.FilterLootableItems([=](RE::TESBoundObject *item) -> bool {
    return ItemIsCollectionCandidate(item);
  });
  ...
  for (const auto &candidate : lister.GetLootableItems()) {
    ...
    EnqueueAddedItem(candidate.BoundObject(),
                     ...
                     GetEffectiveObjectType(candidate.BoundObject()));
    DBG_VMESSAGE("Enqueue Collectible {}/0x{:08x} from Container",
                 candidate.BoundObject()->GetName(),
                 candidate.BoundObject()->GetFormID());
  }
  static RE::BSFixedString extraItemsText(papyrus::GetTranslation(
...
void CollectionManager::CheckEnqueueAddedItem(
    RE::TESBoundObject *form, const INIFile::SecondaryType scope,
...
void CollectionManager::EnqueueAddedItem(RE::TESBoundObject *form,
...
void CollectionManager::ProcessAddedItems() {
...
#endif
  RecursiveLockGuard guard(m_collectionLock);
  constexpr std::chrono::milliseconds InventoryReconciliationIntervalMillis(
      5000LL);
  ...
  for (const auto ownedItem : queuedItems) {
    ...
    if (ItemIsCollectionCandidate(form)) {
      DBG_VMESSAGE("Check collectability of added item 0x{:08x}",
                   form->GetFormID());
      AddToRelevantCollections(ConditionMatcher(form, std::get<1>(ownedItem),
                                                std::get<2>(ownedItem)),
                               gameTime);
    }
  }
  ...
  if (m_notifications > CollectedSpamLimit) {
    ...
    static RE::BSFixedString extraItemsText(papyrus::GetTranslation(
        nullptr, RE::BSFixedString("$SHSE_ADDED_TO_COLLECTION_EXTRAS")));
    if (!extraItemsText.empty()) {
      std::string notificationText(extraItemsText);
...
void CollectionManager::AddToRelevantCollections(
    const ConditionMatcher &matcher, const float gameTime) {
  // resolve ID to Form
  if (!matcher.Form())
    return;
  RecursiveLockGuard guard(m_collectionLock);
  const auto staticTargets(
      m_collectionsByFormID.equal_range(matcher.Form()->GetFormID()));
  bool atLeastOne(false);
  auto checkCollection =
      [&](decltype(staticTargets.first->second) nextCollection) {
        // skip disabled collections
        if (!nextCollection->IsActive())
          return;
        // Do not record if the policy indicates per-item history not required,
        // or not a member, or if observed and not repeatable
        if (CollectibleHistoryNeeded(nextCollection->Policy().Action()) &&
            (nextCollection->Policy().Repeat() ||
             !nextCollection->HaveObserved(matcher.Form())) &&
            nextCollection->IsMemberOf(matcher)) {
          // record membership - suppress notifications if there are too many
          if (nextCollection->RecordItem(matcher.Form(), gameTime,
                                         m_notifications >=
                                             CollectedSpamLimit)) {
  ...
  for (auto formCollection = staticTargets.first;
       formCollection != staticTargets.second; ++formCollection) {
    checkCollection(formCollection->second);
  }
  const auto dynamicTargets(
      m_collectionsByObjectType.equal_range(matcher.GetObjectType()));
  for (auto formCollection = dynamicTargets.first;
       formCollection != dynamicTargets.second; ++formCollection) {
    checkCollection(formCollection->second);
  }
  if (atLeastOne) {
...
CollectionManager::TreatAsCollectible(const ConditionMatcher &matcher) {
...
CollectionManager::TreatAsCollectible(const ConditionMatcher &matcher,
  ...
  auto checkCollection =
      [&](decltype(staticTargets.first->second) nextCollection) {
        ...
        bool repeat;
        bool observed;
        std::tie(inScope, repeat, observed) =
            nextCollection->InScopeAndCollectibleFor(matcher);
        // skip if item out of scope for this collection
        if (!inScope)
          return;

        // If we already saw this item on this scan, this is a repeat
        // observation. m_observed is not updated until Collection add, which
        // happens later on after item is in inventory.
        observed = observed || m_collectedOnThisScan.contains(formID);
        if (repeat || !observed) {
          // The Collection definitively treats this observation of the item as
          // a member
          actionable = true;
          action = UpdateCollectibleHandling(nextCollection->Policy().Action(),
                                             action);
          DBG_VMESSAGE("Item {}/0x{:08x} in Collection {} with action {}",
                       matcher.Form()->GetName(), matcher.Form()->GetFormID(),
                       nextCollection->Name(),
                       CollectibleHandlingString(action));
        } else if (!repeat && !actionable) {
          // The Collection decision is qualified: it's a member, but not
          // collected because repeats are not allowed
  ...
  // prevent dups - if this is an item Collectibility precheck don't trigger
  // this logic, or we won't actually loot the item
...
// Player inventory can get objects from Loot menus and other sources than our
// harvesting, we need to account for them We don't do this on every pass as
// it's a decent amount of work. Filter on Collectible item trait to reduce
// work.
void CollectionManager::ReconcileInventory(std::vector<OwnedItem> &additions) {
  ...
  for (const auto &candidate : lister.GetLootableItems()) {
    ...
    if (!m_lastInventoryCollectibles.contains(item)) {
      DBG_VMESSAGE("Collectible {}/0x{:08x} new in inventory", item->GetName(),
                   item->GetFormID());
      // Assumes loose item since we apparently did not autoloot it
      additions.emplace_back(
          std::make_tuple(item, INIFile::SecondaryType::itemObjects,
                          GetEffectiveObjectType(item)));
    } else {
      DBG_VMESSAGE("Skip {}/0x{:08x} unchanged in inventory", item->GetName(),
                   item->GetFormID());
    }
  }
...
bool CollectionManager::LoadCollectionGroup(
    const std::filesystem::path &defFile, const std::string &groupName,
  ...
  try {
    ...
    const auto collectionGroup(CollectionFactory::Instance().ParseGroup(
        *this, collectionGroupData, groupName));
    BuildDecisionTrees(collectionGroup);
    if (collectionGroup->UseMCM()) {
      REL_MESSAGE("Record MCM-visible Collection Group {}/{}", groupName,
                  defFile.string());
      m_mcmVisibleFileByGroupName.insert(
          std::make_pair(groupName, defFile.string()));
    }
...
void CollectionManager::LoadCollectionFiles(
    const std::wstring &pattern,
    nlohmann::json_schema::json_validator &validator) {
  const std::wregex collectionsFilePattern(pattern);
  for (const auto &nextFile :
       std::filesystem::directory_iterator(FileUtils::GetPluginPath())) {
    if (!std::filesystem::is_regular_file(nextFile)) {
      DBG_MESSAGE("Skip {}, not a regular file",
                  StringUtils::FromUnicode(nextFile.path().generic_wstring()));
      continue;
    }
    std::wstring fileName(nextFile.path().filename().generic_wstring());
    std::wsmatch matches;
    if (!std::regex_search(fileName, matches, collectionsFilePattern)) {
      DBG_MESSAGE("Skip {}, does not match Collections filename pattern",
                  StringUtils::FromUnicode(fileName));
      continue;
    }
    // capture string at index 1 is the Collection Name, always present after a
    // regex match
    REL_MESSAGE("Load JSON Collection Definitions {} for Group {}",
                StringUtils::FromUnicode(fileName),
                StringUtils::FromUnicode(matches[1].str()));
    if (LoadCollectionGroup(
            nextFile, StringUtils::FromUnicode(matches[1].str()), validator)) {
      REL_MESSAGE("JSON Collection Definitions {}/{} parsed and validated",
                  StringUtils::FromUnicode(fileName),
                  StringUtils::FromUnicode(matches[1].str()));
    }
  }
...
[[nodiscard]] bool CollectionManager::LoadSchema(void) {
  ...
  std::string filePath(StringUtils::FromUnicode(FileUtils::GetPluginPath()) +
                       schemaFileName);
  try {
...
bool CollectionManager::LoadData(void) {
  try {
    ...
    REL_ERROR(
        "Collection Definitions not loadable: is the file target at {} inside "
        "'Program Files' or another elevated-privilege location? Error:\n{}",
  ...
  for (const auto &collectionGroup : m_allGroupsByName) {
    ...
    for (const auto &collection : collectionGroup.second->Collections()) {
      REL_MESSAGE("** Collection {}:\n{}", collection->Name(),
                  collection->PrintDefinition());
      if (collection->HasMembers()) {
        REL_MESSAGE("{}", collection->PrintMembers());
        const std::string label(
            MakeLabel(collectionGroup.second->Name(), collection->Name()));
        m_activeCollectionsByGroupName.insert(
            std::make_pair(collectionGroup.second->Name(), label));
      } else {
...
std::string CollectionManager::GroupNameByIndex(const int fileIndex) const {
  ...
  for (const auto &group : m_mcmVisibleFileByGroupName) {
    if (index == fileIndex) {
      DBG_DMESSAGE("Collection Group name={} at index {}", group.first,
                   fileIndex);
      return group.first;
    }
    ++index;
...
int CollectionManager::NumberOfActiveCollections(
    const std::string &groupName) const {
  RecursiveLockGuard guard(m_collectionLock);
  int count(static_cast<int>(m_activeCollectionsByGroupName.count(groupName)));
  REL_DMESSAGE("Collection Group {} has {} active collections", groupName,
               count);
  return static_cast<int>(count);
}

std::string
CollectionManager::NameByIndexInGroup(const std::string &groupName,
                                      const int collectionIndex) const {
  RecursiveLockGuard guard(m_collectionLock);
  ...
  for (auto group = matches.first; group != matches.second; ++group) {
    if (index == collectionIndex) {
      ...
      REL_DMESSAGE("Collection Group {} index {} -> {}", groupName,
                   collectionIndex, name);
      return name;
    }
    ++index;
  }
  REL_ERROR("Collection Group {} index {} not found", groupName,
            collectionIndex);
  return std::string();
}

std::string
CollectionManager::DescriptionByIndexInGroup(const std::string &groupName,
                                             const int collectionIndex) const {
  ...
  if (matchedGroup != m_allGroupsByName.cend() && !collectionName.empty()) {
    const auto collection(
        matchedGroup->second->CollectionByName(collectionName));
    if (collection) {
...
std::string CollectionManager::MakeLabel(const std::string &groupName,
...
const Collection *
CollectionManager::CollectionByLabel(const std::string &groupName,
...
Collection *
CollectionManager::MutableCollectionByLabel(const std::string &groupName,
                                            const std::string &collectionName) {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
...
bool CollectionManager::PolicyRepeat(const std::string &groupName,
                                     const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
  if (matched != m_allCollectionsByLabel.cend()) {
    ...
    DBG_DMESSAGE("Collection {}/{} labeled as {} policy-repeat={}", groupName,
                 collectionName, label, repeat);
    return repeat;
  }
  REL_ERROR("Collection {}/{} labeled as {} not found", groupName,
            collectionName, label);
  return false;
}

bool CollectionManager::PolicyNotify(const std::string &groupName,
                                     const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
...
CollectibleHandling
CollectionManager::PolicyAction(const std::string &groupName,
                                const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
...
void CollectionManager::PolicySetRepeat(const std::string &groupName,
                                        const std::string &collectionName,
...
void CollectionManager::PolicySetNotify(const std::string &groupName,
                                        const std::string &collectionName,
                                        const bool notify) {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
...
void CollectionManager::PolicySetAction(const std::string &groupName,
                                        const std::string &collectionName,
                                        const CollectibleHandling action) {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  auto matched(m_allCollectionsByLabel.find(label));
  if (matched != m_allCollectionsByLabel.cend()) {
...
CollectibleHandling
CollectionManager::GroupPolicyAction(const std::string &groupName) const {
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allGroupsByName.find(groupName));
  if (matched != m_allGroupsByName.cend()) {
    return matched->second->Policy().Action();
...
void CollectionManager::GroupPolicySetRepeat(const std::string &groupName,
                                             const bool allowRepeats) {
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allGroupsByName.find(groupName));
  if (matched != m_allGroupsByName.cend()) {
...
void CollectionManager::GroupPolicySetNotify(const std::string &groupName,
                                             const bool notify) {
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allGroupsByName.find(groupName));
  if (matched != m_allGroupsByName.cend()) {
...
void CollectionManager::GroupPolicySetAction(const std::string &groupName,
                                             const CollectibleHandling action) {
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allGroupsByName.find(groupName));
  if (matched != m_allGroupsByName.cend()) {
...
size_t CollectionManager::TotalItems(const std::string &groupName,
                                     const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
...
size_t
CollectionManager::ItemsObtained(const std::string &groupName,
                                 const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
...
std::string
CollectionManager::StatusMessage(const std::string &groupName,
                                 const std::string &collectionName) const {
  const std::string label(MakeLabel(groupName, collectionName));
  RecursiveLockGuard guard(m_collectionLock);
  const auto matched(m_allCollectionsByLabel.find(label));
...
void CollectionManager::BuildDecisionTrees(
    const std::shared_ptr<CollectionGroup> &collectionGroup) {
  for (const auto collection : collectionGroup->Collections()) {
    const std::string label(
        MakeLabel(collectionGroup->Name(), collection->Name()));
    if (m_allCollectionsByLabel.insert(std::make_pair(label, collection))
            .second) {
...
void CollectionManager::RecordCollectibleForm(
  ...
  DBG_VMESSAGE("Record {}/0x{:08x} as collectible", form->GetName(),
               form->GetFormID());
  m_collectionsByFormID.insert(std::make_pair(form->GetFormID(), collection));
  uniqueMembers.insert(form);
}

void CollectionManager::RecordCollectibleObjectTypes(
  ...
  for (const auto objectType : collection->ObjectTypes()) {
    ...
    DBG_VMESSAGE("Recorded {} as collectible ObjectType",
...
void CollectionManager::ResolveMembership(void) {
  ...
  for (const auto &collection : m_allCollectionsByLabel) {
    ...
    REL_MESSAGE("Collection {} has {} static members", collection.first,
                staticMembers.size());
    for (const auto member : staticMembers) {
      REL_MESSAGE("Static member {}/0x{:08x}", member->GetName(),
                  member->GetFormID());
  ...
  auto processFormType = [&](const RE::FormType formType) {
    for (const auto form :
         RE::TESDataHandler::GetSingleton()->GetFormArray(formType)) {
      ...
      for (const auto &collection : m_allCollectionsByLabel) {
        // Record collection membership for any that match this object - ignore
        // whitelist This resolution step does not incorporate Collections with
        // CategoryRule as the Form's ObjectType cannot be reliably determined
        // during startup
        ConditionMatcher matcher(form);
        if (collection.second->IsStaticMatch(matcher)) {
          REL_MESSAGE("{}/0x{:08x} matched Collection {}", form->GetName(),
                      form->GetFormID(), collection.first);
          // Any condition on this collection that has a scope has aggregated
          // the valid scopes in the matcher
  ...
  for (const auto &signature : SignatureCondition::ValidSignatures()) {
    REL_MESSAGE("Check Signature {}, form type {}", signature.first,
                RE::FormTypeToString(signature.second));
    processFormType(signature.second);
  }
  // named objects processed in DataCase::CategorizeLootables or otherwise
  // Lootable, but not valid in SignatureCondition
  std::vector<RE::FormType> extraFormTypes = {
      RE::FormType::Activator,  RE::FormType::Ammo,
      RE::FormType::Container, // blacklisted containers e.g. Missives
      RE::FormType::Flora,      RE::FormType::Light,  RE::FormType::NPC,
      RE::FormType::Projectile, RE::FormType::Scroll, RE::FormType::Tree,
  };
  for (const auto &extraFormType : extraFormTypes) {
    REL_MESSAGE("Check extra form type {}",
                RE::FormTypeToString(extraFormType));
...
// if this is a game reload, we must resolve membership after cosave data is
// applied
void CollectionManager::OnGameReload(void) {
  ...
  // resolve and print effective membership - no members for new game, cosave
  // state may include members so print reconciled version
  ResolveMembership();
  PrintMembership();
}
