...
ItemCollected::ItemCollected(const RE::TESForm *item,
                             const Collection *collection, const float gameTime)
    : m_item(item), m_collection(collection), m_gameTime(gameTime) {}

std::string ItemCollected::AsString() const {
  std::ostringstream stream;
  stream << "I collected " << m_item->GetName() << " for "
         << m_collection->Name() << '.';
  return stream.str();
}

Collection::Collection(const CollectionGroup *owningGroup,
                       const std::string &name, const std::string &description,
                       const CollectionPolicy &policy,
                       const bool overridesGroup,
                       std::unique_ptr<ItemRule> itemRule)
    : m_name(name), m_description(description), m_effectivePolicy(policy),
      m_overridesGroup(overridesGroup), m_itemRule(std::move(itemRule)),
      m_owningGroup(owningGroup), m_disabled(false) {}

Collection::~Collection() {}

// first element - does Collection determine disposition?
// second element - true for one-time Collectible, already processed: defer
// decision to other rules
std::tuple<bool, bool, bool>
Collection::InScopeAndCollectibleFor(const ConditionMatcher &matcher) const {
  bool inScope(false);
  bool repeat(false);
  bool observed(false);
  if (!matcher.Form())
  ...
  // check Scope - if Collection is scoped, scope for this autoloot check must
  // be valid
  if (!m_scopes.empty() && std::find(m_scopes.cbegin(), m_scopes.cend(),
                                     matcher.Scope()) == m_scopes.cend()) {
    DBG_VMESSAGE("{}/0x{:08x} has invalid scope {}", matcher.Form()->GetName(),
                 matcher.Form()->GetFormID(), int(matcher.Scope()));
    return {inScope, repeat, observed};
  }
  inScope = true;
  if (IsMemberOf(matcher)) {
    // Collection member always handled if repeats allowed, or as-yet unobserved
    // If repeats are disallowed, the item is no longer Collectible after first
    // observation Defer to other rules if repeats are not allowed and this
    // Collection is not dispositive for the item
    return {inScope, m_effectivePolicy.Repeat(),
            m_observed.contains(matcher.Form())};
  }
...
bool Collection::IsActive() const {
  ...
  // Administrative groups are not MCM-managed and always-on. User Groups are
  // active if Collections are MCM-enabled.
  return HasMembers() &&
         (!m_owningGroup->UseMCM() || m_owningGroup->Manager().IsMCMEnabled());
}
...
bool Collection::RecordItem(const RE::TESForm *form, const float gameTime,
                            const bool suppressSpam) {
  DBG_VMESSAGE("Collect {}/0x{:08x} in {}", form->GetName(), form->GetFormID(),
               m_name.c_str());
  if (m_observed.insert({form, gameTime}).second) {
    ...
    if (m_effectivePolicy.Notify()) {
      // don't flood the screen for ages on one pass (especially first-time
      // inventory reconciliation)
      if (!suppressSpam) {
        // notify about these, just once
        static RE::BSFixedString newMemberText(papyrus::GetTranslation(
            nullptr, RE::BSFixedString("$SHSE_ADDED_TO_COLLECTION")));
...
std::string Collection::Name(void) const { return m_name; }

std::string Collection::Description(void) const { return m_description; }
...
nlohmann::json Collection::MakeJSON() const { return nlohmann::json(*this); }
...
void Collection::UpdateFrom(const nlohmann::json &collectionState,
                            const CollectionPolicy &defaultPolicy) {
  if (collectionState["members"].empty()) {
    // ignore cosave, user can rehydrate from inventory etc
    REL_WARNING("Collection State {} member list empty in cosave, skipped",
                m_name);
    return;
  }
  const std::string name(collectionState["name"].get<std::string>());
  const std::string description(
      collectionState["description"].get<std::string>());
  ...
  REL_VMESSAGE("Collection State {} overrides Policy = {}", name,
               overridesPolicy ? "true" : "false");
  SetOverridesGroup(overridesPolicy);
  m_effectivePolicy = overridesPolicy
                          ? CollectionFactory::Instance().ParsePolicy(*policy)
                          : defaultPolicy;
  m_scopes.clear();
...
void ConditionCollection::InitFromStaticMembers() {
  // if this collection has concrete static members, add them now to seed the
  // list
  m_members.clear();
  const auto statics(m_itemRule->StaticMembers());
  std::copy_if(statics.cbegin(), statics.cend(),
               std::inserter(m_members, m_members.end()),
               FormUtils::IsConcrete);
}
...
void ConditionCollection::SetMemberFrom(const nlohmann::json &member,
...
std::ostream &ConditionCollection::PrintMemberDetails(std::ostream &os) const {
  ...
  for (const auto member : m_members) {
    ...
    os << ", Collected? " << (m_observed.contains(member) ? 'Y' : 'N') << ", ("
       << member->GetName() << ")\n";
  }
  return os;
}
...
void CategoryCollection::SetMemberFrom(const nlohmann::json &member,
                                       const RE::TESForm *form) {
  const auto observed(member.find("time"));
  if (observed != member.cend()) {
    // optional, observed member only
...
bool CategoryCollection::IsMemberOf(const ConditionMatcher &matcher) const {
  // Base Object ObjectType changes based on game state, so cannot store member
  // Form IDs
...
CollectionGroup::CollectionGroup(CollectionManager &manager,
                                 const std::string &name,
                                 const CollectionPolicy &policy,
                                 const bool useMCM,
                                 const nlohmann::json &collections)
    : m_manager(manager), m_name(name), m_policy(policy), m_useMCM(useMCM) {
  // input is JSON array, by construction
  m_collections.reserve(collections.size());
  std::for_each(collections.cbegin(), collections.cend(),
                [&](const nlohmann::json &collection) {
                  try {
                    // Group Policy is the default for Group Member Collection
                    auto newCollection(
                        CollectionFactory::Instance().ParseCollection(
                            this, collection, m_policy));
                    newCollection->Reset();
...
CollectionGroup::CollectionByName(const std::string &collectionName) const {
  const auto matched(
      std::find_if(m_collections.cbegin(), m_collections.cend(),
                   [&](const std::shared_ptr<Collection> &collection) -> bool {
                     return collection->Name() == collectionName;
                   }));
  return matched != m_collections.cend() ? *matched
                                         : std::shared_ptr<Collection>();
}
...
void CollectionGroup::UpdateFrom(const nlohmann::json &group) {
  CollectionPolicy defaultPolicy(
      CollectionFactory::Instance().ParsePolicy(group["groupPolicy"]));
