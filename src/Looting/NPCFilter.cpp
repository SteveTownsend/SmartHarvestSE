...
NPCFilter::NPCFilter()
    : m_active(false), m_logResults(false), m_defaultLoot(false),
      m_excludePlayerRace(false) {}

OrderedFilter::OrderedFilter(const nlohmann::json &j) : m_priority(0) {
  // parse includes/excludes, if either are present we proceed to build a dead
  // body RACE/KYWD filter
  ...
  if (j.contains("exclude")) {
    ...
    if (excluded.contains("faction")) {
      m_excludeFactions =
          JSONUtils::ToForms<RE::TESFaction>(excluded["faction"]);
    }
  ...
  if (j.contains("include")) {
    ...
    if (included.contains("faction")) {
      m_includeFactions =
          JSONUtils::ToForms<RE::TESFaction>(included["faction"]);
    }
  ...
  for (const auto &includeRace : m_includeRaces) {
    if (m_excludeRaces.contains(includeRace)) {
      REL_WARNING("NPC Race {}/0x{:08x} is in include and exclude list",
                  includeRace->GetName(), includeRace->GetFormID());
    }
  }
  for (const auto &includeFaction : m_includeFactions) {
    if (m_excludeFactions.contains(includeFaction)) {
      REL_WARNING("NPC Faction {}/0x{:08x} is in include and exclude list",
                  includeFaction->GetName(), includeFaction->GetFormID());
    }
  }
  for (const auto &includeKeyword : m_includeKeywords) {
    if (m_excludeKeywords.contains(includeKeyword)) {
      REL_WARNING("NPC Keyword {}/0x{:08x} is in include and exclude list",
                  FormUtils::SafeGetFormEditorID(includeKeyword),
                  includeKeyword->GetFormID());
    }
  }
...
bool OrderedFilter::DeterminesLootability(const RE::TESNPC *npc,
    ...
    for (std::uint32_t index = 0; index < race->GetNumKeywords(); ++index) {
      ...
      if (keyword.has_value()) {
        std::string keywordName(
            FormUtils::SafeGetFormEditorID(keyword.value()));
        if (keywordName.empty()) {
          continue;
        }
...
void NPCFilter::Load() {
  try {
    ...
    REL_MESSAGE("JSON Filters Schema {} parsed and validated",
                filePath.c_str());
    // check if dead body race filtering file is present
    const std::string filterFileName("SHSE.Filter.DeadBody.json");
    filePath =
        StringUtils::FromUnicode(FileUtils::GetPluginPath()) + filterFileName;
    std::ifstream filterFile(filePath);
    if (filterFile.fail()) {
      REL_MESSAGE("NPC AutoLoot Filtering not configured in {}",
                  filterFileName);
      return;
    }
    ...
    REL_MESSAGE("Exclude Player Race = {}",
                m_excludePlayerRace ? "true" : "false");
  ...
  for (const RE::TESNPC *npc : dhnd->GetFormArray<RE::TESNPC>()) {
    ...
    if (std::string(npc->GetName()).empty()) {
      REL_VMESSAGE("Skip unnamed NPC {}/0x{:08x}", npc->GetName(),
                   npc->GetFormID());
      continue;
    }
    // specific check for concreteness - follow chain of TPLTs: if we find LVLN
    // then we won't see this NPC_ Record in the wild
    if (IsLeveled(npc)) {
      REL_VMESSAGE("Skip Leveled NPC {}/0x{:08x}", npc->GetName(),
                   npc->GetFormID());
      continue;
    }
    IsLootable(npc);
...
bool NPCFilter::IsLootable(const RE::TESNPC *npc) const {
  ...
  if (npcRace) {
    if (m_excludePlayerRace && npcRace == PlayerState::Instance().GetRace()) {
      if (m_logResults) {
        REL_VMESSAGE(
            "NPC {}/0x{:08x} matches Player Race {}/0x{:08x}, Not Lootable",
    ...
    for (const auto &orderedFilter : m_orderedFilters) {
      if (orderedFilter->DeterminesLootability(npc, isLootable)) {
        if (m_logResults) {
          REL_VMESSAGE(
              "NPC {}/0x{:08x} with Race {}/0x{:08x}, Lootable={} @ filter #{}",
              npc->GetName(), npc->GetFormID(),
              FormUtils::SafeGetFormEditorID(npcRace), npcRace->GetFormID(),
              isLootable ? "true" : "false", orderedFilter->Priority());
        }
        return isLootable;
      }
    }
  ...
  if (m_logResults) {
    REL_VMESSAGE(
        "NPC {}/0x{:08x} with Race {}/0x{:08x}, Lootable={} @ DefaultLoot",
        npc->GetName(), npc->GetFormID(),
        FormUtils::SafeGetFormEditorID(npcRace), npcRace->GetFormID(),
        m_defaultLoot ? "true" : "false");
  }
