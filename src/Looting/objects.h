...
// this overload deliberately has no definition, to trap at link-time misuse of
// the baseForm function to handle a REFR
...
RE::EnchantmentItem *
GetEnchantmentFromExtraLists(RE::BSSimpleList<RE::ExtraDataList *> *extraLists);
...
inline bool IsValueWeightExempt(ObjectType objectType) {
  return objectType == ObjectType::critter || objectType == ObjectType::flora ||
         objectType == ObjectType::lockpick || objectType == ObjectType::key ||
         objectType == ObjectType::oreVein || objectType == ObjectType::septims;
}

inline bool AlwaysValueWeightExempt(ObjectType objectType) {
  return objectType == ObjectType::critter || objectType == ObjectType::flora ||
         objectType == ObjectType::key || objectType == ObjectType::oreVein ||
