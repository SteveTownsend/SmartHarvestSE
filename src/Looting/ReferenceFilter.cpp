...
ReferenceFilter::ReferenceFilter(DistanceToTarget &refs,
                                 IRangeChecker &rangeCheck,
                                 const bool respectDoors, const size_t limit)
    : m_refs(refs), m_rangeCheck(rangeCheck), m_respectDoors(respectDoors),
      m_nearestDoor(0.), m_limit(limit) {}

// lootability inquiry from the player using hotkey on unlooted item in
// worldspace
...
// clang-format off
/*
Lootability can be subjective and/or time-sensitive. Dynamic forms (FormID
0xffnnnnnn) may be deleted from under our feet. Current hypothesis is that this
is safe so long as the base object is not itself dynamic. Example from
play-testing CTD logs where we crashed getting FormID for a pending-loot dead
body.
...
When we come to process this dead body a little later, we get CTD. The FormID
has morphed, meaning the REFR was junked.

Line 3797443 : 0x8e70 (2020-05-23 07:24:39.705)
J:\GitHub\SmartHarvestSE\tasks.cpp(295): [DEBUG] Process enqueued dead body
0x10000000 Line 3797443 : 0x8e70 (2020-05-23 07:24:40.707)
J:\GitHub\SmartHarvestSE\LogStackWalker.cpp(7): [MESSAGE] Callstack dump :
...snip...
...
The failing REFR is a dynamic form referencing a dynamic base form. Earlier
REFRs dynamic -> non-dynamic worked OK.

Dynamic forms may be deleted by script. This means we must be especially wary in
handling them. For now, choose to flat-out ignore any REFR to a Dynamic Base -
manual looting is still possible if REFR is not deleted. Filtering also includes
never recording a Dynamic REFR or Base Form in our filter lists, as Dynamic REFR
FormIDs are recycled. Blacklisted containers are allowed through here as they
may still need to be glowed.
*/
// clang-format on

Lootability ReferenceFilter::AnalyzeREFR(const RE::TESObjectREFR *refr,
  ...
  if (blockReason != Lootability::Lootable) {
    DBG_VMESSAGE("skip REFR 0x{:08x}, blocked base form 0x{:08x}", refr->formID,
                 refr->GetBaseObject() ? refr->GetBaseObject()->GetFormID()
                                       : InvalidForm);
    return blockReason;
  }
  ...
  if (blockReason != Lootability::Lootable) {
    DBG_VMESSAGE("skip blocked REFR for object/container 0x{:08x}",
                 refr->formID);
    return blockReason;
  }

  // check blacklist early - this may be a malformed REFR e.g. GetBaseObject()
  // blank, 0x00000000 FormID as observed in play testing
  ...
  const RE::TESFullName *fullName =
      refr->GetBaseObject()->As<RE::TESFullName>();
  if (!fullName || fullName->GetFullNameLength() == 0) {
    if (!dryRun) {
      data->BlacklistReference(refr);
      DBG_VMESSAGE(
          "blacklist REFR with blank name 0x{:08x}, base form 0x{:08x}",
          refr->formID, refr->GetBaseObject()->GetFormID());
    }
    return Lootability::UnnamedReference;
  }

  if (refr == RE::PlayerCharacter::GetSingleton()) {
    DBG_VMESSAGE("skip PlayerCharacter {}/0x{:08x}",
  ...
  if ((refr->GetBaseObject()->formType == RE::FormType::Flora ||
       refr->GetBaseObject()->formType == RE::FormType::Tree) &&
      ((refr->formFlags & RE::TESObjectREFR::RecordFlags::kHarvested) ==
       RE::TESObjectREFR::RecordFlags::kHarvested)) {
    DBG_VMESSAGE("skip REFR 0x{:08x} to harvested Flora {}/0x{:08x}",
  ...
  if (DataCase::GetInstance()->IsSyntheticFloraHarvested(refr)) {
    DBG_VMESSAGE("skip REFR 0x{:08x} to harvested synthetic Flora {}/0x{:08x}",
                 refr->GetFormID(), refr->GetBaseObject()->GetName(),
                 refr->GetBaseObject()->GetFormID());
  ...
  if (ScanGovernor::Instance().IsLockedForHarvest(refr)) {
    DBG_VMESSAGE("skip REFR, harvest pending {}/0x{:08x}",
                 refr->GetBaseObject()->GetName(),
                 refr->GetBaseObject()->formID);
  ...
  if (!dryRun) {
    if (ScanGovernor::Instance().IsLootedContainer(refr)) {
      DBG_VMESSAGE("skip looted container {}/0x{:08x}",
                   refr->GetBaseObject()->GetName(),
                   refr->GetBaseObject()->formID);
      return Lootability::ContainerLootedAlready;
    }

    // FormID can be retrieved using pointer, but we should not dereference the
    // pointer as the REFR may have been recycled
    RE::FormID dynamicForm(
        ScanGovernor::Instance().LootedDynamicREFRFormID(refr));
    if (dynamicForm != InvalidForm) {
      DBG_VMESSAGE("skip looted dynamic container with Form ID 0x{:08x}",
                   dynamicForm);
...
bool ReferenceFilter::IsFollowerOrDead(const RE::TESObjectREFR *refr) const {
  ...
  if (actor) {
    if (!actor->IsDead(true)) {
      ...
      if (IsSummoned(actor) || refr->IsDynamicForm() ||
      ...
      if (affinity == PlayerAffinity::FollowerFaction) {
        DBG_VMESSAGE("NPC {}/0x{:08x} is Follower", actor->GetName(),
                     actor->GetFormID());
...
void ReferenceFilter::FindActors() {
  m_predicate = std::bind(&ReferenceFilter::IsFollowerOrDead, this,
                          std::placeholders::_1);
...
void ReferenceFilter::FindLootableReferences() {
  m_predicate =
      std::bind(&ReferenceFilter::CanLoot, this, std::placeholders::_1);
  FilterNearbyReferences();
}

void ReferenceFilter::FindAllCandidates() {
  m_predicate =
      std::bind(&ReferenceFilter::IsLootCandidate, this, std::placeholders::_1);
  FilterNearbyReferences();
}
...
void ReferenceFilter::RecordReference(RE::TESObjectREFR *refr) {
  if (refr) {
    ...
    // if 3D not loaded do not measure - Ash Piles are exempt from this since
    // they enter this state after Actor disintegrates In general, GetBaseObject
    // appears unreliable until 3D is loaded, see
    // https://github.com/SteveTownsend/SmartHarvestSE/issues/499 for bitter
    // experience
    if (!refr->Is3DLoaded()) {
      if (HasAshPile(refr)) {
        DBG_VMESSAGE("allow no 3D for Ash Pile on REFR 0x{:08x}",
                     refr->GetFormID());
      } else {
    ...
    if (formType == RE::FormType::Furniture ||
      ...
      DBG_VMESSAGE("invalid formtype {} for 0x{:08x}", formType,
                   refr->GetFormID());
      return;
    }

    // If Looting through Doors is not allowed, check distance and record if
    // this is the nearest so far
    if (formType == RE::FormType::Door) {
      if (m_respectDoors) {
        // Locked DOOR (even if not in range) can bar manual looting from a
        // Display Case - if player gets close we may autoloot the contents
        constexpr double DoorToleranceUnits(3. / DistanceUnitInFeet);
        constexpr double MinLootingRangeUnits(1.0);
        m_rangeCheck.IsValid(
            refr); // calculate distance to door, in range or not
        double doorLimit(m_rangeCheck.Distance());
        if (IsLocked(refr)) {
          doorLimit =
              std::max(MinLootingRangeUnits, doorLimit - DoorToleranceUnits);
          DBG_VMESSAGE("Locked in-range Door 0x{:08x}({}) tightened proximity "
                       "to {:0.2f}",
                       refr->GetFormID(), refr->GetBaseObject()->GetName(),
                       doorLimit);
        } else {
          // by the same token, an unlocked Display Case requires some leeway to
          // autoloot its contents
          doorLimit =
              std::min(m_rangeCheck.Radius(), doorLimit + DoorToleranceUnits);
          DBG_VMESSAGE("Unlocked in-range Door 0x{:08x}({}) relaxed allowed "
                       "proximity to {:0.2f}",
                       refr->GetFormID(), refr->GetBaseObject()->GetName(),
                       doorLimit);
        }
        if (m_nearestDoor == 0. || doorLimit < m_nearestDoor) {
          DBG_VMESSAGE("New nearest Door 0x{:08x}({}) at distance {:0.2f}",
                       refr->GetFormID(), refr->GetBaseObject()->GetName(),
                       doorLimit);
          m_nearestDoor = doorLimit;
        }
      } else {
        DBG_VMESSAGE("skip Door 0x{:08x}", refr->GetFormID(),
                     refr->GetBaseObject()->GetName());
      }
      return;
    }
    ...
    // Record this REFR if it represents a potential player-detecting NPC,
    // unless Stealing operation is in progress already Bypass this logic if
    // stealing is currently disallowed for player.
    RE::Actor *actor(refr->As<RE::Actor>());
    if (actor && !actor->IsDead(true)) {
      PlayerAffinity affinity(GetPlayerAffinity(actor));
      if (affinity == PlayerAffinity::FollowerFaction ||
          affinity == PlayerAffinity::TeamMate) {
        // Do not track live summons or any dynamic Actor REFR as Follower
        if (!IsSummoned(actor) && !refr->IsDynamicForm() &&
            !refr->GetBaseObject()->IsDynamicForm()) {
          DBG_VMESSAGE("NPC {}/0x{:08x} at distance {:0.2f} is Follower",
                       actor->GetName(), refr->GetFormID(),
                       m_rangeCheck.Distance());
          ActorTracker::Instance().AddFollower(actor);
        } else {
          DBG_VMESSAGE(
              "NPC {}/0x{:08x} at distance {:0.2f} ineligible as Follower",
              actor->GetName(), refr->GetFormID(), m_rangeCheck.Distance());
        }
      }
    }
    if (!withinRange) {
      DBG_VMESSAGE("omit out of range REFR 0x{:08x}({})", refr->GetFormID(),
                   refr->GetBaseObject()->GetName());
      return;
    }
    if (!m_predicate(refr)) {
      DBG_VMESSAGE("omit ineligible REFR 0x{:08x}({})", refr->GetFormID(),
                   refr->GetBaseObject()->GetName());
      return;
    }
    DBG_VMESSAGE("add REFR 0x{:08x}({}), distance {:0.2f}, formtype {}",
                 refr->GetFormID(), refr->GetBaseObject()->GetName(),
                 m_rangeCheck.Distance(), formType);
    m_refs.emplace_back(m_rangeCheck.Distance(), refr);
  }
}

// update list with all possibly lootable REFRs in cell
void ReferenceFilter::RecordCellReferences(RE::TESObjectCELL *cell, const bool
#if 0
 indoors
#endif
) {
  ...
  DBG_MESSAGE("Filter {} temp REFRS in CELL 0x{:08x}",
              cell->GetRuntimeData().references.size(), cell->GetFormID());
...
void ReferenceFilter::FilterNearbyReferences() {
  ...
  // goodies. Restrict to cells in the same worldspace. If current cell fills
  // the list then ignore others.
  ...
  if (!indoors) {
    ...
    for (const auto &adjacentCell :
         LocationTracker::Instance().AdjacentCells()) {
      // sanity checks
  ...
  // We must confirm distance is valid because the Radius may update adjusted on
  // the fly as Doors are processed
  DBG_MESSAGE("Sort and truncate {} eligible REFRs vs limit {}", m_refs.size(),
              m_limit);

  // This logic needs to reliably handle load spikes. We do not commit to
  // process more than N references. The rest will get processed on future
  // passes. A spike of 200+ in a second makes the VM dump stacks, so pick N
  // accordingly. Prefer closer references, so partition the list by distance
  // order so we handle no more than N. std::nth_element does precisely what we
  // need. End-of-range iterator remains valid as the container is processed in
  // situ by each algorithms
  auto endOfRange(m_refs.begin() +
                  static_cast<ptrdiff_t>(std::min(m_limit, m_refs.size())));
  std::nth_element(m_refs.begin(), endOfRange, m_refs.end(),
                   [&](const TargetREFR &a, const TargetREFR &b) -> bool {
                     return a.first < b.first;
                   });
  // "Nearest Door" restriction can adjust the range downwards during the scan -
  // re-check here
  if (m_respectDoors && m_nearestDoor > 0.) {
    std::sort(m_refs.begin(), endOfRange,
              [&](const TargetREFR &a, const TargetREFR &b) -> bool {
                return a.first < b.first;
              });
    double effectiveRadius(std::min(m_nearestDoor, m_rangeCheck.Radius()));
    auto tooFarAway(std::find_if(m_refs.begin(), endOfRange,
                                 [&](const auto &target) -> bool {
                                   return target.first > effectiveRadius;
                                 }));

    if (tooFarAway != m_refs.end()) {
      DBG_MESSAGE("Erase {} REFRs outside effective loot range {:.2f}",
                  std::distance(tooFarAway, m_refs.end()), effectiveRadius);
      m_refs.erase(tooFarAway, m_refs.end());
    }
  } else if (endOfRange != m_refs.end()) {
    DBG_MESSAGE("Erase {} REFRs exceeding single-pass limit {}",
                std::distance(endOfRange, m_refs.end()), m_limit);
