...
ContainerLister::ContainerLister(const INIFile::SecondaryType targetType,
                                 const RE::TESObjectREFR *refr)
    : m_refr(refr), m_targetType(targetType),
      m_collectibleAction(CollectibleHandling::Leave) {
  m_enchantedLoot = SettingsCache::Instance().EnchantedObjectHandlingType();
}

void ContainerLister::FilterLootableItems(
    std::function<bool(RE::TESBoundObject *)> predicate) {
  ...
  for (auto &item : inv) {
    ...
    if (predicate(itemObject)) {
      DBG_DMESSAGE("Matched filter predicate for {}/0x{:08x}, count={}",
                   itemObject->GetName(), itemObject->GetFormID(), count);
      m_lootableItems.emplace_back(std::move(entry), count, m_enchantedLoot);
    }
  }
}

size_t ContainerLister::CountLootableItems(
    std::function<bool(RE::TESBoundObject *)> predicate) {
  // refactored following QuickLookRE
  size_t items(0);
  auto inv = const_cast<RE::TESObjectREFR *>(m_refr)->GetInventory();
  for (auto &item : inv) {
    ...
    if (predicate(itemObject)) {
      DBG_DMESSAGE("Matched count predicate for {}/0x{:08x}, count={}",
                   itemObject->GetName(), itemObject->GetFormID(), count);
      ++items;
    }
  }
...
InventoryCache ContainerLister::CacheIfExcessHandlingEnabled(
    const bool force, const InventoryUpdates &updates) const {
  // refactored following QuickLookRE
  // return value contains all eligible Inventory items for full scan, or
  // current Inventory item data for deltas
  ...
  for (auto &item : inv) {
    ...
    // register any new user-created Ingestible on full scan as early as
    // possible
    if (itemObject->IsDynamicForm()) {
      if (itemObject->GetFormType() == RE::FormType::AlchemyItem) {
        // User created potion or poison
        DataCase::GetInstance()->RegisterPlayerCreatedALCH(
            itemObject->As<RE::AlchemyItem>());
      }
    }

    // if this is a delta review, ignore untouched items
    if (!force && !updates.contains(itemObject)) {
      DBG_DMESSAGE("Item {}/0x{:08x} not touched since last review",
                   itemObject->GetName(), itemObject->GetFormID());
      continue;
    }

    // do not bother with other checks if excess-handling is not in effect for
    // the item
    InventoryEntry itemEntry(itemObject, count);
    if (itemEntry.HandlingType() == ExcessInventoryHandling::NoLimits)
      continue;

    // Do not auto-sell or otherwise futz with this if it even MIGHT be a Quest
    // Target
    if (!QuestTargets::Instance().AllowsExcessHandling(itemObject)) {
      DBG_DMESSAGE("Quest Item {}/0x{:08x}, exempt from Excess Inventory",
                   itemObject->GetName(), itemObject->GetFormID());
      continue;
    }
    // Check if item is sticky in inventory, requiring deliberate disposal by
    // Player
    // Ex. AddItemMenuSE kit
    const auto isItemSticky(
        CollectionManager::ExcessInventory().TreatAsCollectible(
            ConditionMatcher(itemObject)));
    if (isItemSticky.first) {
      DBG_DMESSAGE("Sticky Item {}/0x{:08x}, exempt from Excess Inventory",
                   itemObject->GetName(), itemObject->GetFormID());
      continue;
    }
    ...
    if (ManagedList::EquippedOrWorn().Contains(itemObject)) {
      DBG_DMESSAGE(
          "Equipped/Worn Item {}/0x{:08x}, exempt from Excess Inventory",
          itemObject->GetName(), itemObject->GetFormID());
      continue;
    }
    ...
    if (entry->extraLists) {
      ...
      if (isFavourite) {
        DBG_DMESSAGE("Favourite Item {}/0x{:08x}, exempt from Excess Inventory",
                     itemObject->GetName(), itemObject->GetFormID());
        continue;
      }
      if (isEnchanted) {
        DBG_DMESSAGE(
            "Player-enchanted Item {}/0x{:08x}, exempt from Excess Inventory",
            itemObject->GetName(), itemObject->GetFormID());
        continue;
      }
      if (isTempered) {
        DBG_DMESSAGE("Tempered Item {}/0x{:08x}, exempt from Excess Inventory",
                     itemObject->GetName(), itemObject->GetFormID());
        continue;
      }
    }
...
std::string ContainerLister::SellItem(RE::TESBoundObject *target,
                                      const bool excessOnly) {
  InventoryEntry entry(GetSingleInventoryEntry(target));
  if (entry.Count() <= 0) {
    std::string err(
        DataCase::GetInstance()->GetTranslation("$SHSE_ITEM_CANNOT_SELL"));
    StringUtils::Replace(err, "{ITEMNAME}", target->GetName());
    return err;
  }
  return entry.Sell(excessOnly);
}
std::string ContainerLister::TransferItem(RE::TESBoundObject *target,
                                          const bool excessOnly) {
  InventoryEntry entry(GetSingleInventoryEntry(target));
  if (entry.Count() <= 0) {
    std::string err(
        DataCase::GetInstance()->GetTranslation("$SHSE_ITEM_CANNOT_TRANSFER"));
    StringUtils::Replace(err, "{ITEMNAME}", target->GetName());
    return err;
  }
  return entry.Transfer(excessOnly);
}
std::string ContainerLister::DeleteItem(RE::TESBoundObject *target,
                                        const bool excessOnly) {
  InventoryEntry entry(GetSingleInventoryEntry(target));
  if (entry.Count() <= 0) {
    std::string err(
        DataCase::GetInstance()->GetTranslation("$SHSE_ITEM_CANNOT_DELETE"));
    StringUtils::Replace(err, "{ITEMNAME}", target->GetName());
    return err;
  }
...
InventoryEntry
ContainerLister::GetSingleInventoryEntry(RE::TESBoundObject *target) const {
  constexpr bool isDestructive(true);
  return GetSingleInventoryEntry(target, isDestructive);
}

InventoryEntry
ContainerLister::GetSingleInventoryEntry(RE::TESBoundObject *target,
  ...
  for (auto &item : inv) {
    ...
    // Mostly skip check for sticky items here: if player wants to get rid of
    // such items like this, that's OK We do make the check if user is
    // introspecting the item (non-destructive)
    if (!isDestructive) {
      const auto isItemSticky(
          CollectionManager::ExcessInventory().TreatAsCollectible(
              ConditionMatcher(itemObject)));
      if (isItemSticky.first) {
        DBG_DMESSAGE("Item {}/0x{:08x} is anchored in inventory",
                     itemObject->GetName(), itemObject->GetFormID());
    ...
    if (isEnchanted)
      return InventoryEntry(target,
                            ExcessInventoryExemption::IsPlayerEnchanted);
...
size_t ContainerLister::AnalyzeLootableItems(
  ...
  const RE::TESContainer *container =
      const_cast<RE::TESObjectREFR *>(m_refr)->GetContainer();
  if (!container)
    return 0;
  ...
  const RE::ExtraContainerChanges *exChanges =
      m_refr->extraList.GetByType<RE::ExtraContainerChanges>();
  if (exChanges && exChanges->changes && exChanges->changes->entryList) {
    for (auto entryData = exChanges->changes->entryList->begin();
      ...
      for (auto extraList = (*entryData)->extraLists->begin();
           extraList != (*entryData)->extraLists->end(); ++extraList) {
        if (*extraList) {
          ...
          if ((enchantItem = ExtraDataList::GetEnchantment(*extraList)) !=
              nullptr) {
            if (TESFormHelper::ConfirmEnchanted(enchantItem,
                                                enchantedObjectHandling)) {
              DBG_DMESSAGE("Enchanted Item {}/0x{:08x}", item->GetName(),
                           item->GetFormID());
          ...
          if (itemEx.IsValuable()) {
            DBG_DMESSAGE("Valuable Item {}/0x{:08x}", item->GetName(),
                         item->GetFormID());
            m_valuableItems.insert(item);
          }
          // Collectible item precheck - do not record as a possible dup, as we
          // check them all again before looting
          static const bool recordDups(false);
          const auto collectible(itemEx.TreatAsCollectible(recordDups));
          if (collectible.first) {
            DBG_DMESSAGE("Collectible Item {}/0x{:08x}", item->GetName(),
                         item->GetFormID());
            // use the most permissive action
            m_collectibleItems.insert(item);
            m_collectibleAction = UpdateCollectibleHandling(m_collectibleAction,
                                                            collectible.second);
          }
        }
      }
...
void ContainerLister::RemoveUnlootable(
    const std::unordered_set<RE::TESBoundObject *> &filter) {
  decltype(m_lootableItems) filteredList;
  filteredList.reserve(m_lootableItems.size());
  std::copy_if(m_lootableItems.cbegin(), m_lootableItems.cend(),
               std::back_inserter(filteredList),
               [&](const InventoryItem &item) -> bool {
                 return !filter.contains(item.BoundObject());
               });
  DBG_DMESSAGE("Lootable Item count {} filtered from {}", filteredList.size(),
               m_lootableItems.size());
