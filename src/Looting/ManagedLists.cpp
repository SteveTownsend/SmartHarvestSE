...
public:
  MatchBySlot(uint32_t slot) : m_mask(slot) {}

  bool Matches(RE::TESForm *pForm) const {
    if (pForm) {
      auto *pBip = pForm->As<RE::BGSBipedObjectForm>();
      if (pBip) {
        return (pBip->bipedModelData.bipedObjectSlots.any(
            static_cast<RE::BIPED_MODEL::BipedObjectSlot>(m_mask)));
      }
    }
    return false;
  }
...
struct GetMatchingEquipped {
  ...
  GetMatchingEquipped(MatchBySlot &matcher, bool isWorn = true,
                      bool isWornLeft = true)
    ...
    if (pEntryData) {
      ...
      if (pExtendList && m_matcher.Matches(pEntryData->object)) {
        for (const auto extraData : *pExtendList) {
          if (extraData) {
            if ((m_isWorn && extraData->HasType(RE::ExtraDataType::kWorn)) ||
                (m_isWornLeft &&
  ...
  [[nodiscard]] RE::TESBoundObject *Found() const { return m_found; }
};

RE::TESBoundObject *
FindEquipped(const RE::ExtraContainerChanges *pContainerChanges,
             MatchBySlot &matcher, bool isWorn, bool isWornLeft) {
...
template <typename T> T *GetWornForm(RE::Actor *thisActor, uint32_t mask) {
  MatchBySlot matcher(mask);
  auto *pContainerChanges = static_cast<RE::ExtraContainerChanges *>(
      thisActor->extraList.GetByType(RE::ExtraDataType::kContainerChanges));
  if (pContainerChanges) {
    RE::TESBoundObject *eqD =
        FindEquipped(pContainerChanges, matcher, true, true);
...
void ManagedList::Reset() {
  // No baseline for whitelist or transfer-list. Blacklist has a list of known
  // no-loot places.
  ...
  if (this == m_blackList.get()) {
    ...
    while (currentSlot <=
      ...
      if (armor) {
        DBG_DMESSAGE("Worn armor {}/0x{:08x} at slot 0x{:08x}",
                     armor->GetName(), armor->GetFormID(), currentSlot);
    ...
    RE::TESForm *equippedRight(
        RE::PlayerCharacter::GetSingleton()->GetEquippedObject(false));
    RE::TESObjectARMO *shield(
        equippedRight ? equippedRight->As<RE::TESObjectARMO>() : nullptr);
    RE::TESObjectWEAP *weaponRight(
        equippedRight ? equippedRight->As<RE::TESObjectWEAP>() : nullptr);
    RE::TESForm *equippedLeft(
        RE::PlayerCharacter::GetSingleton()->GetEquippedObject(true));
    if (!shield && equippedLeft) {
      shield = equippedLeft->As<RE::TESObjectARMO>();
    }
    // 2H weapon seen in LH and RH here
    RE::TESObjectWEAP *weaponLeft(equippedLeft && equippedLeft != equippedRight
                                      ? equippedLeft->As<RE::TESObjectWEAP>()
                                      : nullptr);
    if (weaponRight) {
      DBG_DMESSAGE("RHS weapon {}/0x{:08x}", weaponRight->GetName(),
                   weaponRight->GetFormID());
      wornEquipped.insert(weaponRight);
    }
    if (weaponLeft) {
      DBG_DMESSAGE("LHS weapon {}/0x{:08x}", weaponLeft->GetName(),
                   weaponLeft->GetFormID());
      wornEquipped.insert(weaponLeft);
    }
    if (shield) {
      DBG_DMESSAGE("Shield equipped {}/0x{:08x}", shield->GetName(),
                   shield->GetFormID());
...
bool ManagedList::HasEntryWithSameName(const RE::TESForm *form) const {
  // skipped if entry is a container/NPC (REFR) or place (CELL/LCTN). Only REFRs
  // right now are to Containers and Dead Actors.
  RE::FormType formType(form->GetFormType());
  if (formType == RE::FormType::Location || formType == RE::FormType::Cell ||
      form->As<RE::TESObjectREFR>())
    return false;
  const std::string name(form->GetName());
  return !name.empty() && std::find_if(m_members.cbegin(), m_members.cend(),
...
void ManagedTargets::AddNamed(RE::TESForm *entry, const std::string &name) {
  ...
  if (entry) {
    ...
    // Record any underlying _linked_ container for checking of multiplexed
    // CONTs. We do not always check CONT for match as many are reused _without_
    // linked-ref indirection.
    RE::TESObjectREFR *refr(entry->As<RE::TESObjectREFR>());
    if (refr) {
      const RE::TESObjectACTI *activator(
          refr->GetBaseObject()->As<RE::TESObjectACTI>());
      if (activator) {
        DBG_VMESSAGE("Check ACTI {}/0x{:08x} -> linked container",
                     activator->GetFullName(), activator->GetFormID());
        RE::TESObjectREFR *linkedRefr(refr->GetLinkedRef(nullptr));
        if (linkedRefr) {
          RE::TESObjectCONT *container =
              linkedRefr->GetBaseObject()->As<RE::TESObjectCONT>();
          if (container) {
    ...
    REL_MESSAGE("Free entry in TransferList at index {}",
                m_orderedList.size() - 1);
  }
}
...
bool ManagedTargets::Contains(const RE::TESForm *entry) const {
  ...
  if (m_members.contains(entry->GetFormID()))
    return true;
  // Check for matching linked REFR - if we autoloot the container in this
  // instance, we may transfer loot back to it and enter a toxic loop
  RE::TESObjectREFR *refr(
      const_cast<RE::TESForm *>(entry)->As<RE::TESObjectREFR>());
  if (refr) {
    const RE::TESObjectACTI *activator(
        refr->GetBaseObject()->As<RE::TESObjectACTI>());
    if (activator) {
