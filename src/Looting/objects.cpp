...
bool IsBossContainer(const RE::TESObjectREFR *refr) {
  ...
  const RE::ExtraLocationRefType *exLocRefType =
      extraList->GetByType<RE::ExtraLocationRefType>();
...
bool IsLocked(const RE::TESObjectREFR *refr) {
  ...
  return exLock && (exLock->lock->flags & RE::REFR_LOCK::Flag::kLocked) ==
                       RE::REFR_LOCK::Flag::kLocked;
}

// This appears safe to call during combat - does not introspect an in-flux
// ExtraDataList, just a bitfield
bool HasAshPile(const RE::TESObjectREFR *refr) {
  return refr && refr->extraList.HasType(RE::ExtraDataType::kAshPileRef);
}

// This is unsafe to call during combat - used only during deferred looting of
// Dead Bodies
RE::TESObjectREFR *GetAshPile(const RE::TESObjectREFR *refr) {
  if (!refr)
    return nullptr;
  RE::ObjectRefHandle ashHandle =
      const_cast<RE::TESObjectREFR *>(refr)->extraList.GetAshPileRef();
  if (!ashHandle)
    return nullptr;
...
void PrintManualLootMessage(const std::string &name) {
  static RE::BSFixedString manualLootText(
      DataCase::GetInstance()->GetTranslation("$SHSE_MANUAL_LOOT_MSG"));
...
void ProcessManualLootREFR(const RE::TESObjectREFR *refr) {
  ...
  DBG_VMESSAGE(
      "notify, then block objType == ObjectType::manualLoot for REFR 0x{:08x}",
      refr->GetFormID());
  // this may be a dynamic Form
  DataCase::GetInstance()->BlockReferenceByID(refr->GetFormID(),
                                              Lootability::ManualLootTarget);
}
...
void ProcessManualLootItem(const RE::TESBoundObject *item) {
  ...
  DBG_VMESSAGE(
      "notify, then block objType == ObjectType::manualLoot for Item 0x{:08x}",
      item->GetFormID());
...
PlayerAffinity GetPlayerAffinity(const RE::Actor *actor) {
  ...
  static const RE::TESFaction *followerFaction =
      RE::TESForm::LookupByID(CurrentFollowerFaction)->As<RE::TESFaction>();
  if (followerFaction && actor->IsInFaction(followerFaction)) {
    DBG_DMESSAGE("Actor {}/0x{:08x} is follower", actor->GetName(),
                 actor->GetFormID());
    return PlayerAffinity::FollowerFaction;
  }
  if (actor->IsPlayerTeammate()) {
    DBG_DMESSAGE("Actor {}/0x{:08x} is teammate", actor->GetName(),
                 actor->GetFormID());
...
bool StartsDead(const RE::TESObjectREFR *refr) {
  bool result(refr && refr->loadedData &&
              ((refr->loadedData->flags &
                RE::TESObjectREFR::RecordFlags::kStartsDead) == 0));
...
bool IsDisintegrating(const RE::Actor *actor) {
  if (actor) {
    return actor->GetActorRuntimeData().criticalStage ==
           RE::ACTOR_CRITICAL_STAGE::kDisintegrateStart;
  }
  return false;
}
...
ObjectType GetBaseObjectType(const RE::TESForm *baseForm) {
  ...
  if (baseForm->formType == RE::FormType::Book) {
    // done here instead of during init because state of object can change
    // during gameplay
    ...
    return (weapon->formEnchanting) ? ObjectType::enchantedWeapon
                                    : ObjectType::weapon;
    ...
    return (armor->formEnchanting) ? ObjectType::enchantedArmor
                                   : ObjectType::armor;
  }
  return ObjectType::unknown;
}
...
const std::unordered_map<RE::FormType, std::string>
    nameByFormType({{RE::FormType::ActorCharacter, "ActorCharacter"},
...
const std::unordered_map<ObjectType, std::string>
    nameByObjectType({{ObjectType::unknown, "unknown"},
...
ObjectType GetObjectTypeByTypeName(const std::string &name) {
  ...
  std::string lcName;
  std::transform(
      name.cbegin(), name.cend(), std::back_inserter(lcName),
      [](const char &c) { return static_cast<char>(std::tolower(c)); });
  for (const auto &nextPair : nameByObjectType) {
    if (nextPair.second == lcName) {
      DBG_VMESSAGE("Mapped name {} to ObjectType {}", name.c_str(),
                   GetObjectTypeName(nextPair.first));
...
const std::unordered_map<std::string, ResourceType>
    resourceTypeByName({{"Ore", ResourceType::ore},
...
RE::EnchantmentItem *GetEnchantmentFromExtraLists(
    RE::BSSimpleList<RE::ExtraDataList *> *extraLists) {
  if (!extraLists)
    return nullptr;
  RE::EnchantmentItem *enchantment = nullptr;
  for (auto extraList = extraLists->begin(); extraList != extraLists->end();
       ++extraList) {
...
bool FormTypeIsLootableObject(const RE::FormType formType) {
  static const std::vector<RE::FormType> lootableObjectFormTypes = {
      RE::FormType::AlchemyItem, RE::FormType::Ammo,
      RE::FormType::Armor,       RE::FormType::Book,
      RE::FormType::Container,   RE::FormType::Flora,
      RE::FormType::Ingredient,  RE::FormType::KeyMaster,
      RE::FormType::Misc,        RE::FormType::Note,
      RE::FormType::Projectile,  RE::FormType::Scroll,
      RE::FormType::SoulGem,     RE::FormType::Tree,
      RE::FormType::Weapon};

  return std::find(std::cbegin(lootableObjectFormTypes),
                   std::cend(lootableObjectFormTypes),
                   formType) != std::cend(lootableObjectFormTypes);
}
