...
public:
  ...
  bool UnlockHarvest(const RE::FormID refrID, const RE::FormID baseID,
                     const std::string &baseName, const bool isSilent);
  ...
  void MarkContainerLootedRepeatGlow(const RE::TESObjectREFR *refr,
  ...
  bool IsReferenceLockedContainer(
...
private:
  ...
  Lootability ValidateTarget(RE::TESObjectREFR *&refr,
                             std::vector<RE::TESObjectREFR *> &possibleDupes,
                             const bool dryRun, const bool glowOnly);
  static Lootability CanLootActor(const RE::TESObjectREFR *refr,
  ...
  // record time limit for each harvest operation, indexed on REFR and Base
  // FormID
  ...
  // BlackList for Locked Containers. Never auto-loot unless config permits.
  // Reset on game reload.
  mutable std::unordered_map<const RE::TESObjectREFR *, size_t>
      m_lockedContainers;

  std::vector<const RE::BGSKeyword *> m_spergKeywords;
  std::unique_ptr<ContainerLister> m_spergInventory;
  // handle concurrent ore vein mining by reconciling versus initial inventory
  // snapshot after the last completes
  ...
  static constexpr int ObjectGlowDurationCalibrationSeconds =
      int(PluginFacade::CalibrationThreadDelaySeconds) - 2;
};
