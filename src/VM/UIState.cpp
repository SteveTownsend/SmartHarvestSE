...
UIState::UIState()
    : m_nonce(0), m_vmResponded(false), m_uiDelayed(false), m_waiting(false),
      m_mcmOpen(false) {}
...
// Check VM is not handling UI - blocks if so, to avoid problems in-game and at
// game shutdown Returns True iff there was a UI-induced delay
void UIState::WaitUntilVMGoodToGo() {
#ifdef _PROFILING
  WindowsUtils::ScopedTimer elapsed("VM Status check");
#endif
  // perform UI state checking here, only fire handshake with scripts if UI is
  // open - do not check activate availability here
  ...
  REL_MESSAGE(
      "Script reported UI ready for request {} after {} microseconds", nonce,
...
void UIState::ReportVMGoodToGo(const bool delayed, const int nonce) {
  ...
  if (nonce != m_nonce) {
    ...
    REL_WARNING("VM Good to Go for request {}, expected request {}", nonce,
                m_nonce);
  } else {
...
void UIState::Reset() {
  ...
  // Scan thread was waiting for response when game load kicked off. Release it
  // with a failure code once .
  if (m_waiting) {
