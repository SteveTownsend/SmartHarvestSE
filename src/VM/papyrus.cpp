...
// Plumb Papyrus trace into Debug and Logging DLLs so don't have to futz with
...
RE::BSFixedString GetTextObjectType(RE::StaticFunctionTag *,
                                    RE::TESForm *thisForm) {
  if (!thisForm)
    return nullptr;
  ...
  ObjectType objType =
      baseObj ? shse::GetEffectiveObjectType(baseObj) : ObjectType::unknown;
...
RE::BSFixedString GetObjectTypeNameByType(RE::StaticFunctionTag *,
...
int32_t GetObjectTypeByName(RE::StaticFunctionTag *,
                            RE::BSFixedString objectTypeName) {
  return static_cast<int32_t>(
      shse::GetObjectTypeByTypeName(objectTypeName.c_str()));
}

int32_t GetResourceTypeByName(RE::StaticFunctionTag *,
                              RE::BSFixedString resourceTypeName) {
  return static_cast<int32_t>(
      shse::ResourceTypeByName(resourceTypeName.c_str()));
}

float GetSetting(RE::StaticFunctionTag *, int32_t section_first,
                 int32_t section_second, RE::BSFixedString key) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  DBG_VMESSAGE("Config setting {}/{}/{} = {}",
               INIFile::PrimaryTypeString(first),
               INIFile::SecondaryTypeString(second), str.c_str(), result);
  return result;
}

float GetSettingObjectArrayEntry(RE::StaticFunctionTag *, int32_t section_first,
                                 int32_t section_second, int32_t index) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  if (second == INIFile::SecondaryType::valueWeight) {
    float tmp_value =
        static_cast<float>(ini->GetSetting(first, second, key.c_str()));
    if (tmp_value < 0.0f) {
      value = 0.0f;
    ...
    shse::LootingType tmp_value = shse::LootingTypeFromIniSetting(
        ini->GetSetting(first, second, key.c_str()));
    // weightless objects and OreVeins are always looted unless explicitly
    // disabled : oreVeins use a third option for BYOH materials, though
    if (ObjectType(index) == ObjectType::oreVein) {
      value = static_cast<float>(
          std::min(tmp_value, shse::LootingType::LootOreVeinAlways));
    } else if (shse::IsValueWeightExempt(static_cast<ObjectType>(index)) &&
               tmp_value > shse::LootingType::LootAlwaysSilent) {
      value = static_cast<float>(
          tmp_value == shse::LootingType::LootIfValuableEnoughNotify
              ? shse::LootingType::LootAlwaysNotify
              : shse::LootingType::LootAlwaysSilent);
    } else {
...
int GetSettingGlowArrayEntry(RE::StaticFunctionTag *, int32_t section_first,
                             int32_t section_second, int32_t index) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  DBG_VMESSAGE("Config setting (glow array) {}/{}/{} = {}",
               INIFile::PrimaryTypeString(first),
               INIFile::SecondaryTypeString(second), key.c_str(), value);
  return value;
}

void PutSetting(RE::StaticFunctionTag *, int32_t section_first,
                int32_t section_second, RE::BSFixedString key, float value) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  DBG_VMESSAGE("Config setting (put) {}/{}/{} = {}",
               INIFile::PrimaryTypeString(first),
               INIFile::SecondaryTypeString(second), str, value);
  ini->PutSetting(first, second, str.c_str(), static_cast<double>(value));
}

void PutSettingObjectArrayEntry(RE::StaticFunctionTag *, int32_t section_first,
                                int32_t section_second, int index,
                                float value) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  std::string key(shse::GetObjectTypeName(ObjectType(index)));
  StringUtils::ToLower(key);
  DBG_VMESSAGE("Put config setting (array) {}/{}/{} = {}",
               INIFile::PrimaryTypeString(first),
               INIFile::SecondaryTypeString(second), key.c_str(), value);
  ini->PutSetting(first, second, key.c_str(), static_cast<double>(value));
}

void PutSettingGlowArrayEntry(RE::StaticFunctionTag *, int32_t section_first,
                              int32_t section_second, int index, int value) {
  INIFile::PrimaryType first = static_cast<INIFile::PrimaryType>(section_first);
  INIFile::SecondaryType second =
      static_cast<INIFile::SecondaryType>(section_second);
  ...
  std::string key(shse::GlowName(shse::GlowReason(index)));
  StringUtils::ToLower(key);
  DBG_VMESSAGE("Put config setting (glow array) {}/{}/{} = {}",
               INIFile::PrimaryTypeString(first),
               INIFile::SecondaryTypeString(second), key.c_str(), value);
  ini->PutSetting(first, second, key.c_str(), static_cast<double>(value));
}
...
void SetLootableForProducer(RE::StaticFunctionTag *, RE::TESForm *producer,
                            RE::TESForm *lootable) {
  if (!producer) {
  ...
  REL_MESSAGE("Store Lootable 0x{:08x} for producer 0x{:08x}",
              lootable->GetFormID(), producer->GetFormID());
  RE::TESLevItem *leveledItem(lootable->As<RE::TESLevItem>());
  if (leveledItem) {
    shse::ProducerLootables::Instance().ResolveLootableForProducer(producer,
                                                                   leveledItem);
    return;
  }
  ...
  REL_WARNING("Lootable 0x{:08x} for producer 0x{:08x} has invalid Form Type",
              producer->GetFormID(), lootable->GetFormID());
}
...
void ReportOKToScan(RE::StaticFunctionTag *, const bool delayed,
                    const int nonce) {
...
void AddEntryToTransferList(RE::StaticFunctionTag *, RE::TESForm *entry,
...
void NotifyActivated(RE::StaticFunctionTag *, RE::TESForm *itemForm,
                     int itemType, bool collectible, int refrID, int baseID,
                     bool notify, RE::BSFixedString baseName, int count,
                     bool activated, bool isSilent, bool isWhitelisted) {
  DBG_VMESSAGE("NotifyActivated={} for REFR 0x{:08x} to 0x{:08x}/{}", activated,
               refrID, baseID, baseName.c_str());
  if (activated) {
    if (notify) {
    ...
    if (collectible) {
      shse::CollectionManager::Collectibles().CheckEnqueueAddedItem(
          itemForm->As<RE::TESBoundObject>(),
          INIFile::SecondaryType::containers, ObjectType(itemType));
    }
  }
  shse::ScanGovernor::Instance().UnlockHarvest(
      RE::FormID(refrID), RE::FormID(baseID), baseName.c_str(), isSilent);
}

bool ActivateItem(const char *context, RE::TESObjectREFR *target,
                  RE::TESObjectREFR *activator, bool suppressMessage,
                  int activateCount) {
  if (!target || !activator) {
    if (target) {
      REL_ERROR("{} target {}/0x{:08x} has no activator", context,
                target->GetName(), target->GetFormID());
    } else if (activator) {
      REL_ERROR("{} activator {}/0x{:08x} has no target", context,
                activator->GetName(), activator->GetFormID());
    } else {
  ...
  if (suppressMessage) {
    ...
    setting =
        iniSettingCollection
            ? iniSettingCollection->GetSetting("bShowHUDMessages:Interface")
            : nullptr;
    showHUD = (setting && setting->GetType() == RE::Setting::Type::kBool)
                  ? setting->GetBool()
                  : false;
  }
  if (showHUD) {
  ...
  DBG_VMESSAGE("{} target {}/0x{:08x} with activator {}/0x{:08x} count={}",
               context, target->GetName(), target->GetFormID(),
...
bool ActivateItem1(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem1", target, activator, suppressMessage,
                      activateCount);
}
bool ActivateItem2(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem2", target, activator, suppressMessage,
                      activateCount);
}
bool ActivateItem3(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem3", target, activator, suppressMessage,
                      activateCount);
}
bool ActivateItem4(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem4", target, activator, suppressMessage,
                      activateCount);
}
bool ActivateItem5(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem5", target, activator, suppressMessage,
                      activateCount);
}
bool ActivateItem6(RE::StaticFunctionTag *, RE::TESObjectREFR *target,
                   RE::TESObjectREFR *activator, bool suppressMessage,
                   int activateCount) {
  return ActivateItem("ActivateItem6", target, activator, suppressMessage,
                      activateCount);
}

void ProcessContainerCollectibles(RE::StaticFunctionTag *,
...
bool IsQuestTarget(RE::StaticFunctionTag *, RE::TESForm *item) {
  ...
  REL_MESSAGE("Item {}/0x{:08x} is {}a Quest Target",
              item ? item->GetName() : "invalid",
              item ? item->GetFormID() : InvalidForm, cannotLoot ? "" : "not ");
  return cannotLoot;
}
...
std::string ValidTransferTargetLocation(RE::StaticFunctionTag *,
                                        RE::TESObjectREFR *refr,
                                        const bool linksChest,
  ...
  DBG_VMESSAGE("Check REFR 0x{:08x} as transfer target - linked to chest {}, "
               "known good {}",
               refr->GetFormID(), linksChest, knownGood);
  ...
  if (linksChest) {
    ...
    const RE::TESObjectACTI *activator(
        refr->GetBaseObject()->As<RE::TESObjectACTI>());
    if (activator) {
      DBG_VMESSAGE("Check ACTI {}/0x{:08x} for linked container",
                   activator->GetFullName(), activator->GetFormID());
      const RE::TESObjectREFR *linkedRefr(refr->GetLinkedRef(nullptr));
      if (linkedRefr) {
        container = linkedRefr->GetBaseObject()->As<RE::TESObjectCONT>();
        if (container) {
          DBG_VMESSAGE("ACTI {}/0x{:08x} has linked container {}/0x{:08x}",
                       activator->GetFullName(), activator->GetFormID(),
  ...
  if (!container) {
    REL_ERROR(
        "REFR 0x{:08x} for ValidTransferTargetLocation is not a container",
        refr->GetFormID());
    return "";
  }

  if (container->data.flags.any(RE::CONT_DATA::Flag::kRespawn)) {
    REL_ERROR("Respawning container {}/0x{:08x} for REFR 0x{:08x} not a valid "
              "transfer target",
              container->GetFullName(), container->GetFormID(),
              refr->GetFormID());
    return "";
  ...
  if (shse::ManagedList::TransferList().HasContainer(container->GetFormID())) {
    REL_ERROR("Multiplexed linked container {}/0x{:08x} for REFR 0x{:08x} is "
  ...
  if (!knownGood && !shse::LocationTracker::Instance().IsPlayerAtHome()) {
    REL_ERROR("Player not in their house, cannot use {}/0x{:08x} for REFR "
              "0x{:08x} as a transfer target",
              container->GetFullName(), container->GetFormID(),
              refr->GetFormID());
    return "";
  }
  std::string placeName(
      shse::LocationTracker::Instance().CurrentPlayerPlace()->GetName());
  if (knownGood) {
    if (placeName.empty()) {
      placeName = "Unknown";
    }
    DBG_VMESSAGE("Forced known good @ {} -> Transfer target {}/0x{:08x}",
                 placeName, container->GetFullName(), container->GetFormID());
    return placeName;
  } else {
    DBG_VMESSAGE("Player house {} -> Transfer target {}/0x{:08x}", placeName,
                 container->GetFullName(), container->GetFormID());
    return placeName;
  }
}
...
std::string SellItem(RE::StaticFunctionTag *, RE::TESForm *item,
                     const bool excessOnly) {
  shse::ContainerLister lister(INIFile::SecondaryType::deadbodies,
                               RE::PlayerCharacter::GetSingleton());
  return lister.SellItem(item->As<RE::TESBoundObject>(), excessOnly);
}

std::string TransferItem(RE::StaticFunctionTag *, RE::TESForm *item,
                         const bool excessOnly) {
  shse::ContainerLister lister(INIFile::SecondaryType::deadbodies,
                               RE::PlayerCharacter::GetSingleton());
...
std::string DeleteItem(RE::StaticFunctionTag *, RE::TESForm *item,
                       const bool excessOnly) {
  shse::ContainerLister lister(INIFile::SecondaryType::deadbodies,
                               RE::PlayerCharacter::GetSingleton());
...
bool IsLootableObject(RE::StaticFunctionTag *, RE::TESObjectREFR *refr) {
  ...
  if (formType == RE::FormType::Door || formType == RE::FormType::Furniture ||
      formType == RE::FormType::Hazard ||
      formType == RE::FormType::IdleMarker ||
      formType == RE::FormType::MovableStatic ||
      formType ==
          RE::FormType::Projectile || // do not allow whitelist/blacklist - need
                                      // to do from Inventory
...
RE::BSFixedString PrintFormID(RE::StaticFunctionTag *, const int formID) {
  return RE::BSFixedString(
      StringUtils::FormIDString(RE::FormID(formID)).c_str());
}
...
RE::BSFixedString Replace(RE::StaticFunctionTag *, RE::BSFixedString str,
                          RE::BSFixedString target,
                          RE::BSFixedString replacement) {
  std::string s_str(str.c_str());
  return (StringUtils::Replace(s_str, target.c_str(), replacement.c_str()))
             ? s_str.c_str()
             : nullptr;
}

RE::BSFixedString ReplaceArray(RE::StaticFunctionTag *, RE::BSFixedString str,
                               std::vector<RE::BSFixedString> targets,
                               std::vector<RE::BSFixedString> replacements) {
  std::string result(str.c_str());
  if (result.empty() || targets.size() != replacements.size())
    return nullptr;

  for (std::vector<RE::BSFixedString>::const_iterator
           target = targets.cbegin(),
           replacement = replacements.cbegin();
...
// void RecordAddedItem(RE::StaticFunctionTag*, RE::TESForm* baseItem, int
// scope, int objectType)
// {
// 	shse::CollectionManager::Collectibles().CheckEnqueueAddedItem(
// 		baseItem->As<RE::TESBoundObject>(),
// INIFile::SecondaryType(scope), ObjectType(objectType));
// }
...
int CollectionsInGroup(RE::StaticFunctionTag *, const std::string groupName) {
  return shse::CollectionManager::Collectibles().NumberOfActiveCollections(
      groupName);
}

std::string CollectionNameByIndexInGroup(RE::StaticFunctionTag *,
                                         const std::string groupName,
                                         const int collectionIndex) {
  return shse::CollectionManager::Collectibles().NameByIndexInGroup(
      groupName, collectionIndex);
}

std::string CollectionDescriptionByIndexInGroup(RE::StaticFunctionTag *,
                                                const std::string groupName,
                                                const int collectionIndex) {
  return shse::CollectionManager::Collectibles().DescriptionByIndexInGroup(
      groupName, collectionIndex);
}

bool CollectionAllowsRepeats(RE::StaticFunctionTag *,
                             const std::string groupName,
                             const std::string collectionName) {
  return shse::CollectionManager::Collectibles().PolicyRepeat(groupName,
                                                              collectionName);
}
bool CollectionNotifies(RE::StaticFunctionTag *, const std::string groupName,
                        const std::string collectionName) {
  return shse::CollectionManager::Collectibles().PolicyNotify(groupName,
                                                              collectionName);
}
int CollectionAction(RE::StaticFunctionTag *, const std::string groupName,
                     const std::string collectionName) {
  return static_cast<int>(shse::CollectionManager::Collectibles().PolicyAction(
      groupName, collectionName));
}
void PutCollectionAllowsRepeats(RE::StaticFunctionTag *,
                                const std::string groupName,
                                const std::string collectionName,
                                const bool allowRepeats) {
  shse::CollectionManager::Collectibles().PolicySetRepeat(
      groupName, collectionName, allowRepeats);
}
void PutCollectionNotifies(RE::StaticFunctionTag *, const std::string groupName,
                           const std::string collectionName,
                           const bool notifies) {
  shse::CollectionManager::Collectibles().PolicySetNotify(
      groupName, collectionName, notifies);
}
void PutCollectionAction(RE::StaticFunctionTag *, const std::string groupName,
                         const std::string collectionName, const int action) {
  shse::CollectionManager::Collectibles().PolicySetAction(
      groupName, collectionName,
      shse::CollectibleHandlingFromIniSetting(double(action)));
}

bool CollectionGroupAllowsRepeats(RE::StaticFunctionTag *,
                                  const std::string groupName) {
  return shse::CollectionManager::Collectibles().GroupPolicyRepeat(groupName);
}
bool CollectionGroupNotifies(RE::StaticFunctionTag *,
                             const std::string groupName) {
  return shse::CollectionManager::Collectibles().GroupPolicyNotify(groupName);
}
int CollectionGroupAction(RE::StaticFunctionTag *,
                          const std::string groupName) {
  return static_cast<int>(
      shse::CollectionManager::Collectibles().GroupPolicyAction(groupName));
}
void PutCollectionGroupAllowsRepeats(RE::StaticFunctionTag *,
                                     const std::string groupName,
                                     const bool allowRepeats) {
  shse::CollectionManager::Collectibles().GroupPolicySetRepeat(groupName,
                                                               allowRepeats);
}
void PutCollectionGroupNotifies(RE::StaticFunctionTag *,
                                const std::string groupName,
                                const bool notifies) {
  shse::CollectionManager::Collectibles().GroupPolicySetNotify(groupName,
                                                               notifies);
}
void PutCollectionGroupAction(RE::StaticFunctionTag *,
                              const std::string groupName, const int action) {
  shse::CollectionManager::Collectibles().GroupPolicySetAction(
      groupName, shse::CollectibleHandlingFromIniSetting(double(action)));
}

int CollectionTotal(RE::StaticFunctionTag *, const std::string groupName,
                    const std::string collectionName) {
  return static_cast<int>(shse::CollectionManager::Collectibles().TotalItems(
      groupName, collectionName));
}

std::string CollectionStatus(RE::StaticFunctionTag *,
                             const std::string groupName,
                             const std::string collectionName) {
  return shse::CollectionManager::Collectibles().StatusMessage(groupName,
                                                               collectionName);
}

int CollectionObtained(RE::StaticFunctionTag *, const std::string groupName,
                       const std::string collectionName) {
  return static_cast<int>(shse::CollectionManager::Collectibles().ItemsObtained(
      groupName, collectionName));
}

int AdventureTypeCount(RE::StaticFunctionTag *) {
  return static_cast<int>(
      shse::AdventureTargets::Instance().AvailableAdventureTypes());
}

std::string AdventureTypeName(RE::StaticFunctionTag *,
                              const int adventureType) {
  return shse::AdventureTargets::Instance().AdventureTypeName(
      size_t(adventureType));
}

int ViableWorldsByType(RE::StaticFunctionTag *, const int adventureType) {
  return static_cast<int>(shse::AdventureTargets::Instance().ViableWorldCount(
      size_t(adventureType)));
}

std::string WorldNameByIndex(RE::StaticFunctionTag *, const int worldIndex) {
  return shse::AdventureTargets::Instance().ViableWorldNameByIndexInView(
      size_t(worldIndex));
}

void SetAdventureTarget(RE::StaticFunctionTag *, const int worldIndex) {
  return shse::AdventureTargets::Instance().SelectCurrentDestination(
      size_t(worldIndex));
}
...
void SyncShader(RE::StaticFunctionTag *, const int index,
...
std::string TimelineDayName(RE::StaticFunctionTag *, const int timelineDay) {
  return shse::Saga::Instance().DateStringByIndex(
      static_cast<unsigned int>(timelineDay));
}
...
std::string GetSagaDayPage(RE::StaticFunctionTag *, const int pageNumber) {
  return shse::Saga::Instance().PageByNumber(
...
bool RegisterFuncs(RE::BSScript::Internal::VirtualMachine *a_vm) {
  ...
  a_vm->RegisterFunction("GetTextObjectType", SHSE_PROXY,
                         papyrus::GetTextObjectType);

  a_vm->RegisterFunction("NotifyActivated", SHSE_PROXY,
                         papyrus::NotifyActivated);
  ...
  a_vm->RegisterFunction("NotifyManualLootItem", SHSE_PROXY,
                         papyrus::NotifyManualLootItem);
  ...
  a_vm->RegisterFunction("IsLootableObject", SHSE_PROXY,
                         papyrus::IsLootableObject);
  a_vm->RegisterFunction("UseUnderwear", SHSE_PROXY, papyrus::UseUnderwear);
  a_vm->RegisterFunction("ValidTransferTargetLocation", SHSE_PROXY,
                         papyrus::ValidTransferTargetLocation);
  a_vm->RegisterFunction("SupportsExcessHandling", SHSE_PROXY,
                         papyrus::SupportsExcessHandling);
  ...
  a_vm->RegisterFunction("CheckItemAsExcess", SHSE_PROXY,
                         papyrus::CheckItemAsExcess);
  a_vm->RegisterFunction("ProcessContainerCollectibles", SHSE_PROXY,
                         papyrus::ProcessContainerCollectibles);
  a_vm->RegisterFunction("TryForceHarvest", SHSE_PROXY,
                         papyrus::TryForceHarvest);

  a_vm->RegisterFunction("GetSetting", SHSE_PROXY, papyrus::GetSetting);
  a_vm->RegisterFunction("GetSettingObjectArrayEntry", SHSE_PROXY,
                         papyrus::GetSettingObjectArrayEntry);
  a_vm->RegisterFunction("GetSettingGlowArrayEntry", SHSE_PROXY,
                         papyrus::GetSettingGlowArrayEntry);
  a_vm->RegisterFunction("PutSetting", SHSE_PROXY, papyrus::PutSetting);
  a_vm->RegisterFunction("PutSettingObjectArrayEntry", SHSE_PROXY,
                         papyrus::PutSettingObjectArrayEntry);
  a_vm->RegisterFunction("PutSettingGlowArrayEntry", SHSE_PROXY,
                         papyrus::PutSettingGlowArrayEntry);

  a_vm->RegisterFunction("GetObjectTypeNameByType", SHSE_PROXY,
                         papyrus::GetObjectTypeNameByType);
  a_vm->RegisterFunction("GetObjectTypeByName", SHSE_PROXY,
                         papyrus::GetObjectTypeByName);
  a_vm->RegisterFunction("GetResourceTypeByName", SHSE_PROXY,
                         papyrus::GetResourceTypeByName);
  ...
  a_vm->RegisterFunction("SetLootableForProducer", SHSE_PROXY,
                         papyrus::SetLootableForProducer);
  a_vm->RegisterFunction("ClearLootableForProducer", SHSE_PROXY,
                         papyrus::ClearLootableForProducer);
  a_vm->RegisterFunction("SetHarvested", SHSE_PROXY, papyrus::SetHarvested);
  a_vm->RegisterFunction("PrepareSPERGMining", SHSE_PROXY,
                         papyrus::PrepareSPERGMining);
  a_vm->RegisterFunction("PostprocessSPERGMining", SHSE_PROXY,
                         papyrus::PostprocessSPERGMining);
  a_vm->RegisterFunction("PeriodicReminder", SHSE_PROXY,
                         papyrus::PeriodicReminder);
  a_vm->RegisterFunction("PeriodicReminderString", SHSE_PROXY,
  ...
  a_vm->RegisterFunction("AddEntryToTransferList", SHSE_PROXY,
                         papyrus::AddEntryToTransferList);
  ...
  a_vm->RegisterFunction("CollectionsInUse", SHSE_PROXY,
                         papyrus::CollectionsInUse);
  a_vm->RegisterFunction("CollectionGroups", SHSE_PROXY,
                         papyrus::CollectionGroups);
  a_vm->RegisterFunction("CollectionGroupName", SHSE_PROXY,
                         papyrus::CollectionGroupName);
  a_vm->RegisterFunction("CollectionGroupFile", SHSE_PROXY,
                         papyrus::CollectionGroupFile);
  a_vm->RegisterFunction("CollectionsInGroup", SHSE_PROXY,
                         papyrus::CollectionsInGroup);
  a_vm->RegisterFunction("CollectionNameByIndexInGroup", SHSE_PROXY,
                         papyrus::CollectionNameByIndexInGroup);
  a_vm->RegisterFunction("CollectionDescriptionByIndexInGroup", SHSE_PROXY,
                         papyrus::CollectionDescriptionByIndexInGroup);
  a_vm->RegisterFunction("CollectionAllowsRepeats", SHSE_PROXY,
                         papyrus::CollectionAllowsRepeats);
  a_vm->RegisterFunction("CollectionNotifies", SHSE_PROXY,
                         papyrus::CollectionNotifies);
  a_vm->RegisterFunction("CollectionAction", SHSE_PROXY,
                         papyrus::CollectionAction);
  a_vm->RegisterFunction("CollectionTotal", SHSE_PROXY,
                         papyrus::CollectionTotal);
  a_vm->RegisterFunction("CollectionStatus", SHSE_PROXY,
                         papyrus::CollectionStatus);
  a_vm->RegisterFunction("CollectionObtained", SHSE_PROXY,
                         papyrus::CollectionObtained);
  a_vm->RegisterFunction("PutCollectionAllowsRepeats", SHSE_PROXY,
                         papyrus::PutCollectionAllowsRepeats);
  a_vm->RegisterFunction("PutCollectionNotifies", SHSE_PROXY,
                         papyrus::PutCollectionNotifies);
  a_vm->RegisterFunction("PutCollectionAction", SHSE_PROXY,
                         papyrus::PutCollectionAction);
  a_vm->RegisterFunction("CollectionGroupAllowsRepeats", SHSE_PROXY,
                         papyrus::CollectionGroupAllowsRepeats);
  a_vm->RegisterFunction("CollectionGroupNotifies", SHSE_PROXY,
                         papyrus::CollectionGroupNotifies);
  a_vm->RegisterFunction("CollectionGroupAction", SHSE_PROXY,
                         papyrus::CollectionGroupAction);
  a_vm->RegisterFunction("PutCollectionGroupAllowsRepeats", SHSE_PROXY,
                         papyrus::PutCollectionGroupAllowsRepeats);
  a_vm->RegisterFunction("PutCollectionGroupNotifies", SHSE_PROXY,
                         papyrus::PutCollectionGroupNotifies);
  a_vm->RegisterFunction("PutCollectionGroupAction", SHSE_PROXY,
                         papyrus::PutCollectionGroupAction);

  a_vm->RegisterFunction("AdventureTypeCount", SHSE_PROXY,
                         papyrus::AdventureTypeCount);
  a_vm->RegisterFunction("AdventureTypeName", SHSE_PROXY,
                         papyrus::AdventureTypeName);
  a_vm->RegisterFunction("ViableWorldsByType", SHSE_PROXY,
                         papyrus::ViableWorldsByType);
  a_vm->RegisterFunction("WorldNameByIndex", SHSE_PROXY,
                         papyrus::WorldNameByIndex);
  a_vm->RegisterFunction("SetAdventureTarget", SHSE_PROXY,
                         papyrus::SetAdventureTarget);
  a_vm->RegisterFunction("ClearAdventureTarget", SHSE_PROXY,
                         papyrus::ClearAdventureTarget);
  a_vm->RegisterFunction("HasAdventureTarget", SHSE_PROXY,
                         papyrus::HasAdventureTarget);

  a_vm->RegisterFunction("ToggleCalibration", SHSE_PROXY,
                         papyrus::ToggleCalibration);
  ...
  a_vm->RegisterFunction("GetTimelineDays", SHSE_PROXY,
                         papyrus::GetTimelineDays);
  a_vm->RegisterFunction("TimelineDayName", SHSE_PROXY,
                         papyrus::TimelineDayName);
  a_vm->RegisterFunction("PageCountForDay", SHSE_PROXY,
                         papyrus::PageCountForDay);
