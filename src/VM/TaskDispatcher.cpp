...
TaskDispatcher::TaskDispatcher()
    : m_player(nullptr), m_legacyCarryWeightChecked(false) {
...
void TaskDispatcher::GlowObjects() {
  // Dispatch the queued glow requests via the TaskInterface - do not require
  // activation availability here
  ...
  decltype(m_queuedGlow) queued;
  {
    ...
    if (m_queuedGlow.size()) {
      if (status != ScanStatus::GoodToGo) {
        REL_WARNING("Delay {} queued Glow requests, scan status {}",
                    m_queuedGlow.size(), status);
        return;
      }
  ...
  m_taskInterface->AddTask([=](void) {
    ...
    for (const auto request : queued) {
      ...
      if (!doneRefrs.insert(refr->GetFormID()).second) {
        REL_WARNING("Skipping repeat glow request for REFR 0x{:08x}",
                    refr->GetFormID());
        continue;
      }
      if (refr->Is3DLoaded() && !refr->IsDisabled()) {
        RE::TESEffectShader *shader(nullptr);
        if (static_cast<int>(glowReason) >= 0 &&
            static_cast<int>(glowReason) <=
                static_cast<int>(GlowReason::SimpleTarget)) {
          shader = m_shaders[static_cast<int>(glowReason)];
        } else {
          shader = m_shaders[static_cast<int>(GlowReason::SimpleTarget)];
        }
        if (!shader) {
          REL_WARNING("Skipping glow request for REFR 0x{:08x}, no shader",
                      refr->GetFormID());
          continue;
        }
...
void TaskDispatcher::SetShader(const int index, RE::TESEffectShader *shader) {
  ...
  REL_MESSAGE("Shader 0x{:08x} set for GlowReason {}", shader->formID,
              GlowName(static_cast<GlowReason>(index)));
  m_shaders[index] = shader;
}

void TaskDispatcher::EnqueueLootFromNPC(RE::TESObjectREFR *npc,
  ...
  m_taskInterface->AddTask([=](void) {
    ...
    for (const auto request : queued) {
      ...
      CollectionManager::Collectibles().CheckEnqueueAddedItem(
          item, INIFile::SecondaryType::deadbodies, objectType);
      npc->RemoveItem(item, count, RE::ITEM_REMOVE_REASON::kRemove, nullptr,
                      m_player);
    }
  });
}
      ...
      if (status != ScanStatus::GoodToGo) {
        ...
        if (const auto processLists = RE::ProcessLists::GetSingleton();
          ...
          for (auto &targetHandle : processLists->highActorHandles) {
            if (const auto target = targetHandle.get();
                target && target->GetActorRuntimeData().currentProcess) {
              if (const auto base = target->GetActorBase();
                  base && !base->AffectsStealthMeter()) {
                continue;
              }
...
void TaskDispatcher::EnqueueCarryWeightStateChange(bool doReload,
                                                   const bool needsBeefUp) {
  ...
  m_taskInterface->AddTask([=](void) {
    ...
    if (!m_legacyCarryWeightChecked) {
      ...
      if (!isBeefedUp) {
        RE::ActorValueOwner *actorValueOwner(
            RE::PlayerCharacter::GetSingleton()->AsActorValueOwner());
        if (actorValueOwner) {
          int carryWeight = static_cast<int>(
              actorValueOwner->GetActorValue(RE::ActorValue::kCarryWeight));
          int weightDelta(0);
    ...
    if (isBeefedUp && !needsBeefUp) {
      ...
      m_player->GetMagicTarget()->DispelEffect(
          PlayerState::Instance().CarryWeightSpell(), handle);
    } else if (!isBeefedUp and needsBeefUp) {
      // Cast the SPEL on the player
      RE::MagicCaster *caster(
          RE::PlayerCharacter::GetSingleton()->GetMagicCaster(
              RE::MagicSystem::CastingSource::kInstant));
      if (caster) {
        REL_MESSAGE("Cast CarryWeight SPEL on Player");
        caster->CastSpellImmediate(PlayerState::Instance().CarryWeightSpell(),
                                   true, m_player, 0.0f, false, 0.0f, m_player);
      }
    }
  });
}
