...
private:
  ...
  // allow extended interval before looting if 'leveled list on death' perks
  // apply to player
  static constexpr int ReallyDeadWaitIntervalSeconds = 2;
  static constexpr int ReallyDeadWaitIntervalSecondsLong = 4;

  // Dead bodies pending looting, in order of our encountering them, with the
  // time of registration. Used to delay looting bodies until the game has
