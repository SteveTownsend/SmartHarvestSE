...
// combat victims and loot them once combat ends, no sooner than N seconds after
// their death
...
void ActorTracker::RecordIfKilledByParty(const RE::Actor *victim) {
  ...
  if (!victim || !victim->GetActorRuntimeData().myKiller ||
      !m_checkedBodies.insert(victim).second)
    return;
  ...
  if (killer == RE::PlayerCharacter::GetSingleton()) {
    DBG_MESSAGE("Record killing of {}/0x{:08x}",
                victim->As<RE::TESObjectREFR>()->GetName(),
                victim->As<RE::TESObjectREFR>()->GetFormID());
...
void ActorTracker::ReleaseIfReliablyDead(DistanceToTarget &refs) {
  RecursiveLockGuard guard(m_actorLock);
  const int interval(shse::PlayerState::Instance().PerksAddLeveledItemsOnDeath()
                         ? ReallyDeadWaitIntervalSecondsLong
                         : ReallyDeadWaitIntervalSeconds);
...
void ActorTracker::UpdateFrom(const nlohmann::json &j) {
  ...
  for (const nlohmann::json &victim : j["victims"]) {
    RecordVictim(PartyVictim(victim["name"].get<std::string>(),
                             victim["time"].get<float>()));
  }
}
