...
void LocationTracker::RecordMarkedPlaces() {
  ...
  m_markedPlaces =
      AdventureTargets::Instance().GetWorldMarkedPlaces(m_playerParentWorld);
  ...
  std::for_each(m_markedPlaces.cbegin(), m_markedPlaces.cend(),
                [&](const auto &posForm) {
  ...
  coordinates.setcontent(static_cast<alglib::ae_int_t>(numPlaces), 3,
                         &pointData[0]);
  ...
  alglib::kdtreebuildtagged(coordinates, tags,
                            static_cast<alglib::ae_int_t>(numPlaces), 3, 0, 2,
                            m_markers);
}

CompassDirection LocationTracker::DirectionToDestinationFromStart(
    const AlglibPosition &start, const AlglibPosition &destination) const {
  ...
  // which arc of the compass does this fall into? The value is in the range
  // [-180.0, 180.0]
  static const std::vector<std::pair<double, CompassDirection>> directions = {
      {-157.5, CompassDirection::South}, {-112.5, CompassDirection::SouthWest},
      {-67.5, CompassDirection::West},   {-22.5, CompassDirection::NorthWest},
      {22.5, CompassDirection::North},   {67.5, CompassDirection::NorthEast},
      {112.5, CompassDirection::East},   {157.5, CompassDirection::SouthEast},
...
std::string
LocationTracker::ParentLocationName(const RE::BGSLocation *location) const {
...
std::string LocationTracker::Proximity(const double milesAway,
...
std::string
LocationTracker::ConversationalDistance(const double milesAway) const {
...
std::string LocationTracker::PlayerExactLocation() const {
  ...
  if (m_playerLocation) {
    locationStr << " at " << m_playerLocation->GetName() << "/0x"
                << StringUtils::FromFormID(m_playerLocation->GetFormID())
                << ' ';
  }
  if (m_playerCellID != InvalidForm) {
    RE::TESObjectCELL *cell(
        RE::TESForm::LookupByID<RE::TESObjectCELL>(m_playerCellID));
    locationStr << " in Cell " << FormUtils::SafeGetFormEditorID(cell) << "/0x"
                << StringUtils::FromFormID(m_playerCellID);
  }
  return locationStr.str();
}

void LocationTracker::PrintPlayerLocation(
...
std::string LocationTracker::NearbyLocationAsString(
    const RE::BGSLocation *location, const double milesAway,
    CompassDirection heading, const bool historic) const {
  RE::BSFixedString locationText(papyrus::GetTranslation(
      nullptr, historic ? RE::BSFixedString("$SHSE_WHERE_WAS_I")
                        : RE::BSFixedString("$SHSE_WHERE_AM_I")));
  ...
  if (!locationMessage.empty()) {
    ...
    StringUtils::Replace(locationMessage, "{VICINITY}",
                         ParentLocationName(location));
  }
  return locationMessage;
}

void LocationTracker::PrintAdventureTargetInfo(const RE::BGSLocation *location,
                                               const double milesAway,
                                               CompassDirection heading) const {
  static RE::BSFixedString locationText(papyrus::GetTranslation(
      nullptr, RE::BSFixedString("$SHSE_WHERE_IS_ADVENTURE_TARGET")));
  if (!locationText.empty()) {
    ...
    if (milesAway < OnlyYards) {
      ...
      StringUtils::Replace(locationMessage, "{TARGET}",
...
void LocationTracker::PrintDifferentWorld(
    const RE::TESWorldSpace *world) const {
  std::string locationMessage;
  static RE::BSFixedString locationText(papyrus::GetTranslation(
      nullptr, RE::BSFixedString("$SHSE_ADVENTURE_DIFFERENT_WORLD")));
  if (!locationText.empty()) {
...
void LocationTracker::DisplayPlayerLocation() const {
  ...
  // display location relative to Adventure Target, or nearest marker if no
...
std::string LocationTracker::LocationRelativeToNearestMapMarker(
    const AlglibPosition &position, const bool historic) const {
  std::string locationStr;
  ...
  CompassDirection heading(DirectionToDestinationFromStart(
      nearestMarker.EndPoint(), nearestMarker.StartPoint()));

  double milesAway(UnitsToMiles(nearestMarker.UnitsAway()));
  RE::BGSLocation *location(
      RE::TESForm::LookupByID<RE::BGSLocation>(nearestMarker.LocationID()));
  if (location) {
    DBG_MESSAGE("Position is {} of nearest Location map marker {}/0x{:08x} at "
                "distance {:0.2f} miles",
                CompassDirectionName(heading).c_str(), location->GetName(),
                nearestMarker.LocationID(), milesAway);
    locationStr =
        NearbyLocationAsString(location, milesAway, heading, historic);
  } else {
    REL_WARNING("Could not determine Location for 0x{:08x}",
                nearestMarker.LocationID());
  }
  return locationStr;
}

void LocationTracker::PlayerLocationRelativeToNearestMapMarker(
  ...
  if (m_playerLocation) {
    DBG_MESSAGE("Player ({:0.2f}, {:0.2f}) is in location {}/0x{:08x}",
                playerPos[0], playerPos[1], m_playerLocation->GetName(),
                m_playerLocation->GetFormID());
...
LocationTracker::NearestMapMarker(const AlglibPosition &refPos) const {
  ...
  DBG_MESSAGE(
      "Nearest Map Marker to ({:0.2f},{:0.2f},{:0.2f}) has formID 0x{:08x}",
      refPos[0], refPos[1], refPos[2], location);
...
double DistanceBetween(const AlglibPosition &pos1, const AlglibPosition &pos2) {
  ...
  DBG_MESSAGE("pos1({:0.2f},{:0.2f},{:0.2f}), pos2({:0.2f},{:0.2f},{:0.2f}), "
              "distance {:0.2f} units",
...
RelativeLocationDescriptor
LocationTracker::MarkedLocationPosition(const Position targetPosition,
                                        const RE::BGSLocation *location,
                                        const AlglibPosition &refPos) const {
  AlglibPosition markerPos(
      {targetPosition[0], targetPosition[1], targetPosition[2]});
  double unitsAway(DistanceBetween(markerPos, refPos));
  return RelativeLocationDescriptor(refPos, markerPos, location->GetFormID(),
                                    unitsAway);
}

const RE::BGSLocation *
LocationTracker::PlayerLocationRelativeToAdventureTarget() const {
#ifdef _PROFILING
  ...
  const RE::BGSLocation *location(
      AdventureTargets::Instance().TargetLocation());
  ...
  if (m_playerIndoors) {
    static RE::BSFixedString locationText(papyrus::GetTranslation(
        nullptr, RE::BSFixedString("$SHSE_GO_OUTSIDE_ADVENTURE_TARGET")));
    if (!locationText.empty()) {
      std::string locationMessage(locationText);
      if (!locationMessage.empty()) {
        RE::DebugNotification(locationMessage.c_str());
  ...
  AlglibPosition playerPos(PlayerState::Instance().GetAlglibPosition());
  RelativeLocationDescriptor targetLocation(
      MarkedLocationPosition(targetPosition, location, playerPos));
  if (targetLocation.equals(RelativeLocationDescriptor::Invalid())) {
    REL_WARNING("Could not determine adventure target marker");
    return nullptr;
  }
  CompassDirection heading(DirectionToDestinationFromStart(
      targetLocation.EndPoint(), targetLocation.StartPoint()));

  double milesAway(UnitsToMiles(targetLocation.UnitsAway()));
  DBG_MESSAGE("Player is {} of adventure target marker {}/0x{:08x} at distance "
              "{:0.2f} miles",
              CompassDirectionName(heading).c_str(), location->GetName(),
              targetLocation.LocationID(), milesAway);
  PrintAdventureTargetInfo(location, milesAway, heading);
  return location;
}

CellOwnership
LocationTracker::GetCellOwnership(const RE::TESObjectCELL *cell) const {
  if (!cell)
  ...
  if (owner->formType == RE::FormType::NPC) {
    ...
    RE::TESNPC *playerBase =
        RE::PlayerCharacter::GetSingleton()->GetActorBase();
  ...
  REL_WARNING("Owner 0x{:08x} exists but uncategorized in cell 0x{:08x}",
              owner->GetFormID(), cell->GetFormID());
  return CellOwnership::NoOwner;
}

RE::TESForm *
LocationTracker::GetCellOwner(const RE::TESObjectCELL *cell) const {
  for (const RE::BSExtraData &extraData : cell->extraList) {
    if (extraData.GetType() == RE::ExtraDataType::kOwnership) {
      DBG_VMESSAGE(
          "GetCellOwner Hit 0x{:08x}",
          static_cast<const RE::ExtraOwnership &>(extraData).owner->formID);
...
const RE::TESWorldSpace *
LocationTracker::ParentWorld(const RE::TESObjectCELL *cell) {
  ...
  while (world) {
    if (world->locationMap.size() > 0) {
      ...
      DBG_MESSAGE(
          "Found location-bearing parent world {}/0x{:08x} for cell 0x{:08x}",
    ...
    if (std::find(worlds.cbegin(), worlds.cend(), world) != worlds.cend()) {
      ...
      REL_ERROR("Cycle in worldspace graph at {}/0x{:08x} for cell 0x{:08x}",
                world->GetName(), world->GetFormID(), cell->GetFormID());
      break;
    }
...
bool LocationTracker::Refresh() {
  ...
  if (playerCellID != originalCellID) {
    // CELL change only triggers visited-place update if we went from outdoors
    ...
    if (playerCell) {
      if (m_playerIndoors) {
        ...
        const auto playerCoordinates(
            const_cast<RE::TESObjectCELL *>(playerCell)->GetCoordinates());
        m_playerCellX = playerCoordinates->cellX;
        m_playerCellY = playerCoordinates->cellY;
        REL_MESSAGE("Player cell updated to 0x{:08x} outdoors at ({},{})",
                    m_playerCellID, m_playerCellX, m_playerCellY);
      }
      ...
      if (parentWorld && parentWorld != m_playerParentWorld) {
        ...
        DBG_MESSAGE("Player Parent WorldSpace updated to {}/0x{:08x}",
                    m_playerParentWorld->GetName(),
                    m_playerParentWorld->GetFormID());
  ...
  if (m_playerCellID != originalCellID || playerLocation != originalLocation ||
    ...
    // know we left a Location
    playerMoved = playerMoved || playerLocation != originalLocation ||
                  m_tellPlayerIfCanLootAfterLoad;

    // Output messages for any looting-restriction place change
    static const bool allowIfRestricted(false);
    bool couldLootInPrior(LocationTracker::Instance().IsPlaceLootable(
    ...
    // location change may mark the end of an in-progress 'Venture into the
    // Unknown'
    ...
    if (!IsPlacePlayerHome(m_playerCellID, m_playerLocation)) {
      if (m_playerLocation &&
          PlayerHouses::Instance().IsValidHouseLocation(m_playerLocation)) {
        // record Location as a player house and notify as it is a new one in
        // this game load
    ...
    if (tellPlayer) {
      ...
      if ((couldLootInPrior || m_tellPlayerIfCanLootAfterLoad) &&
        ...
        static RE::BSFixedString restrictedPlaceMsg(papyrus::GetTranslation(
            nullptr, RE::BSFixedString("$SHSE_POPULATED_CHECK")));
    ...
    if (tellPlayer) {
      ...
      if ((!couldLootInPrior || m_tellPlayerIfCanLootAfterLoad) &&
        ...
        static RE::BSFixedString unrestrictedPlaceMsg(papyrus::GetTranslation(
            nullptr, RE::BSFixedString("$SHSE_UNPOPULATED_CHECK")));
...
void LocationTracker::RecordCurrentPlace(const float gameTime) {
  VisitedPlaces::Instance().RecordVisit(
      m_playerParentWorld, m_playerLocation, m_playerCellID,
      PlayerState::Instance().GetPosition(), gameTime);
}

bool LocationTracker::IsPlayerInLootablePlace(
    const bool lootableIfRestricted, const bool lootableIfRestrictedHome) {
  RecursiveLockGuard guard(m_locationLock);
  return IsPlaceLootable(m_playerCellID, m_playerLocation, lootableIfRestricted,
                         lootableIfRestrictedHome);
}

bool LocationTracker::IsPlaceLootable(const RE::FormID cellID,
                                      const RE::BGSLocation *location,
  ...
  DBG_DMESSAGE("Check autoloot for cell 0x{:08x}, location {}/0x{:08x}", cellID,
               ...
               location ? location->GetFormID() : InvalidForm);
  ...
  if (!lootableIfRestricted &&
      IsPlaceRestrictedLootSettlement(cellID, location)) {
...
decltype(LocationTracker::m_adjacentCells)
LocationTracker::AdjacentCells() const {
  RecursiveLockGuard guard(m_locationLock);
...
bool LocationTracker::IsAdjacent(RE::TESObjectCELL *cell) const {
  ...
  if (dx <= 1 && dy <= 1) {
    REL_MESSAGE("Cell 0x{:08x} at ({},{}) is adjacent to player",
                cell->GetFormID(), checkCoordinates->cellX,
                checkCoordinates->cellY);
    return true;
  }
  return false;
}
...
void LocationTracker::RecordAdjacentCells(const RE::TESObjectCELL *current) {
  ...
  if (!m_playerIndoors) {
    ...
    if (current->GetRuntimeData().worldSpace) {
      DBG_VMESSAGE("Worldspace is {}/0x{:08x}",
                   current->GetRuntimeData().worldSpace->GetName(),
                   current->GetRuntimeData().worldSpace->GetFormID());
      auto nextSlot = m_adjacentCells.begin();
      for (const auto &worldCell :
           current->GetRuntimeData().worldSpace->cellMap) {
        ...
        if (candidateCell->IsInteriorCell()) {
          DBG_VMESSAGE("Candidate cell 0x{:08x} flagged as interior",
                       candidateCell->GetFormID());
          continue;
        }
        // check if adjacent to player CELL - do not record if either X/Y grid
        // coordinate differs by more than 1
...
RE::TESObjectCELL *LocationTracker::PlayerCell() const {
  ...
  RE::TESObjectCELL *playerCell(
      RE::PlayerCharacter::GetSingleton()->parentCell);
  RE::FormID currentCellID(playerCell && playerCell->IsAttached()
...
bool LocationTracker::IsPlaceWhitelisted(
    const RE::FormID cellID, const RE::BGSLocation *location) const {
  RecursiveLockGuard guard(m_locationLock);
  // Player Location may be empty e.g. if we are in the wilderness
  // Player Cell should never be empty
  return ManagedList::WhiteList().Contains(location) ||
         ManagedList::WhiteList().ContainsID(cellID);
}

bool LocationTracker::IsPlaceBlacklisted(
    const RE::FormID cellID, const RE::BGSLocation *location) const {
  RecursiveLockGuard guard(m_locationLock);
  // Player Location may be empty e.g. if we are in the wilderness
  // Player Cell should never be empty
  return ManagedList::BlackList().Contains(location) ||
         ManagedList::BlackList().ContainsID(cellID);
}
...
bool LocationTracker::IsPlaceRestrictedLootSettlement(
    const RE::FormID cellID, const RE::BGSLocation *location) const {
  RecursiveLockGuard guard(m_locationLock);
  // whitelist check done before we get called
...
bool LocationTracker::IsPlayerInFriendlyCell() const {
  ...
  DBG_DMESSAGE("Cell ownership {}, allow Ownerless looting = {}",
               CellOwnershipName(ownership).c_str(),
