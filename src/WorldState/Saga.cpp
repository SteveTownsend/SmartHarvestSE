...
size_t Saga::DaysWithEvents() const {
  ...
  ptrdiff_t count(std::count_if(
      m_eventsByDay.cbegin(), m_eventsByDay.cend(),
      [&](const std::vector<SagaEvent> &events) -> bool {
        if (!events.empty()) {
          m_daysWithEvents.push_back({index, {}});
          ++index;
          // sort each day's events by time before we render the events as text.
          // This view on the current dataset is used to display text in MCM.
          auto &sortedEvents = m_daysWithEvents.back().second;
          std::for_each(
              events.cbegin(), events.cend(), [&](const SagaEvent &event) {
                sortedEvents.insert(
                    {std::visit(
                         [&](auto const &e) {
                           return GameCalendar::Instance().DayPartInMinutes(
                               e.GameTime());
                         },
                         event),
                     event});
...
void Saga::AddPaginatedText(std::ostringstream &page, const std::string &text,
...
void Saga::FlushPaginatedText(std::ostringstream &page) const {
  if (m_currentPageLength > 0 && m_hasContent) {
    ...
    // see
    // https://stackoverflow.com/questions/624260/how-to-reuse-an-ostringstream
    // including comments
    page.seekp(0); // seek put ptr to start - no reallocation unless next page
                   // has more text
...
std::string Saga::DateStringByIndex(const unsigned int dayIndex) const {
  ...
  if (dayIndex - 1 < m_daysWithEvents.size()) {
    // seed freeform text pages for this day's events. Text is stateful for some
    // events.
    ...
    for (const auto timeEvent : sortedEvents) {
      ...
      std::string eventStr(std::visit(
          [&](auto const &e) { return e.AsString(); }, timeEvent.second));
      if (!eventStr.empty()) {
        if (first || currentMinute != timeEvent.first) {
          // add newline for new paragraph if this is a time change - not needed
          // if this forces new page
          if (!first) {
          ...
          std::string timestampedEvent(
              GameCalendar::Instance().TimeString(currentMinute));
    ...
    DBG_MESSAGE("Current day has {} pages for {} events",
                m_currentDayPages.size(), sortedEvents.size());
    return GameCalendar::Instance().DateString(
        m_daysWithEvents[dayIndex - 1].first);
  }
  return "";
}
