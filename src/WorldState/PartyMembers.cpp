...
PartyUpdate::PartyUpdate(const RE::Actor *follower,
                         const PartyUpdateType eventType, const float gameTime)
...
void PartyMembers::AdjustParty(const Followers &followers,
  ...
  for (const auto newFollower : followers) {
    if (m_followers.find(newFollower) == m_followers.cend()) {
      ...
      DBG_MESSAGE("Follower {}/0x{:08x} joined party at {:0.3f}",
                  newFollower->GetName(), newFollower->GetFormID(), gameTime);
    }
  }
  for (const auto existingFollower : m_followers) {
    if (followers.find(existingFollower) == followers.cend()) {
      updated = true;
      PartyUpdateType updateType(existingFollower->IsDead(true)
                                     ? PartyUpdateType::Died
                                     : PartyUpdateType::Departed);
      ...
      DBG_MESSAGE("Follower {}/0x{:08x} left party at {:0.3f}",
                  existingFollower->GetName(), existingFollower->GetFormID(),
                  gameTime);
    }
  }
  if (updated) {
...
void PartyMembers::UpdateFrom(const nlohmann::json &j) {
  ...
  for (const nlohmann::json &update : j["updates"]) {
    RE::FormID followerID(
        StringUtils::ToFormID(update["follower"].get<std::string>()));
    PartyUpdateType updateType(
        static_cast<PartyUpdateType>(update["event"].get<int>()));
    const float gameTime(update["time"].get<float>());
    RE::Actor *actor(
        LoadOrder::Instance().RehydrateCosaveFormAs<RE::Actor>(followerID));
    if (!actor) {
