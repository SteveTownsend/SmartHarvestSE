...
bool PopulationCenters::CannotLoot(const RE::FormID cellID,
                                   const RE::BGSLocation *location) const {
  RecursiveLockGuard guard(m_centersLock);
  PopulationCenterSize centerSize(
      SettingsCache::Instance().PreventPopulationCenterLooting());
  if (centerSize == PopulationCenterSize::None)
    return false;

  if (location) {
    const auto locationRecord(m_centers.find(location));
    // if small locations are excluded we automatically exclude any larger, so
    // use >= here, assuming this is a population center
...
void PopulationCenters::Categorize(void) {
  ...
  for (RE::BGSLocation *location : dhnd->GetFormArray<RE::BGSLocation>()) {
    ...
    if (size != PopulationCenterSize::None) {
      REL_MESSAGE("{}/0x{:08x} is population center of type {}",
                  location->GetName(), location->GetFormID(), largestMatch);
      m_centers.insert(std::make_pair(location, size));
    } else {
      DBG_MESSAGE("{}/0x{:08x} is not a population center", location->GetName(),
                  location->GetFormID());
    }
  }

  // We also categorize descendants of population centers. Not all will follow
  // the same rule as the parent. For example, preventing looting in Whiterun
  // should also prevent looting in the Bannered Mare, but not in Whiterun
  // Sewers. Use child location keywords to control this.
  std::unordered_set<std::string> lootableChildLocations = {
      // not all Skyrim core, necessarily
      "LocTypeClearable",  "LocTypeDungeon",       "LocTypeDraugrCrypt",
      "LocTypeNordicRuin", "LocTypeSprigganGrove", "zzzBMLocVampireDungeon"};
#if _DEBUG
  ...
  for (RE::BGSLocation *location : dhnd->GetFormArray<RE::BGSLocation>()) {
    ...
    while (antecedent != nullptr) {
      ...
      if (matched != m_centers.cend()) {
        ...
        DBG_MESSAGE("{}/0x{:08x} is a descendant of population center "
                    "{}/0x{:08x} with size {}",
                    location->GetName(), location->GetFormID(),
                    antecedent->GetName(), antecedent->GetFormID(),
    ...
    // Scan location keywords to determine if lootable, or bucketed with its
    // population center antecedent
    uint32_t numKeywords(location->GetNumKeywords());
...
#endif
      if (lootableChildLocations.find(keywordName) !=
          lootableChildLocations.cend()) {
        allowLooting = true;
        REL_MESSAGE("{}/0x{:08x} is lootable child location due to keyword {}",
                    location->GetName(), location->GetFormID(), keywordName);
        break;
      }
    }
    if (allowLooting)
    ...
    // Store the child location with the same criterion as parent, unless it's
    // inherently lootable e.g. dungeon within the city limits like Whiterun
    // Sewers, parts of the Ratway
    REL_MESSAGE(
        "{}/0x{:08x} stored with same rule {} as its parent population center",
        location->GetName(), location->GetFormID(),
        PopulationCenterSizeName(parentSize));
    m_centers.insert(std::make_pair(location, parentSize));
  }
#if _DEBUG
  // this debug output from a given load order drives the list of 'really
  // lootable' child location types above
...
void PopulationCenters::AddOtherPlaces(void) {
  ...
  for (const RE::FormID formID : helgenRebornHouses) {
    const RE::TESObjectCELL *cell(
        DataCase::GetInstance()->FindExactMatch<RE::TESObjectCELL>(espName,
                                                                   formID));
    if (cell) {
      REL_MESSAGE("Cell {}/0x{:08x} treated as Town", cell->GetName(),
                  cell->GetFormID());
      m_cells.insert(
          std::make_pair(cell->GetFormID(), PopulationCenterSize::Towns));
    }
  }
  if (m_cells.size() > baseline) {
    REL_VMESSAGE("Added {} Helgen Reborn CELLs as Town, expected {}",
                 m_cells.size() - baseline, helgenRebornHouses.size());
  }
  ...
  for (const RE::FormID formID : LC_BuildYourNobleHouseHouses) {
    ...
    if (cell) {
      REL_MESSAGE(
          "LC_Build Your Noble House Cell {}/0x{:08x} treated as Settlement",
          cell->GetName(), cell->GetFormID());
      m_cells.insert(
          std::make_pair(cell->GetFormID(), PopulationCenterSize::Towns));
    }
  }
  if (m_cells.size() > baseline) {
    REL_VMESSAGE(
        "Added {} LC_Build Your Noble House CELLs as Town, expected {}",
        m_cells.size() - baseline, LC_BuildYourNobleHouseHouses.size());
  }
}
