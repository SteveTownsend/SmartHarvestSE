...
constexpr unsigned int GameCalendar::DaysPerYear() const {
  ...
  return std::accumulate(m_monthDays.cbegin(), m_monthDays.cend(), days,
                         [&](const unsigned int &total, const auto &value) {
...
std::string GameCalendar::DateString(const unsigned int days) const {
  ...
  oss << m_dayNames[dayOfWeek] << ", " << m_dayOfMonth[dayOfMonth] << " day of "
      << m_monthDays[month].first << ", 4E " << year;
  return oss.str();
}

std::string
GameCalendar::TimeString(const unsigned int timeOfDayMinutes) const {
  ...
  if (hours == 0)
    ...
    hours -= 12;
  std::ostringstream oss;
  oss << std::to_string(hours) << '.' << std::setfill('0') << std::setw(2)
      << std::to_string(minutes) << (isPM ? "pm" : "am");
  return oss.str();
}
...
std::string GameCalendar::DateTimeString(const float gameTime) const {
  ...
  dtString << TimeString(unsigned int(timeOfDay * MinutesPerDay)) << ", "
           << DateString(elapsedDays);
  return dtString.str();
}
