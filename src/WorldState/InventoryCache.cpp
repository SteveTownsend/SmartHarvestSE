...
InventoryEntry::InventoryEntry(RE::TESBoundObject *item, const int count)
    : m_item(item), m_exemption(ExcessInventoryExemption::NotExempt),
      m_count(count), m_totalDelta(0), m_maxItems(0), m_maxItemsByWeight(0),
      m_maxCount(0), m_value(0), m_saleProceeds(0), m_weight(0.0),
      m_salePercent(0.0), m_handled(0) {
  init();
}

InventoryEntry::InventoryEntry(RE::TESBoundObject *item,
                               const ExcessInventoryExemption exemption)
    : m_item(item), m_exemption(exemption), m_count(0), m_totalDelta(0),
      m_maxItems(0), m_maxItemsByWeight(0), m_maxCount(0), m_value(0),
      m_saleProceeds(0), m_weight(0.0), m_salePercent(0.0), m_handled(0) {
  init();
}

void InventoryEntry::init() {
  m_crafting = SettingsCache::Instance().HandleExcessCraftingItems() &&
               CraftingItems::Instance().IsCraftingItem(m_item);
  m_excessType = GetExcessObjectType(m_item);
  if (m_crafting) {
    m_excessHandling = SettingsCache::Instance().CraftingItemsExcessHandling();
  } else {
    m_excessHandling =
        SettingsCache::Instance().ExcessInventoryHandlingType(m_excessType);
  }
}
...
void InventoryEntry::Populate() {
  ...
  if (m_crafting) {
    m_maxItems = std::max(m_maxItems,
                          SettingsCache::Instance().CraftingItemsExcessCount());
    maxWeight = std::max(maxWeight,
                         SettingsCache::Instance().CraftingItemsExcessWeight());
  }
  ...
  DBG_DMESSAGE(
      "Excess handling for {} item {}/0x{:08x} type={}, item count={} vs max "
      "{}, per-item weight={:0.2f} vs per-item total {} -> {} items",
      (m_crafting ? "crafting" : "non-crafting"), m_item->GetName(),
      m_item->GetFormID(), GetObjectTypeName(m_excessType), m_count, m_maxItems,
      weight, maxWeight, m_maxItemsByWeight);
  m_salePercent = SettingsCache::Instance().SaleValuePercentMultiplier();
}

int InventoryEntry::Headroom(const int delta) const {
  // record item count in aggregate delta to avoid over-looting of
  // locally-abundant items in the 'Leave Behind' category
  if (m_excessHandling == ExcessInventoryHandling::LeaveBehind) {
    DBG_VMESSAGE("Check LeaveBehind for {}/0x{:08x}: delta {} vs max {}, "
                 "cached {}, total-delta {}",
                 m_item->GetName(), m_item->GetFormID(), delta, m_maxCount,
                 m_count, m_totalDelta);
  ...
  // all other cases are reconciled periodically, so take them and sort it out
  // then
  return UnlimitedItems;
}

void InventoryEntry::HandleExcess() {
  static RE::TESBoundObject *goldItem(
      RE::TESForm::LookupByID<RE::TESBoundObject>(DataCase::Gold));
  if (m_excessHandling == ExcessInventoryHandling::NoLimits ||
      m_excessHandling == ExcessInventoryHandling::LeaveBehind) {
    return;
  }
  ...
  if (m_handled > 0) {
    if (m_excessHandling == ExcessInventoryHandling::ConvertToSeptims) {
      RE::PlayerCharacter::GetSingleton()->RemoveItem(
          const_cast<RE::TESBoundObject *>(m_item), m_handled,
          RE::ITEM_REMOVE_REASON::kRemove, nullptr, nullptr);
      m_saleProceeds = static_cast<uint32_t>(
          static_cast<double>(m_handled * m_value) * m_salePercent);
      if (m_saleProceeds > 0) {
        RE::PlayerCharacter::GetSingleton()->AddObjectToContainer(
            goldItem, nullptr, m_saleProceeds, nullptr);
        DBG_VMESSAGE("Sold excess {} of {}/0x{:08x} for {} gold", m_handled,
                     m_item->GetName(), m_item->GetFormID(), m_saleProceeds);
      } else {
        DBG_VMESSAGE("Excess {} of {}/0x{:08x} discarded as worthless",
                     m_handled, m_item->GetName(), m_item->GetFormID());
      }
    } else {
      ...
      if (refr && refr->GetBaseObject()->As<RE::TESObjectCONT>()) {
        ...
        RE::PlayerCharacter::GetSingleton()->RemoveItem(
            m_item, m_handled, RE::ITEM_REMOVE_REASON::kRemove, nullptr,
            nullptr);
        refr->AddObjectToContainer(const_cast<RE::TESBoundObject *>(m_item),
                                   nullptr, m_handled, nullptr);
        DBG_VMESSAGE(
            "Moved excess {} of {}/0x{:08x} to Container {} for REFR 0x{:08x}",
            m_handled, m_item->GetName(), m_item->GetFormID(), m_transferTarget,
            refr->GetFormID());
      }
    }
  }
...
std::string InventoryEntry::Transfer(const bool excessOnly) {
  if (!UseTransferForExcess(m_excessHandling))
    return "Transfer not set up for type " +
           (m_crafting ? "crafting" : GetObjectTypeName(m_excessType));
  Populate();
  if (!excessOnly)
    m_maxCount = 0;
  HandleExcess();
  std::string transferred(
      DataCase::GetInstance()->GetTranslation("$SHSE_ITEM_TRANSFERRED"));
...
std::string InventoryEntry::Delete(const bool excessOnly) {
  ...
  HandleExcess();
  std::string deleted(
      DataCase::GetInstance()->GetTranslation("$SHSE_ITEM_DELETED"));
...
std::string InventoryEntry::Disposition() {
  ...
  oss << m_item->GetName() << " (" << (m_crafting ? "crafting" : "non-crafting")
      << '/' << GetObjectTypeName(m_excessType);
  ...
  if (m_excessHandling != ExcessInventoryHandling::NoLimits) {
    ...
    if (m_excessHandling == ExcessInventoryHandling::ConvertToSeptims)
      oss << ", sell as " << static_cast<size_t>(m_salePercent * 100.0)
          << "% of value";
