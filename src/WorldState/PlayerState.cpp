...
PlayerState::PlayerState()
    : m_perksAddLeveledItemsOnDeath(false), m_harvestedIngredientMultiplier(1.),
      m_currentlyBeefedUp(false), m_sneaking(false), m_valid(false),
      m_ownershipRule(OwnershipRule::MAX),
      m_belongingsCheck(SpecialObjectHandling::MAX),
      m_disableWhileMounted(false), m_gameTime(0.0) {
  m_carryWeightSpell = DataCase::GetInstance()->FindExactMatch<RE::SpellItem>(
      "SmartHarvestSE.esp", 0xd7d);
  if (!m_carryWeightSpell) {
    REL_FATALERROR("Cannot find Smart Harvest CarryWeight SPEL - make sure "
                   "SmartHarvestSE.esp is enabled");
    return;
  }
  REL_MESSAGE("Smart Harvest CarryWeight SPEL {}/0x{:08x}",
              m_carryWeightSpell->GetName(), m_carryWeightSpell->GetFormID());
  m_carryWeightEffect =
      DataCase::GetInstance()->FindExactMatch<RE::EffectSetting>(
          "SmartHarvestSE.esp", 0xd7e);
  if (!m_carryWeightEffect) {
    REL_FATALERROR("Cannot find Smart Harvest CarryWeight MGEF - make sure "
                   "SmartHarvestSE.esp is enabled");
    return;
  }
  REL_MESSAGE("Smart Harvest CarryWeight MGEF {}/0x{:08x}",
              m_carryWeightEffect->GetName(), m_carryWeightEffect->GetFormID());
  m_valid = true;
}

double PlayerState::SneakDistanceInterior() const {
  static const double SneakMaxDistance(
      utils::GetGameSettingFloat("fSneakMaxDistance"));
  return SneakMaxDistance;
}

double PlayerState::SneakDistanceExterior() const {
  static const double SneakMaxDistanceExterior(
      utils::GetGameSettingFloat("fSneakExteriorDistanceMult") *
      SneakDistanceInterior());
  return SneakMaxDistanceExterior;
}

void PlayerState::Refresh(const bool onMCMPush, const bool onGameReload) {
  // re-evaluate perks if timer has popped - force after game reload or settings
  // update
  ...
  // Update state cache if sneak state or settings may have changed. Affected
  // REFRs were not blacklisted so we will recheck them on next pass.
  const bool sneaking(RE::PlayerCharacter::GetSingleton()->IsSneaking());
  if (onGameReload || onMCMPush || m_sneaking != sneaking) {
    m_sneaking = sneaking;
    // no player detection by NPC is a prerequisite for autoloot of
    // crime-to-activate items
    m_ownershipRule = sneaking
                          ? SettingsCache::Instance().CrimeCheckSneaking()
                          : SettingsCache::Instance().CrimeCheckNotSneaking();
    ...
    if (!m_sneaking && SettingsCache::Instance().DisallowMiningIfSneaking()) {
      DataCase::GetInstance()->UnblockReferences(
...
void PlayerState::ReviewExcessInventory(bool force) {
#ifdef _PROFILING
  WindowsUtils::ScopedTimer elapsed(force ? "Review Excess Inventory Full"
                                          : "Review Excess Inventory Delta");
#endif
  RecursiveLockGuard guard(m_playerLock);

  // full resync if forced or last full scan was long enough ago that we want to
  // sync up, otherwise check deltas
  constexpr std::chrono::milliseconds ExcessInventoryIntervalMillis(15000LL);
  const decltype(m_lastExcessCheck) cutoffPoint(
  ...
  if (force || !updates.empty()) {
    const ContainerLister lister(INIFile::SecondaryType::deadbodies,
                                 RE::PlayerCharacter::GetSingleton());
    DBG_MESSAGE("Check Excess Inventory force={}, {} deltas", force,
                m_updates.size());
    InventoryCache validItems =
        lister.CacheIfExcessHandlingEnabled(force, updates);
    if (force) {
      m_currentItems.swap(validItems);
    } else {
      // validItems contains current data for each item - upsert to rolling
      // cache
      std::for_each(
          validItems.cbegin(), validItems.cend(), [&](const auto &current) {
            auto insertion = m_currentItems.insert(current);
            if (!insertion.second) {
              DBG_MESSAGE("Update existing cache for {}/0x{:08x}",
                          current.first->GetName(), current.first->GetFormID());
              insertion.first->second = current.second;
            } else {
              DBG_MESSAGE("Added cache entry for {}/0x{:08x}",
                          current.first->GetName(), current.first->GetFormID());
            }
          });
    }
...
void PlayerState::ReconcileCarryWeight(const bool doReload) {
  ...
  bool manageIfWeaponDrawn(
      SettingsCache::Instance().UnencumberedIfWeaponDrawn());
  ...
  if (!manageDuringCombat && !manageIfWeaponDrawn && !managePlayerHome) {
    ...
    if (managePlayerHome) {
      // when location changes to/from player house, adjust carry weight
      // accordingly
    ...
    if (manageDuringCombat) {
      bool playerInCombat(RE::PlayerCharacter::GetSingleton()->IsInCombat() &&
                          !RE::PlayerCharacter::GetSingleton()->IsDead(true));
    ...
    if (manageIfWeaponDrawn) {
      ...
      if (actorState) {
        DBG_VMESSAGE("Player WeaponState {}",
                     static_cast<uint32_t>(actorState->GetWeaponState()));
        bool isWeaponDrawn(actorState->IsWeaponDrawn());
        // when state changes between drawn/sheathed, adjust carry weight
        // accordingly
        if (isWeaponDrawn) {
          needsBeefUp = true;
  ...
  if (needsBeefUp != m_currentlyBeefedUp) {
    ...
    TaskDispatcher::Instance().EnqueueCarryWeightStateChange(doReload,
                                                             needsBeefUp);
...
bool PlayerState::CanLoot() const {
  // disable auto-looting if we are inside player house - player 'current
  // location' may be validly empty
  ...
  if (SettingsCache::Instance().DisableDuringCombat() &&
...
void PlayerState::CheckPerks(const bool force) {
  ...
  const auto cutoffPoint(
      timeNow - std::chrono::milliseconds(
                    static_cast<long long>(PerkCheckIntervalSeconds * 1000.0)));
  if (force || m_lastPerkCheck <= cutoffPoint) {
    m_perksAddLeveledItemsOnDeath = false;
    auto player(RE::PlayerCharacter::GetSingleton());
    if (player) {
      m_perksAddLeveledItemsOnDeath =
          DataCase::GetInstance()->PerksAddLeveledItemsOnDeath(player);
      DBG_MESSAGE("Leveled items added on death by perks? {}",
                  m_perksAddLeveledItemsOnDeath ? "true" : "false");
    }

    m_harvestedIngredientMultiplier =
        DataCase::GetInstance()->PerkIngredientMultiplier(player);
    DBG_VMESSAGE("Perk for harvesting -> multiplier {:0.2f}",
                 m_harvestedIngredientMultiplier);
...
void PlayerState::ExcludeMountedIfForbidden(void) {
  ...
  if (shse::LoadOrder::Instance().ModPrecedesSHSE(convenientHorsesName)) {
    REL_MESSAGE(
        "Block looting while mounted: Convenient Horses loads before SHSE");
    m_disableWhileMounted = true;
  } else if (shse::LoadOrder::Instance().IncludesMod(convenientHorsesName)) {
    REL_MESSAGE("SHSE handles looting while mounted: Convenient Horses loads "
                "after SHSE");
  }
}
...
void PlayerState::UpdateGameTime() {
  ...
  DBG_MESSAGE("GameTime is now {:0.3f}/{}", gameTime,
              GameCalendar::Instance().DateTimeString(gameTime));
  m_gameTime = gameTime;
}
...
double PlayerState::ArrowMovingThreshold() const {
  ...
  const double footUnitsPerDelay =
      SettingsCache::Instance().DelaySeconds() /
      DistanceUnitInFeet; // 1 foot per second in units, for slow time
  return IsTimeSlowed() ? footUnitsPerDelay : 5.0 * footUnitsPerDelay;
}

// returns -1 if items are marked to be left behind and inventory is full, or
// the number allowed before limits are breached
