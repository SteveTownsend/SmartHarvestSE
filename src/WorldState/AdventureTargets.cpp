...
std::string AdventureTargetName(const AdventureTargetType adventureTarget) {
  ...
  case AdventureTargetType::AnimalDen:
    return "Animal Den";
  case AdventureTargetType::AshSpawnLair:
    return "Ash Spawn Lair";
  case AdventureTargetType::AyleidRuin:
    return "Ayleid Ruin";
  case AdventureTargetType::BanditCamp:
    return "Bandit Camp";
  case AdventureTargetType::Cave:
    return "Cave";
  case AdventureTargetType::Clearable:
    return "Clearable";
  case AdventureTargetType::DragonLair:
    return "Dragon Lair";
  case AdventureTargetType::DragonPriestLair:
    return "Dragon Priest Lair";
  case AdventureTargetType::DraugrCrypt:
    return "Draugr Crypt";
  case AdventureTargetType::Dungeon:
    return "Dungeon";
  case AdventureTargetType::DwarvenRuin:
    return "Dwarven Ruin";
#if _DEBUG
  case AdventureTargetType::FakeForTesting:
    return "Fake for Testing";
#endif
  case AdventureTargetType::FalmerHive:
    return "Falmer Hive";
  case AdventureTargetType::ForswornCamp:
    return "Forsworn Camp";
  case AdventureTargetType::Fort:
    return "Fort";
  case AdventureTargetType::GiantCamp:
    return "Giant Camp";
  case AdventureTargetType::GoblinDen:
    return "Goblin Den";
  case AdventureTargetType::Grove:
    return "Grove";
  case AdventureTargetType::HagravenNest:
    return "Hagraven Nest";
  case AdventureTargetType::Mine:
    return "Mine";
  case AdventureTargetType::NordicRuin:
    return "Nordic Ruin";
  case AdventureTargetType::OgreDen:
    return "Ogre Den";
  case AdventureTargetType::OrcStronghold:
    return "Orc Stronghold";
  case AdventureTargetType::RieklingCamp:
    return "Riekling Camp";
  case AdventureTargetType::RuinedFort:
    return "Ruined Fort";
  case AdventureTargetType::Settlement:
    return "Settlement";
  case AdventureTargetType::Shipwreck:
    return "Shipwreck";
  case AdventureTargetType::VampireLair:
    return "Vampire Lair";
  case AdventureTargetType::WerebeastLair:
    return "Werebeast Lair";
  case AdventureTargetType::WarlockLair:
    return "Warlock Lair";
  case AdventureTargetType::WitchmanLair:
    return "Witchman Lair";
  default:
    return "";
  };
}
...
void AdventureEvent::ResetSagaState() { m_lastTarget = nullptr; }

AdventureEvent AdventureEvent::StartedAdventure(const RE::TESWorldSpace *world,
...
AdventureEvent AdventureEvent::StartAdventure(const RE::TESWorldSpace *world,
                                              const RE::BGSLocation *location) {
...
AdventureEvent::AdventureEvent(const AdventureEventType eventType,
                               ...
                               const RE::BGSLocation *location,
                               const float gameTime)
    : m_eventType(eventType), m_world(world), m_location(location),
      m_gameTime(gameTime) {}
...
AdventureEvent::AdventureEvent(const AdventureEventType eventType,
                               ...
                               const RE::BGSLocation *location)
    : m_eventType(eventType), m_world(world), m_location(location),
      m_gameTime(PlayerState::Instance().CurrentGameTime()) {}
AdventureEvent::AdventureEvent(const AdventureEventType eventType,
                               const float gameTime)
    : m_eventType(eventType), m_world(nullptr), m_location(nullptr),
      m_gameTime(gameTime) {}
...
std::string AdventureEvent::AsString() const {
  ...
  if (m_eventType == AdventureEventType::Started) {
    stream << "My Adventure to " << m_location->GetName() << " in "
           << m_world->GetName() << " began.";
...
AdventureTargets::AdventureTargets()
    : m_targetLocation(nullptr), m_targetWorld(nullptr) {}
...
Position AdventureTargets::GetInteriorCellPosition(
    const RE::TESObjectCELL *cell, const RE::BGSLocation *location) const {
  // check for direct CELL linkage to outside world
  for (const auto cellRefr : cell->GetRuntimeData().references) {
    if (cellRefr && cellRefr->GetBaseObject() &&
        cellRefr->GetBaseObject()->GetFormType() == RE::FormType::Door) {
      DBG_MESSAGE("Interior CELL 0x{:08x} has DOOR 0x{:08x}", cell->GetFormID(),
                  cellRefr->GetFormID());
      ...
      if (cellRefr->extraList.HasType<RE::ExtraTeleport>()) {
        ...
        const RE::ExtraTeleport *xtel(
            cellRefr->extraList.GetByType<RE::ExtraTeleport>());
        if (xtel && xtel->teleportData) {
          RE::TESObjectREFR *egressRefr(
              xtel->teleportData->linkedDoor.get().get());
          if (egressRefr) {
            DBG_MESSAGE("DOOR 0x{:08x} XTEL has linked door 0x{:08x}",
                        cellRefr->GetFormID(), egressRefr->GetFormID());
            // Parent Cell is blank for WRLD persistent references, assumed
            // exterior. If CELL is present it must be outdoors.
            if (!egressRefr->GetParentCell() ||
                egressRefr->GetParentCell()->IsExteriorCell()) {
              LinkLocationToWorld(location, egressRefr->GetWorldspace());
              DBG_MESSAGE("Interior CELL 0x{:08x} has egress via DOOR 0x{:08x} "
                          "at Position ({:.2f},{:.2f},{:.2f})",
                          cell->GetFormID(), egressRefr->GetFormID(),
                          egressRefr->GetPositionX(),
                          egressRefr->GetPositionY(),
                          egressRefr->GetPositionZ());
              return Position({egressRefr->GetPositionX(),
                               egressRefr->GetPositionY(),
                               egressRefr->GetPositionZ()});
            }
          } else {
            DBG_MESSAGE("DOOR 0x{:08x} XTEL linked door 0x{:08x} invalid",
                        cellRefr->GetFormID(),
                        xtel->teleportData->linkedDoor.native_handle());
          }
        }
      }
    }
...
Position
AdventureTargets::GetRefHandlePosition(const RE::ObjectRefHandle handle,
...
RE::TESWorldSpace *
AdventureTargets::GetRefHandleWorld(const RE::ObjectRefHandle handle) const {
  if (!handle)
    return nullptr;
  const RE::TESObjectREFR *refr(handle.get().get());
...
Position
AdventureTargets::GetRefIDPosition(const RE::FormID refID,
                                   const RE::BGSLocation *location) const {
  const RE::TESObjectREFR *refr(
      RE::TESForm::LookupByID<RE::TESObjectREFR>(refID));
  return GetRefrPosition(refr, location);
}

Position
AdventureTargets::GetRefrPosition(const RE::TESObjectREFR *refr,
                                  const RE::BGSLocation *location) const {
  if (!refr)
    return InvalidPosition;
  // Parent Cell is blank for WRLD persistent references, assumed exterior. If
  // CELL is present it must be outdoors.
  if (!refr->GetParentCell() || refr->GetParentCell()->IsExteriorCell()) {
    LinkLocationToWorld(location, refr->GetWorldspace());
    DBG_MESSAGE("Exterior REFR 0x{:08x} has Position ({:.2f},{:.2f},{:.2f})",
                refr->GetFormID(), refr->GetPositionX(), refr->GetPositionY(),
                refr->GetPositionZ());
    return Position(
...
void AdventureTargets::LinkLocationToWorld(
    const RE::BGSLocation *location, const RE::TESWorldSpace *world) const {
  if (!location || !world)
    return;
  ...
  if (inserted.second) {
    DBG_MESSAGE("Found worldspace {}/0x{:08x} for location {}/0x{:08x}",
                world->GetName(), world->GetFormID(), location->GetName(),
                location->GetFormID());
  } else if (inserted.first->second != world) {
    REL_WARNING("Found second worldspace {}/0x{:08x} for location {}/0x{:08x}, "
                "already have {}/0x{:08x}",
                world->GetName(), world->GetFormID(), location->GetName(),
                location->GetFormID(), inserted.first->second->GetName(),
                inserted.first->second->GetFormID());
  }
}
...
void AdventureTargets::Categorize() {
  ...
  // determine Worldspace for each Location - first scan Worldspace Location
  // lists skip this one
  ...
  for (RE::TESWorldSpace *world :
       RE::TESDataHandler::GetSingleton()->GetFormArray<RE::TESWorldSpace>()) {
    if (world->GetFormID() == AvoidanceExterior ||
        world->GetFullNameLength() == 0)
      continue;
    if (world->GetFormID() == SkyrimWorldspace) {
      RE::BGSLocation *tamriel(
          RE::TESForm::LookupByID<RE::BGSLocation>(TamrielLocation));
      if (tamriel) {
    ...
    for (const auto location : world->locationMap) {
      ...
      if (!location.second->GetFullNameLength()) {
        DBG_MESSAGE("Skip unnamed WRLD Location 0x{:08x}",
                    location.second->GetFormID());
        continue;
      }
  ...
  // now scan full Location list and match each Location with its WorldSpace,
  // and find Position as XYZ Coordinates
  for (const RE::BGSLocation *location :
       RE::TESDataHandler::GetSingleton()->GetFormArray<RE::BGSLocation>()) {
    ...
    if (location->worldLocMarker) {
      ...
      if (worldSpace) {
        DBG_MESSAGE("World Location Marker 0x{:08x} implies worldspace "
                    "{}/0x{:08x} for location {}/0x{:08x}",
                    location->worldLocMarker.get()->GetFormID(),
                    worldSpace->GetName(), worldSpace->GetFormID(),
                    location->GetName(), location->GetFormID());
      }
      LinkLocationToWorld(location, worldSpace);
    }

    // check ACPR/LCPR, if WRLD is referenced anywhere in the parent-child tree
    // that is the answer
    if (location->overrideData) {
      DBG_MESSAGE("LCPR found for location {}/0x{:08x}", location->GetName(),
                  location->GetFormID());
      ...
      DBG_MESSAGE("No LCPR for location {}/0x{:08x}", location->GetName(),
                  location->GetFormID());
    }
    ...
    if (position == InvalidPosition) {
      ...
      for (const auto &csr : location->specialRefs) {
        ...
        // location-ref-type (LCRT) is a keyword, we want Map Marker
        // (preferred), Location Center or Outside Entrance
        if (csr.type->GetFormID() == MapMarkerLCRT) {
          ...
          if (cell) {
            if (cell->IsInteriorCell()) {
              ...
              if (newPosition != InvalidPosition) {
                DBG_MESSAGE("Location {}/0x{:08x} is Interior CELL 0x{:08x}",
                            location->GetName(), location->GetFormID(),
                            parentID);
              ...
              Position newPosition(
                  GetRefIDPosition(csr.refData.refID, location));
              if (newPosition != InvalidPosition) {
                DBG_MESSAGE("Location {}/0x{:08x} is in Exterior CELL 0x{:08x}",
                            location->GetName(), location->GetFormID(),
                            parentID);
                fallbackPosition = newPosition;
              }
            ...
            Position newPosition(GetRefIDPosition(csr.refData.refID, location));
            if (newPosition != InvalidPosition) {
              DBG_MESSAGE("Location {}/0x{:08x} is in WRLD 0x{:08x}",
                          location->GetName(), location->GetFormID(), parentID);
              fallbackPosition = newPosition;
            }
            LinkLocationToWorld(location, parentSpace->As<RE::TESWorldSpace>());
          } else {
            DBG_MESSAGE("Location {}/0x{:08x} center/entrance skipped",
                        location->GetName(), location->GetFormID());
          }
        }
      }
    ...
    if (position == InvalidPosition) {
      if (fallbackPosition == InvalidPosition) {
        DBG_MESSAGE("Location {}/0x{:08x} cannot be Placed in world",
                    location->GetName(), location->GetFormID());
        continue;
      }
      position = fallbackPosition;
    }
    DBG_MESSAGE("Location {}/0x{:08x} at ({:.2f},{:.2f},{:.2f})",
                location->GetName(), location->GetFormID(), position[0],
  ...
  for (const auto &markedLocation : m_locationCoordinates) {
    ...
    if (matched != m_worldByLocation.cend()) {
      ...
      if (worldList == m_markedLocationsByWorld.end()) {
        worldList =
            m_markedLocationsByWorld.insert({matched->second, {}}).first;
      }
      worldList->second.insert(location);
      DBG_MESSAGE("Mapped worldspace {}/0x{:08x} for location {}/0x{:08x}",
                  worldList->first->GetName(), worldList->first->GetFormID(),
                  location->GetName(), location->GetFormID());
    }
  }
  ...
  for (const auto locationsForType : m_locationsByType) {
    if (locationsForType.empty()) {
      REL_WARNING("No Adventure Targets for type {}",
                  AdventureTargetName(adventureType));
    } else {
      REL_MESSAGE("{} Adventure Targets for type {}", locationsForType.size(),
                  AdventureTargetName(adventureType));
      targets += locationsForType.size();
      for (const RE::BGSLocation *location : locationsForType) {
        REL_MESSAGE("  {}/0x{:08x}", location->GetName(),
                    location->GetFormID());
      }
    }
    adventureType =
        static_cast<AdventureTargetType>(uint32_t(adventureType) + 1);
  }
  REL_MESSAGE("{} Adventure Targets created from {} unique Locations", targets,
              RE::TESDataHandler::GetSingleton()
                  ->GetFormArray<RE::BGSLocation>()
                  .size());
}

std::string
AdventureTargets::AdventureTypeName(const size_t adventureType) const {
  if (adventureType >= m_validAdventureTypes.size()) {
    return "";
  }
  return AdventureTargetNameByIndex(
      size_t(m_validAdventureTypes[adventureType]));
}
...
size_t AdventureTargets::AvailableAdventureTypes() const {
  ...
  for (const auto locations : m_locationsByType) {
    if (std::find_if(locations.cbegin(), locations.cend(),
                     [=](const RE::BGSLocation *location) -> bool {
                       return !VisitedPlaces::Instance().IsKnown(location);
                     }) != locations.cend()) {
      // establish mapping between MCM index and original adventure type if
      // there are unknown locations for this type
...
AdventureTargets::GetWorldMarkedPlaces(const RE::TESWorldSpace *world) const {
  RecursiveLockGuard guard(m_adventureLock);
    ...
    for (const RE::BGSLocation *location : worldLocations->second) {
      ...
      if (locationCoordinates != m_locationCoordinates.cend()) {
        ...
        if (markedPlaces.insert(std::make_pair(location, position)).second) {
          DBG_VMESSAGE(
              "Location {}/0x{:08x} at coordinates ({:0.2f},{:0.2f},{:0.2f})",
              location->GetName(), location->GetFormID(), position[0],
              position[1], position[2]);
        } else {
          DBG_VMESSAGE("Location {}/0x{:08x} already recorded",
                       location->GetName(), location->GetFormID());
        }
      } else {
        DBG_VMESSAGE("Location {}/0x{:08x} has no map marker",
                     location->GetName(), location->GetFormID());
      }
    }
  }
...
// This can only be called from MCM so thread-safe. Build view containing the
// viable Worlds/Locations for this type.
size_t AdventureTargets::ViableWorldCount(const size_t adventureType) const {
  ...
  if (adventureType >= m_validAdventureTypes.size())
    return 0;
  // map from MCM index for list of adventure types with valid worlds back to
  // enumeration
  for (const RE::BGSLocation *location :
       m_locationsByType[size_t(m_validAdventureTypes[adventureType])]) {
    ...
    if (worldLocations == m_unvisitedLocationsByWorld.cend()) {
      worldLocations =
          m_unvisitedLocationsByWorld.insert({worldSpace, {}}).first;
  ...
  std::sort(
      m_sortedWorlds.begin(), m_sortedWorlds.end(),
      [=](const RE::TESWorldSpace *lhs, const RE::TESWorldSpace *rhs) -> bool {
        std::string leftName(lhs->GetName());
        std::string rightName(rhs->GetName());
        return std::lexicographical_compare(leftName.cbegin(), leftName.cend(),
                                            rightName.cbegin(),
                                            rightName.cend());
      });
...
std::string
AdventureTargets::ViableWorldNameByIndexInView(const size_t worldIndex) const {
  RecursiveLockGuard guard(m_adventureLock);
  if (worldIndex >= m_sortedWorlds.size()) {
    return "";
  }
  DBG_VMESSAGE("WorldSpace {} at index {}",
               m_sortedWorlds[worldIndex]->GetName(), worldIndex);
  std::string name(m_sortedWorlds[worldIndex]->GetName());
  if (m_sortedWorlds[worldIndex] ==
      LocationTracker::Instance().CurrentPlayerWorld()) {
...
void AdventureTargets::SelectCurrentDestination(const size_t worldIndex) {
  ...
  if (worldLocations == m_unvisitedLocationsByWorld.cend())
    return;
  std::vector<const RE::BGSLocation *> candidates(
      worldLocations->second.cbegin(), worldLocations->second.cend());
  if (candidates.empty())
    return;
  std::random_device
  ...
  DBG_MESSAGE("Adventure started to random location {}/0x{:08x} of {} "
              "candidates in WorldSpace {}/0x{:08x}",
              m_targetLocation->GetName(), m_targetLocation->GetFormID(),
              candidates.size(), m_targetWorld->GetName(),
              m_targetWorld->GetFormID());
  RecordEvent(AdventureEvent::StartAdventure(m_targetWorld, m_targetLocation));
}

void AdventureTargets::CheckReachedCurrentDestination(
    const RE::BGSLocation *newLocation) {
  RecursiveLockGuard guard(m_adventureLock);
  if (m_targetLocation) {
    if (newLocation == m_targetLocation) {
      DBG_MESSAGE("Completed Adventure to location {}/0x{:08x} in WorldSpace "
                  "{}/0x{:08x}",
                  m_targetLocation->GetName(), m_targetLocation->GetFormID(),
                  m_targetWorld->GetName(), m_targetWorld->GetFormID());
      RecordEvent(AdventureEvent::CompleteAdventure());
      static RE::BSFixedString arrivalMsg(papyrus::GetTranslation(
          nullptr, RE::BSFixedString("$SHSE_ADVENTURE_ARRIVED")));
      if (!arrivalMsg.empty()) {
...
void AdventureTargets::AbandonCurrentDestination() {
  ...
  if (m_targetLocation) {
    DBG_MESSAGE(
        "Abandoned Adventure to location {}/0x{:08x} in WorldSpace {}/0x{:08x}",
        m_targetLocation->GetName(), m_targetLocation->GetFormID(),
        m_targetWorld->GetName(), m_targetWorld->GetFormID());
    m_targetLocation = nullptr;
    m_targetWorld = nullptr;
...
Position AdventureTargets::TargetPosition(void) const {
  ...
  if (matched != m_locationCoordinates.cend()) {
    DBG_MESSAGE(
        "Adventure location {}/0x{:08x} has Position ({:.2f},{:.2f},{:.2f})",
        m_targetLocation->GetName(), m_targetLocation->GetFormID(),
        matched->second[0], matched->second[1], matched->second[2]);
...
void AdventureTargets::AsJSON(nlohmann::json &j) const {
  ...
  if (m_targetLocation) {
    j["currentLocation"] =
        StringUtils::FromFormID(m_targetLocation->GetFormID());
...
void AdventureTargets::UpdateFrom(const nlohmann::json &j) {
  ...
  for (const nlohmann::json &adventureEvent : j["events"]) {
    ...
    const AdventureEventType eventType(
        static_cast<AdventureEventType>(adventureEvent["event"].get<int>()));
    const auto eventWorldspace(adventureEvent.find("world"));
    const RE::TESWorldSpace *worldspaceForm(
        eventWorldspace != adventureEvent.cend()
            ? LoadOrder::Instance().RehydrateCosaveFormAs<RE::TESWorldSpace>(
                  StringUtils::ToFormID(eventWorldspace->get<std::string>()))
            : nullptr);
    const auto eventLocation(adventureEvent.find("location"));
    const RE::BGSLocation *locationForm(
        eventLocation != adventureEvent.cend()
            ? LoadOrder::Instance().RehydrateCosaveFormAs<RE::BGSLocation>(
                  StringUtils::ToFormID(eventLocation->get<std::string>()))
            : nullptr);
    ...
    case AdventureEventType::Started:
      RecordEvent(AdventureEvent::StartedAdventure(worldspaceForm, locationForm,
                                                   gameTime));
      break;
